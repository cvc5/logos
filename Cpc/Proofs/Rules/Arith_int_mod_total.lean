...
private abbrev modTerm (t s : Term) : Term :=
  Term.Apply (Term.Apply Term.mod t) s

private abbrev modTotalTerm (t s : Term) : Term :=
  Term.Apply (Term.Apply Term.mod_total t) s

private abbrev modTotalPremise (s : Term) : Term :=
  Term.Apply
    (Term.Apply Term.eq
      (Term.Apply (Term.Apply Term.eq s) (Term.Numeral 0)))
    (Term.Boolean false)

private abbrev modTotalConclusion (t s : Term) : Term :=
  Term.Apply (Term.Apply Term.eq (modTerm t s)) (modTotalTerm t s)

private theorem eo_to_smt_mod_eq (x y : Term) :
    __eo_to_smt (modTerm x y) =
      SmtTerm.mod (__eo_to_smt x) (__eo_to_smt y) := by
  rfl
...
private theorem smtx_eval_mod_term_eq
...
private theorem prog_arith_int_mod_total_info
    (t s P : Term)
    (hProg : __eo_prog_arith_int_mod_total t s (Proof.pf P) ≠ Term.Stuck) :
    ∃ s0,
      P = modTotalPremise s0 ∧
      s0 = s ∧
      __eo_prog_arith_int_mod_total t s (Proof.pf P) =
        modTotalConclusion t s := by
  unfold __eo_prog_arith_int_mod_total at hProg
  ...
  next a b c d h1 h2 heq =>
    ...
    subst hd
    refine ⟨_, rfl, rfl, ?_⟩
    simp [__eo_prog_arith_int_mod_total, __eo_requires, __eo_eq,
      SmtEval.native_ite, native_teq, SmtEval.native_not,
      modTotalConclusion, modTerm, modTotalTerm]

private theorem typeof_args_of_prog_arith_int_mod_total_bool
    (t s P : Term)
    (hTTrans : RuleProofs.eo_has_smt_translation t)
    (hSTrans : RuleProofs.eo_has_smt_translation s)
    (hProgEq :
      __eo_prog_arith_int_mod_total t s (Proof.pf P) =
        modTotalConclusion t s)
    (hResultTy :
      __eo_typeof (__eo_prog_arith_int_mod_total t s (Proof.pf P)) =
        Term.Bool) :
    __eo_typeof t = Term.Int ∧ __eo_typeof s = Term.Int := by
  rw [hProgEq] at hResultTy
  change __eo_typeof_eq (__eo_typeof (modTerm t s))
      (__eo_typeof (modTotalTerm t s)) = Term.Bool at hResultTy
...
private theorem typed___eo_prog_arith_int_mod_total_impl
    (t s P : Term) :
    ...
    RuleProofs.eo_has_bool_type
      (__eo_prog_arith_int_mod_total t s (Proof.pf P)) := by
  intro hTTrans hSTrans hProgEq hResultTy
  have hArgsTy :=
    typeof_args_of_prog_arith_int_mod_total_bool t s P hTTrans hSTrans
      hProgEq hResultTy
  ...
  have hModTy :
      __smtx_typeof (__eo_to_smt (modTerm t s)) = SmtType.Int := by
    rw [eo_to_smt_mod_eq, typeof_mod_eq]
  ...
  have hModTotalTy :
    ...
    simp [native_ite, native_Teq, hTSmtTy, hSSmtTy]
  have hModTrans : RuleProofs.eo_has_smt_translation (modTerm t s) := by
    simp [RuleProofs.eo_has_smt_translation, hModTy]
  rw [hProgEq]
  exact RuleProofs.eo_has_bool_type_eq_of_same_smt_type
    (modTerm t s) (modTotalTerm t s)
...
private theorem facts___eo_prog_arith_int_mod_total_impl
    ...
    __eo_typeof (__eo_prog_arith_int_mod_total t s (Proof.pf P)) =
      Term.Bool ->
    eo_interprets M (modTotalPremise s) true ->
    eo_interprets M (__eo_prog_arith_int_mod_total t s (Proof.pf P)) true := by
  intro hTTrans hSTrans hProgEq hResultTy hPrem
  have hProgBool :
      RuleProofs.eo_has_bool_type
        (__eo_prog_arith_int_mod_total t s (Proof.pf P)) :=
    typed___eo_prog_arith_int_mod_total_impl t s P
      hTTrans hSTrans hProgEq hResultTy
  have hProgBool' :
      RuleProofs.eo_has_bool_type (modTotalConclusion t s) := by
  ...
  have hEvalMod :
      __smtx_model_eval M (__eo_to_smt (modTerm t s)) =
        SmtValue.Numeral (native_mod_total ti si) := by
    rw [eo_to_smt_mod_eq, smtx_eval_mod_term_eq]
    rw [hSEval, hTEval]
    simp [__smtx_model_eval_eq, __smtx_model_eval_ite,
      __smtx_model_eval_mod_total, native_veq, hSNeZeroDecide]
  have hEvalModTotal :
  ...
  exact RuleProofs.eo_interprets_eq_of_rel M
    (modTerm t s) (modTotalTerm t s) hProgBool' <| by
      rw [hEvalMod, hEvalModTotal]
...
public theorem cmd_step_arith_int_mod_total_properties
  ...
  __eo_typeof (__eo_cmd_step_proven s CRule.arith_int_mod_total args premises) = Term.Bool ->
  StepRuleProperties M (premiseTermList s premises)
    (__eo_cmd_step_proven s CRule.arith_int_mod_total args premises) :=
by
          ...
          | nil =>
              ...
              | cons p premises =>
                  ...
                  | nil =>
                      ...
                      change __eo_typeof
                          (__eo_prog_arith_int_mod_total T1 S1 (Proof.pf P)) =
                        Term.Bool at hResultTy
                      change __eo_prog_arith_int_mod_total T1 S1 (Proof.pf P) ≠
                        Term.Stuck at hProg
                      rcases prog_arith_int_mod_total_info T1 S1 P hProg with
                      ...
                      · change RuleProofs.eo_has_smt_translation
                        ...
                        exact RuleProofs.eo_has_smt_translation_of_has_bool_type
                          (__eo_prog_arith_int_mod_total T1 S1 (Proof.pf P))
                          (typed___eo_prog_arith_int_mod_total_impl
