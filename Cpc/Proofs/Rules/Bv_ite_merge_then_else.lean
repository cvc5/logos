...
private def mergeThenElseCond (c1 c2 : Term) : Term :=
  bvAnd (bvNot c1) (bvAnd (bvNot c2) bvOne)

private def mergeThenElseLhs (c1 c2 t1 e1 : Term) : Term :=
  bvIte c1 t1 (bvIte c2 t1 e1)

private def mergeThenElseRhs (c1 c2 t1 e1 : Term) : Term :=
  bvIte (mergeThenElseCond c1 c2) e1 t1

private theorem prog_bv_ite_merge_then_else_eq_of_ne_stuck
    (c1 c2 t1 e1 : Term) :
    c1 ≠ Term.Stuck ->
    c2 ≠ Term.Stuck ->
    t1 ≠ Term.Stuck ->
    e1 ≠ Term.Stuck ->
    __eo_prog_bv_ite_merge_then_else c1 c2 t1 e1 =
      Term.Apply (Term.Apply (Term.UOp UserOp.eq)
        (mergeThenElseLhs c1 c2 t1 e1))
        (mergeThenElseRhs c1 c2 t1 e1) := by
  intro hC1 hC2 hT1 hE1
  rw [__eo_prog_bv_ite_merge_then_else.eq_5 c1 c2 t1 e1 hC1 hC2 hT1 hE1]
  rfl

private theorem typeof_args_of_prog_bv_ite_merge_then_else_bool
    (c1 c2 t1 e1 : Term) :
    c1 ≠ Term.Stuck ->
    c2 ≠ Term.Stuck ->
    t1 ≠ Term.Stuck ->
    e1 ≠ Term.Stuck ->
    __eo_typeof (__eo_prog_bv_ite_merge_then_else c1 c2 t1 e1) = Term.Bool ->
    __eo_typeof c1 =
  ...
  rw [prog_bv_ite_merge_then_else_eq_of_ne_stuck c1 c2 t1 e1 hC1 hC2
    hT1 hE1] at hTy
  change __eo_typeof_eq
      (__eo_typeof_bvite (__eo_typeof c1)
        ...
        (__eo_typeof_bvite (__eo_typeof c2) (__eo_typeof t1)
          (__eo_typeof e1)))
      (__eo_typeof_bvite (__eo_typeof (mergeThenElseCond c1 c2))
        (__eo_typeof e1) (__eo_typeof t1)) = Term.Bool at hTy
  have hOperandsNN :=
...
private theorem smt_typeof_merge_then_else_cond
    (c1 c2 : Term) :
    __smtx_typeof (__eo_to_smt c1) = SmtType.BitVec 1 ->
    __smtx_typeof (__eo_to_smt c2) = SmtType.BitVec 1 ->
    __smtx_typeof (__eo_to_smt (mergeThenElseCond c1 c2)) =
      SmtType.BitVec 1 := by
  intro hC1Ty hC2Ty
  have hNotC1 : __smtx_typeof (__eo_to_smt (bvNot c1)) = SmtType.BitVec 1 :=
    smt_typeof_bvnot_bitvec_one c1 hC1Ty
  have hNotC2 : __smtx_typeof (__eo_to_smt (bvNot c2)) = SmtType.BitVec 1 :=
    smt_typeof_bvnot_bitvec_one c2 hC2Ty
  have hInner :
      __smtx_typeof (__eo_to_smt (bvAnd (bvNot c2) bvOne)) =
        SmtType.BitVec 1 :=
    smt_typeof_bvand_bitvec_one (bvNot c2) bvOne hNotC2 smt_typeof_bvone
  exact smt_typeof_bvand_bitvec_one (bvNot c1) (bvAnd (bvNot c2) bvOne)
    hNotC1 hInner

private theorem smt_typeof_then_else_left_right
    (c1 c2 t1 e1 : Term) :
    ...
    __eo_typeof (__eo_prog_bv_ite_merge_then_else c1 c2 t1 e1) = Term.Bool ->
    __smtx_typeof (__eo_to_smt (mergeThenElseLhs c1 c2 t1 e1)) =
      __smtx_typeof (__eo_to_smt (mergeThenElseRhs c1 c2 t1 e1)) := by
  ...
  rcases typeof_args_of_prog_bv_ite_merge_then_else_bool c1 c2 t1 e1
  ...
  have hLeftTy :
      __smtx_typeof (__eo_to_smt (mergeThenElseLhs c1 c2 t1 e1)) =
        __smtx_typeof (__eo_to_smt t1) :=
    smt_typeof_bvite_of_smt_types c1 t1 (bvIte c2 t1 e1)
      (__smtx_typeof (__eo_to_smt t1)) hC1SmtTy rfl hInnerTy
  have hCondTy :
      __smtx_typeof (__eo_to_smt (mergeThenElseCond c1 c2)) =
        SmtType.BitVec 1 :=
    smt_typeof_merge_then_else_cond c1 c2 hC1SmtTy hC2SmtTy
  have hRightTy :
      __smtx_typeof (__eo_to_smt (mergeThenElseRhs c1 c2 t1 e1)) =
        __smtx_typeof (__eo_to_smt t1) :=
    smt_typeof_bvite_of_smt_types (mergeThenElseCond c1 c2) e1 t1
      (__smtx_typeof (__eo_to_smt t1)) hCondTy hE1Same rfl
  rw [hLeftTy, hRightTy]

private theorem smt_typeof_then_else_left_non_none
    (c1 c2 t1 e1 : Term) :
    RuleProofs.eo_has_smt_translation c1 ->
    RuleProofs.eo_has_smt_translation c2 ->
    RuleProofs.eo_has_smt_translation t1 ->
...
private theorem typed___eo_prog_bv_ite_merge_then_else_impl
    (c1 c2 t1 e1 : Term) :
    RuleProofs.eo_has_smt_translation c1 ->
    RuleProofs.eo_has_smt_translation c2 ->
    RuleProofs.eo_has_smt_translation t1 ->
    ...
    RuleProofs.eo_has_bool_type
      (__eo_prog_bv_ite_merge_then_else c1 c2 t1 e1) := by
  intro hC1Trans hC2Trans hT1Trans hE1Trans hResultTy
  have hC1NotStuck : c1 ≠ Term.Stuck :=
    RuleProofs.term_ne_stuck_of_has_smt_translation c1 hC1Trans
  have hC2NotStuck : c2 ≠ Term.Stuck :=
  ...
  have hE1NotStuck : e1 ≠ Term.Stuck :=
    RuleProofs.term_ne_stuck_of_has_smt_translation e1 hE1Trans
  rw [prog_bv_ite_merge_then_else_eq_of_ne_stuck c1 c2 t1 e1 hC1NotStuck
    hC2NotStuck hT1NotStuck hE1NotStuck]
  exact RuleProofs.eo_has_bool_type_eq_of_same_smt_type
    (mergeThenElseLhs c1 c2 t1 e1) (mergeThenElseRhs c1 c2 t1 e1)
    (smt_typeof_then_else_left_right c1 c2 t1 e1 hC1Trans hC2Trans
      hT1Trans hE1Trans hResultTy)
    (smt_typeof_then_else_left_non_none c1 c2 t1 e1 hC1Trans hC2Trans
      hT1Trans hE1Trans hResultTy)
...
private theorem eval_merge_then_else_cond
    ...
    __smtx_model_eval M (__eo_to_smt (mergeThenElseCond c1 c2)) =
      bv1 (!b1 && !b2) := by
  ...
  have hNotC2Eval :
      __smtx_model_eval M (__eo_to_smt (bvNot c2)) = bv1 (!b2) :=
    eval_bvnot_bv1_term M c2 b2 hC2Eval
  have hInnerEval :
      __smtx_model_eval M (__eo_to_smt (bvAnd (bvNot c2) bvOne)) =
        bv1 ((!b2) && true) :=
    eval_bvand_term M (bvNot c2) bvOne (!b2) true hNotC2Eval
      (eval_bvone M)
  have hOuterEval :
      __smtx_model_eval M (__eo_to_smt (mergeThenElseCond c1 c2)) =
        bv1 ((!b1) && ((!b2) && true)) :=
    eval_bvand_term M (bvNot c1) (bvAnd (bvNot c2) bvOne) (!b1)
...
private theorem eval_bvite_merge_then_else
    ...
    RuleProofs.eo_has_smt_translation e1 ->
    __eo_typeof (__eo_prog_bv_ite_merge_then_else c1 c2 t1 e1) = Term.Bool ->
    __smtx_model_eval M (__eo_to_smt (mergeThenElseLhs c1 c2 t1 e1)) =
      __smtx_model_eval M (__eo_to_smt (mergeThenElseRhs c1 c2 t1 e1)) := by
  intro hC1Trans hC2Trans hT1Trans hE1Trans hResultTy
  have hC1NotStuck : c1 ≠ Term.Stuck :=
    RuleProofs.term_ne_stuck_of_has_smt_translation c1 hC1Trans
  have hC2NotStuck : c2 ≠ Term.Stuck :=
  ...
  have hCondEval :
      __smtx_model_eval M (__eo_to_smt (mergeThenElseCond c1 c2)) =
        bv1 (!b1 && !b2) :=
    eval_merge_then_else_cond M c1 c2 b1 b2 hC1Eval hC2Eval
  have hCondEq :=
    eval_eq_bv1_one M (mergeThenElseCond c1 c2) (!b1 && !b2) hCondEval
  change __smtx_model_eval M
    ...
    __smtx_model_eval M
      (SmtTerm.ite
        (SmtTerm.eq (__eo_to_smt (mergeThenElseCond c1 c2))
          (SmtTerm.Binary 1 1))
        (__eo_to_smt e1) (__eo_to_smt t1))
  rw [smtx_eval_ite_term_eq, hC1Eq]
  rw [smtx_eval_ite_term_eq, hC2Eq]
  rw [smtx_eval_ite_term_eq, hCondEq]
  cases b1 <;> cases b2 <;> rfl

private theorem facts___eo_prog_bv_ite_merge_then_else_impl
    (M : SmtModel) (hM : model_wf M) (c1 c2 t1 e1 : Term) :
    RuleProofs.eo_has_smt_translation c1 ->
    RuleProofs.eo_has_smt_translation c2 ->
    RuleProofs.eo_has_smt_translation t1 ->
    RuleProofs.eo_has_smt_translation e1 ->
    __eo_typeof (__eo_prog_bv_ite_merge_then_else c1 c2 t1 e1) = Term.Bool ->
    eo_interprets M (__eo_prog_bv_ite_merge_then_else c1 c2 t1 e1) true := by
  intro hC1Trans hC2Trans hT1Trans hE1Trans hResultTy
  have hC1NotStuck : c1 ≠ Term.Stuck :=
    RuleProofs.term_ne_stuck_of_has_smt_translation c1 hC1Trans
  have hC2NotStuck : c2 ≠ Term.Stuck :=
  ...
  have hProgBool : RuleProofs.eo_has_bool_type
      (__eo_prog_bv_ite_merge_then_else c1 c2 t1 e1) :=
    typed___eo_prog_bv_ite_merge_then_else_impl c1 c2 t1 e1 hC1Trans
      hC2Trans hT1Trans hE1Trans hResultTy
  rw [prog_bv_ite_merge_then_else_eq_of_ne_stuck c1 c2 t1 e1 hC1NotStuck
    hC2NotStuck hT1NotStuck hE1NotStuck]
  apply RuleProofs.eo_interprets_eq_of_rel M
  · simpa [prog_bv_ite_merge_then_else_eq_of_ne_stuck c1 c2 t1 e1
      hC1NotStuck hC2NotStuck hT1NotStuck hE1NotStuck] using hProgBool
  · change RuleProofs.smt_value_rel
      (__smtx_model_eval M (__eo_to_smt (mergeThenElseLhs c1 c2 t1 e1)))
      (__smtx_model_eval M (__eo_to_smt (mergeThenElseRhs c1 c2 t1 e1)))
    rw [eval_bvite_merge_then_else M hM c1 c2 t1 e1 hC1Trans hC2Trans
      hT1Trans hE1Trans hResultTy]
    exact RuleProofs.smt_value_rel_refl _

public theorem cmd_step_bv_ite_merge_then_else_properties
    (M : SmtModel) (hM : model_wf M)
    (s : CState) (args : CArgList) (premises : CIndexList) :
  cmdTranslationOk (CCmd.step CRule.bv_ite_merge_then_else args premises) ->
  AllHaveBoolType (premiseTermList s premises) ->
  __eo_typeof (__eo_cmd_step_proven s CRule.bv_ite_merge_then_else args premises) = Term.Bool ->
  StepRuleProperties M (premiseTermList s premises)
    (__eo_cmd_step_proven s CRule.bv_ite_merge_then_else args premises) :=
by
  intro hCmdTrans _hPremisesBool hResultTy
  have hProg : __eo_cmd_step_proven s CRule.bv_ite_merge_then_else args premises ≠
      Term.Stuck :=
                  ...
                  | nil =>
                      ...
                      | nil =>
                          ...
                          have hA4Trans : RuleProofs.eo_has_smt_translation a4 :=
                            hATransPair.2.2.2.1
                          change __eo_typeof
                              (__eo_prog_bv_ite_merge_then_else a1 a2 a3 a4) =
                            Term.Bool at hResultTy
                          refine ⟨?_, ?_⟩
                          · intro _hTrue
                            change eo_interprets M
                              (__eo_prog_bv_ite_merge_then_else a1 a2 a3 a4) true
                            exact facts___eo_prog_bv_ite_merge_then_else_impl M hM
                              a1 a2 a3 a4 hA1Trans hA2Trans hA3Trans
                              hA4Trans hResultTy
                          · exact RuleProofs.eo_has_smt_translation_of_has_bool_type _
                              (typed___eo_prog_bv_ite_merge_then_else_impl
                                a1 a2 a3 a4 hA1Trans hA2Trans hA3Trans
                                hA4Trans hResultTy)
