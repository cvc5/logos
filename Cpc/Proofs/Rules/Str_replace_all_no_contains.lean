...
private abbrev replaceAllNoContainsPremise (t s : Term) : Term :=
  Term.Apply
    (Term.Apply Term.eq (Term.Apply (Term.Apply Term.str_contains t) s))
    (Term.Boolean false)

private abbrev replaceAllNoContainsLhs (t s r : Term) : Term :=
  Term.Apply (Term.Apply (Term.Apply Term.str_replace_all t) s) r

private abbrev replaceAllNoContainsConclusion (t s r : Term) : Term :=
  Term.Apply (Term.Apply Term.eq (replaceAllNoContainsLhs t s r)) t
...
private theorem native_seq_replace_all_eq_self_of_indexof_neg
...
private theorem native_seq_replace_all_eq_self_of_contains_false
  ...
  exact native_seq_replace_all_eq_self_of_indexof_neg xs pat repl
    (Int.lt_of_not_ge hNot)

private theorem prog_str_replace_all_no_contains_info
    (t s r P : Term)
    (hProg : __eo_prog_str_replace_all_no_contains t s r (Proof.pf P) ≠
      Term.Stuck) :
    ∃ t0 s0,
      P = replaceAllNoContainsPremise t0 s0 ∧
      t0 = t ∧
      s0 = s ∧
      __eo_prog_str_replace_all_no_contains t s r (Proof.pf P) =
        replaceAllNoContainsConclusion t s r := by
  unfold __eo_prog_str_replace_all_no_contains at hProg
  ...
  next heq =>
    ...
    simp [__eo_prog_str_replace_all_no_contains, __eo_requires, __eo_eq,
      __eo_and, SmtEval.native_ite, native_teq, native_and,
      SmtEval.native_not, replaceAllNoContainsConclusion,
      replaceAllNoContainsLhs]

private theorem typed___eo_prog_str_replace_all_no_contains_impl
    (t s r P : Term)
    ...
    (hProgEq :
      __eo_prog_str_replace_all_no_contains t s r (Proof.pf P) =
        replaceAllNoContainsConclusion t s r) :
    RuleProofs.eo_has_bool_type
      (__eo_prog_str_replace_all_no_contains t s r (Proof.pf P)) := by
  rcases hTy with ⟨T, hTTy, hSTy, hRTy⟩
  let lhs := replaceAllNoContainsLhs t s r
  let rhs := t
  ...
  rw [hProgEq]
  simpa [replaceAllNoContainsConclusion, lhs, rhs] using hBoolEq

private theorem facts___eo_prog_str_replace_all_no_contains_impl
    ...
    (hTy : ∃ T, __eo_typeof t = Term.Apply Term.Seq T ∧
      __eo_typeof s = Term.Apply Term.Seq T ∧
      __eo_typeof r = Term.Apply Term.Seq T)
    (hPrem : eo_interprets M (replaceAllNoContainsPremise t s) true)
    (hProgEq :
      __eo_prog_str_replace_all_no_contains t s r (Proof.pf P) =
        replaceAllNoContainsConclusion t s r) :
    eo_interprets M
      (__eo_prog_str_replace_all_no_contains t s r (Proof.pf P)) true := by
  rcases hTy with ⟨T, hTTy, hSTy, hRTy⟩
  let lhs := replaceAllNoContainsLhs t s r
  let rhs := t
  have hBoolEq :
      RuleProofs.eo_has_bool_type (Term.Apply (Term.Apply Term.eq lhs) rhs) := by
    simpa [hProgEq, replaceAllNoContainsConclusion, lhs, rhs] using
      typed___eo_prog_str_replace_all_no_contains_impl t s r P
  ...
  have hReplaceSelf :
      native_seq_replace_all (native_unpack_seq ts) (native_unpack_seq ss)
          (native_unpack_seq rs) =
        native_unpack_seq ts :=
    native_seq_replace_all_eq_self_of_contains_false
...
public theorem cmd_step_str_replace_all_no_contains_properties
    (M : SmtModel) (hM : model_wf M)
    (s : CState) (args : CArgList) (premises : CIndexList) :
  cmdTranslationOk (CCmd.step CRule.str_replace_all_no_contains args premises) ->
  AllHaveBoolType (premiseTermList s premises) ->
  __eo_typeof (__eo_cmd_step_proven s CRule.str_replace_all_no_contains args premises) = Term.Bool ->
  StepRuleProperties M (premiseTermList s premises)
    (__eo_cmd_step_proven s CRule.str_replace_all_no_contains args premises) :=
by
  intro hCmdTrans _hPremisesBool hResultTy
  have hProg : __eo_cmd_step_proven s CRule.str_replace_all_no_contains args premises ≠
      Term.Stuck :=
                      ...
                      | nil =>
                          ...
                          have hA3Trans : RuleProofs.eo_has_smt_translation a3 := by
                            simpa [cmdTranslationOk, cArgListTranslationOk] using
                              hCmdTrans.2.2.1
                          change __eo_typeof
                              (__eo_prog_str_replace_all_no_contains a1 a2 a3 (Proof.pf P)) =
                            Term.Bool at hResultTy
                          have hProgRule :
                              __eo_prog_str_replace_all_no_contains a1 a2 a3 (Proof.pf P) ≠
                                Term.Stuck :=
                            term_ne_stuck_of_typeof_bool hResultTy
                          rcases prog_str_replace_all_no_contains_info
                              a1 a2 a3 P hProgRule with
                            ⟨t0, s0, hPremShape, ht0, hs0, hProgEq⟩
                          subst t0
                          subst s0
                          let lhs := replaceAllNoContainsLhs a1 a2 a3
                          let rhs := a1
                          ...
                          · intro hTrue
                            ...
                            have hPrem :
                                eo_interprets M
                                  (replaceAllNoContainsPremise a1 a2) true := by
                              simpa [hPremShape] using hPremRaw
                            exact facts___eo_prog_str_replace_all_no_contains_impl
                              M hM a1 a2 a3 P hA1Trans hA2Trans hA3Trans
                              hArgTypes hPrem hProgEq
                          · exact RuleProofs.eo_has_smt_translation_of_has_bool_type _
                              (typed___eo_prog_str_replace_all_no_contains_impl
