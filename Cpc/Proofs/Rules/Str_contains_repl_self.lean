...
private abbrev containsReplSelfRepl (x y : Term) : Term :=
  Term.Apply (Term.Apply (Term.Apply Term.str_replace x) y) x

private abbrev containsReplSelfLhs (x y : Term) : Term :=
  Term.Apply (Term.Apply Term.str_contains (containsReplSelfRepl x y)) y

private abbrev containsReplSelfRhs (x y : Term) : Term :=
  Term.Apply (Term.Apply Term.str_contains x) y

private abbrev containsReplSelfConclusion (x y : Term) : Term :=
  Term.Apply (Term.Apply Term.eq (containsReplSelfLhs x y))
    (containsReplSelfRhs x y)
...
private theorem native_seq_contains_append_right
  ...
  have : before ++ pat ++ after ++ rest =
      before ++ pat ++ (after ++ rest) := by
    simp [List.append_assoc]
  rw [this]
  exact native_seq_contains_of_decomp before pat (after ++ rest)

private theorem native_seq_contains_append_left
    (pre xs pat : List SmtValue)
    (h : native_seq_contains xs pat = true) :
    native_seq_contains (pre ++ xs) pat = true := by
  rcases native_seq_contains_decomp_exists xs pat h with ⟨before, after, rfl⟩
  have : pre ++ (before ++ pat ++ after) =
      (pre ++ before) ++ pat ++ after := by
    simp [List.append_assoc]
  rw [this]
...
private theorem native_seq_contains_replace_self
    (xs pat : List SmtValue) :
    native_seq_contains (native_seq_replace xs pat xs) pat =
      native_seq_contains xs pat := by
  by_cases hContains : native_seq_contains xs pat = true
  · rw [hContains]
    cases pat with
    | nil =>
        simp [native_seq_replace, native_seq_contains_nil]
    | cons p ps =>
        ...
        have hRight :
            native_seq_contains
              (xs ++ xs.drop (Int.toNat (native_seq_indexof xs (p :: ps) 0) +
                (p :: ps).length)) (p :: ps) = true :=
          native_seq_contains_append_right xs
            (xs.drop (Int.toNat (native_seq_indexof xs (p :: ps) 0) +
              (p :: ps).length)) (p :: ps) hContains
        have hLeft :
            native_seq_contains
              (xs.take (Int.toNat (native_seq_indexof xs (p :: ps) 0)) ++
                (xs ++ xs.drop (Int.toNat (native_seq_indexof xs (p :: ps) 0) +
                  (p :: ps).length))) (p :: ps) = true :=
          native_seq_contains_append_left
            (xs.take (Int.toNat (native_seq_indexof xs (p :: ps) 0)))
            (xs ++ xs.drop (Int.toNat (native_seq_indexof xs (p :: ps) 0) +
              (p :: ps).length)) (p :: ps) hRight
        simpa [List.append_assoc] using hLeft
  · have hFalse : native_seq_contains xs pat = false := by
      cases h : native_seq_contains xs pat
      · rfl
      · exact False.elim (hContains h)
    rw [native_seq_replace_eq_self_of_contains_false xs pat xs hFalse]

private theorem prog_str_contains_repl_self_eq
    (x y : Term)
    (hX : x ≠ Term.Stuck) (hY : y ≠ Term.Stuck) :
    __eo_prog_str_contains_repl_self x y =
      containsReplSelfConclusion x y := by
  have hRaw :
      __eo_prog_str_contains_repl_self x y =
        Term.Apply
          (Term.Apply Term.eq
            (Term.Apply
              (Term.Apply Term.str_contains
                (Term.Apply (Term.Apply (Term.Apply Term.str_replace x) y) x))
              y))
          (Term.Apply (Term.Apply Term.str_contains x) y) := by
    unfold __eo_prog_str_contains_repl_self
    split
    · exact False.elim (hX rfl)
    · exact False.elim (hY rfl)
    · rfl
  simpa [containsReplSelfConclusion, containsReplSelfLhs,
    containsReplSelfRhs, containsReplSelfRepl] using hRaw

private theorem typed___eo_prog_str_contains_repl_self_impl
    (x y : Term)
    (hXTrans : RuleProofs.eo_has_smt_translation x)
    (hYTrans : RuleProofs.eo_has_smt_translation y)
    (hXTy : __eo_typeof x = Term.Apply Term.Seq T)
    (hYTy : __eo_typeof y = Term.Apply Term.Seq T) :
    RuleProofs.eo_has_bool_type (__eo_prog_str_contains_repl_self x y) := by
  let repl := containsReplSelfRepl x y
  let lhs := containsReplSelfLhs x y
  let rhs := containsReplSelfRhs x y
  ...
  have hReplTy :
    ...
    simpa [repl, containsReplSelfRepl, __eo_to_smt, __eo_to_smt_type, __smtx_typeof] using hReplSmtTy
  have hLhsTy : __smtx_typeof (__eo_to_smt lhs) = SmtType.Bool := by
    ...
    simp [hReplSmtTy, hYSmtTy, __smtx_typeof_seq_op_2_ret, native_ite,
      native_Teq]
  have hRhsTy : __smtx_typeof (__eo_to_smt rhs) = SmtType.Bool := by
    ...
    simp [hXSmtTy, hYSmtTy, __smtx_typeof_seq_op_2_ret, native_ite,
      native_Teq]
  have hBoolEq :
      RuleProofs.eo_has_bool_type (containsReplSelfConclusion x y) :=
  ...
  rw [prog_str_contains_repl_self_eq x y hXNotStuck hYNotStuck]
  exact hBoolEq

private theorem facts___eo_prog_str_contains_repl_self_impl
    (M : SmtModel) (hM : model_wf M) (x y : Term)
    (hXTrans : RuleProofs.eo_has_smt_translation x)
    (hYTrans : RuleProofs.eo_has_smt_translation y)
    (hXTy : __eo_typeof x = Term.Apply Term.Seq T)
    (hYTy : __eo_typeof y = Term.Apply Term.Seq T) :
    eo_interprets M (__eo_prog_str_contains_repl_self x y) true := by
  let repl := containsReplSelfRepl x y
  let lhs := containsReplSelfLhs x y
  let rhs := containsReplSelfRhs x y
  have hXNotStuck : x ≠ Term.Stuck :=
  ...
  have hProg :
      __eo_prog_str_contains_repl_self x y =
        containsReplSelfConclusion x y :=
    prog_str_contains_repl_self_eq x y hXNotStuck hYNotStuck
  have hBoolEq :
      RuleProofs.eo_has_bool_type (containsReplSelfConclusion x y) := by
    simpa [hProg] using
      typed___eo_prog_str_contains_repl_self_impl x y hXTrans hYTrans hXTy hYTy
  have hXSmtTy := smtx_typeof_of_eo_seq x T hXTrans hXTy
  have hYSmtTy := smtx_typeof_of_eo_seq y T hYTrans hYTy
  have hXEvalTy :
  ...
  have hEvalEq :
    ...
    change __smtx_model_eval M
        (SmtTerm.str_contains
          (SmtTerm.str_replace (__eo_to_smt x) (__eo_to_smt y)
            (__eo_to_smt x))
          (__eo_to_smt y)) =
      __smtx_model_eval M (SmtTerm.str_contains (__eo_to_smt x) (__eo_to_smt y))
    ...
    simp [__smtx_model_eval_str_contains, __smtx_model_eval_str_replace,
      Smtm.native_unpack_pack_seq, native_seq_contains_replace_self]
  rw [hProg]
  exact RuleProofs.eo_interprets_eq_of_rel M lhs rhs hBoolEq <| by
    rw [hEvalEq]
    exact RuleProofs.smt_value_rel_refl (__smtx_model_eval M (__eo_to_smt rhs))

public theorem cmd_step_str_contains_repl_self_properties
    (M : SmtModel) (hM : model_wf M)
    (s : CState) (args : CArgList) (premises : CIndexList) :
  cmdTranslationOk (CCmd.step CRule.str_contains_repl_self args premises) ->
  AllHaveBoolType (premiseTermList s premises) ->
  __eo_typeof (__eo_cmd_step_proven s CRule.str_contains_repl_self args premises) = Term.Bool ->
  StepRuleProperties M (premiseTermList s premises)
    (__eo_cmd_step_proven s CRule.str_contains_repl_self args premises) :=
by
  intro hCmdTrans _hPremisesBool hResultTy
  have hProg : __eo_cmd_step_proven s CRule.str_contains_repl_self args premises ≠
      Term.Stuck :=
      ...
      | cons a2 args =>
          ...
          | nil =>
              ...
              | nil =>
                  ...
                  have hProgEq :
                      __eo_cmd_step_proven s CRule.str_contains_repl_self
                        (CArgList.cons a1 (CArgList.cons a2 CArgList.nil))
                        CIndexList.nil =
                        containsReplSelfConclusion a1 a2 := by
                    change __eo_prog_str_contains_repl_self a1 a2 =
                      containsReplSelfConclusion a1 a2
                    exact prog_str_contains_repl_self_eq a1 a2
                      hA1NotStuck hA2NotStuck
                  let lhs := containsReplSelfLhs a1 a2
                  rw [hProgEq] at hResultTy
                  change __eo_typeof_eq (__eo_typeof lhs)
                      (__eo_typeof (containsReplSelfRhs a1 a2)) =
                    Term.Bool at hResultTy
                  have hLhsNotStuck : __eo_typeof lhs ≠ Term.Stuck :=
                    (RuleProofs.eo_typeof_eq_bool_operands_not_stuck
                      (__eo_typeof lhs) (__eo_typeof (containsReplSelfRhs a1 a2))
                      hResultTy).1
                  have hArgTypes :
                      ∃ T, __eo_typeof a1 = Term.Apply Term.Seq T ∧
                        __eo_typeof a2 = Term.Apply Term.Seq T := by
                    change __eo_typeof_str_contains
                        (__eo_typeof (containsReplSelfRepl a1 a2))
                    ...
                    rcases eo_typeof_str_replace_args_of_ne_stuck
                        ...
                        (__eo_typeof a1) hReplNotStuck with
                  ...
                  have hProps :
                      StepRuleProperties M (premiseTermList s CIndexList.nil)
                        (__eo_prog_str_contains_repl_self a1 a2) := by
                    refine ⟨?_, RuleProofs.eo_has_smt_translation_of_has_bool_type _
                      (typed___eo_prog_str_contains_repl_self_impl a1 a2
                        hA1Trans hA2Trans hA1Ty hA2Ty)⟩
                    intro _hTrue
                    exact facts___eo_prog_str_contains_repl_self_impl M hM
                      a1 a2 hA1Trans hA2Trans hA1Ty hA2Ty
                  change StepRuleProperties M []
                    (__eo_prog_str_contains_repl_self a1 a2)
