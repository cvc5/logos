...
private abbrev substrRangePremise (m : Term) : Term :=
  Term.Apply
    (Term.Apply Term.eq
      (Term.Apply (Term.Apply Term.geq (Term.Numeral 0)) m))
    (Term.Boolean true)

private abbrev substrRangeLhs (x n m : Term) : Term :=
  Term.Apply (Term.Apply (Term.Apply Term.str_substr x) n) m

private abbrev substrRangeConclusion (x n m A : Term) : Term :=
  Term.Apply (Term.Apply Term.eq (substrRangeLhs x n m)) (__seq_empty A)
...
private theorem prog_str_substr_empty_range_info
    (x n m A P : Term)
    (hProg : __eo_prog_str_substr_empty_range x n m A (Proof.pf P) ≠
      Term.Stuck) :
    ∃ T m0,
      A = Term.Apply Term.Seq T ∧
      P = substrRangePremise m0 ∧
      m0 = m ∧
      __eo_prog_str_substr_empty_range x n m A (Proof.pf P) =
        substrRangeConclusion x n m (Term.Apply Term.Seq T) := by
  unfold __eo_prog_str_substr_empty_range at hProg
  ...
  next heq =>
    ...
    simp [__eo_prog_str_substr_empty_range, __eo_requires, __eo_eq,
      SmtEval.native_ite, native_teq, SmtEval.native_not,
      eo_mk_apply_eq_seq_empty, substrRangeConclusion, substrRangeLhs]
...
private theorem facts___eo_prog_str_substr_empty_range_impl
    (M : SmtModel) (hM : model_wf M) (x n m P T : Term)
    (hBoolEq :
      RuleProofs.eo_has_bool_type
        (substrRangeConclusion x n m (Term.Apply Term.Seq T)))
    ...
    (hPrem : eo_interprets M (substrRangePremise m) true)
    (hProgEq :
      __eo_prog_str_substr_empty_range x n m (Term.Apply Term.Seq T)
          (Proof.pf P) =
        substrRangeConclusion x n m (Term.Apply Term.Seq T)) :
    eo_interprets M
      (__eo_prog_str_substr_empty_range x n m (Term.Apply Term.Seq T)
        (Proof.pf P)) true := by
  let lhs := substrRangeLhs x n m
  ...
  have hMLenNonpos : mi ≤ 0 := by
    rw [RuleProofs.eo_interprets_iff_smt_interprets] at hPrem
    cases hPrem with
    | intro_true _ hEval =>
        change __smtx_model_eval M
            (SmtTerm.eq
              (SmtTerm.geq (SmtTerm.Numeral 0) (__eo_to_smt m))
              (SmtTerm.Boolean true)) =
          SmtValue.Boolean true at hEval
        rw [smtx_eval_eq_term_eq, smtx_eval_geq_term_eq,
          smtx_eval_numeral_term_eq, hMEval,
          smtx_eval_boolean_term_eq] at hEval
        have hLeBool : native_zleq mi 0 = true := by
  ...
  have hEvalEq :
    ...
    simp [__smtx_model_eval_str_substr, hXElem,
      native_seq_extract_empty_of_len_nonpos _ _ _ hMLenNonpos,
      native_pack_seq]
  rw [hProgEq]
  exact RuleProofs.eo_interprets_eq_of_rel M lhs rhs
    (by simpa [substrRangeConclusion, lhs, rhs] using hBoolEq) <| by
    rw [hEvalEq]
    exact RuleProofs.smt_value_rel_refl (__smtx_model_eval M (__eo_to_smt rhs))

public theorem cmd_step_str_substr_empty_range_properties
    (M : SmtModel) (hM : model_wf M)
    (s : CState) (args : CArgList) (premises : CIndexList) :
  cmdTranslationOk (CCmd.step CRule.str_substr_empty_range args premises) ->
  AllHaveBoolType (premiseTermList s premises) ->
  __eo_typeof (__eo_cmd_step_proven s CRule.str_substr_empty_range args premises) = Term.Bool ->
  StepRuleProperties M (premiseTermList s premises)
    (__eo_cmd_step_proven s CRule.str_substr_empty_range args premises) :=
by
  intro hCmdTrans _hPremisesBool hResultTy
  have hProg : __eo_cmd_step_proven s CRule.str_substr_empty_range args premises ≠
      Term.Stuck :=
                  ...
                  | nil =>
                      ...
                      | cons p premises =>
                          ...
                          | nil =>
                              ...
                              change __eo_typeof
                                  (__eo_prog_str_substr_empty_range a1 a2 a3 A
                                    (Proof.pf P)) =
                              ...
                              have hProgRule :
                                  __eo_prog_str_substr_empty_range a1 a2 a3 A
                                    ...
                                    Term.Stuck :=
                                term_ne_stuck_of_typeof_bool hResultTy
                              rcases prog_str_substr_empty_range_info a1 a2 a3 A P
                                  hProgRule with
                                ⟨T, m0, hATyArg, hPremShape, hm0, hProgEq⟩
                              subst A
                              subst m0
                              ...
                              have hBoolEq :
                                  RuleProofs.eo_has_bool_type
                                    (substrRangeConclusion a1 a2 a3
                                      (Term.Apply Term.Seq T)) := by
                                have hEqBool :
                                    RuleProofs.eo_has_bool_type
                                ...
                                simpa [substrRangeConclusion, lhs, rhs] using
                                  hEqBool
                              refine ⟨?_, ?_⟩
                              · intro hTrue
                                ...
                                have hPrem :
                                    eo_interprets M (substrRangePremise a3) true := by
                                  simpa [hPremShape] using hPremRaw
                                change eo_interprets M
                                  (__eo_prog_str_substr_empty_range a1 a2 a3
                                    (Term.Apply Term.Seq T) (Proof.pf P)) true
                                exact facts___eo_prog_str_substr_empty_range_impl
