...
namespace ReRangeNonSingleton2Proof
...
private theorem prog_form (s t P : Term)
    (hNe : __eo_prog_re_range_non_singleton_2 s t (Proof.pf P) ≠ Term.Stuck) :
    P = mkNotLenOne t ∧
      __eo_prog_re_range_non_singleton_2 s t (Proof.pf P) = concl s t := by
  unfold __eo_prog_re_range_non_singleton_2 at hNe ⊢
  split at hNe
...
private theorem native_re_range_empty_of_second_len_ne_one
    (ss tt : SmtSeq)
    (hLenNe : native_seq_len (native_unpack_seq tt) ≠ 1) :
    native_re_range (native_unpack_seq ss) (native_unpack_seq tt) =
      native_re_none := by
  cases hUnpack : native_unpack_seq tt with
  | nil =>
      simp [hUnpack, native_re_range, native_re_none]
  | cons v vs =>
      cases vs with
      | nil =>
          have hLen : native_seq_len (native_unpack_seq tt) = 1 := by
...
private theorem facts
  ...
  have hPremRel :=
    RuleProofs.eo_interprets_eq_rel M (innerLenOne t) (Term.Boolean false) hPrem
  have hLenEval :
      __smtx_model_eval M (SmtTerm.str_len (__eo_to_smt t)) =
        SmtValue.Numeral (native_seq_len (native_unpack_seq tt)) := by
    simp [__smtx_model_eval, __smtx_model_eval_str_len, hTEval]
  have hOneEval :
      __smtx_model_eval M (SmtTerm.Numeral 1) =
        SmtValue.Numeral 1 := by
    rw [__smtx_model_eval.eq_def] <;> simp only
  have hInnerEval :
      __smtx_model_eval M (__eo_to_smt (innerLenOne t)) =
        SmtValue.Boolean
          (native_veq
            (SmtValue.Numeral (native_seq_len (native_unpack_seq tt)))
            (SmtValue.Numeral 1)) := by
  ...
  have hLenNe : native_seq_len (native_unpack_seq tt) ≠ 1 := by
    intro hLen
  ...
  have hRangeNone :
      native_re_range (native_unpack_seq ss) (native_unpack_seq tt) =
        native_re_none :=
    native_re_range_empty_of_second_len_ne_one ss tt hLenNe
  have hLhsEval :
...
by
  ...
  have hProg : __eo_cmd_step_proven s CRule.re_range_non_singleton_2 args premises ≠
      Term.Stuck :=
              ...
              | cons n1 premises =>
                  ...
                  | nil =>
                      ...
                      show StepRuleProperties M [__eo_state_proven_nth s n1]
                        (__eo_prog_re_range_non_singleton_2 a1 a2
                          (Proof.pf (__eo_state_proven_nth s n1)))
                      generalize hP : __eo_state_proven_nth s n1 = P
                      have hProgNe :
                          __eo_prog_re_range_non_singleton_2 a1 a2 (Proof.pf P) ≠
                            Term.Stuck := by
                        rw [← hP]
                        change __eo_cmd_step_proven s CRule.re_range_non_singleton_2
                            (CArgList.cons a1 (CArgList.cons a2 CArgList.nil))
                            (CIndexList.cons n1 CIndexList.nil) ≠ Term.Stuck
                        exact hProg
                      obtain ⟨hPremShape, hProgEq⟩ :=
                        ReRangeNonSingleton2Proof.prog_form a1 a2 P hProgNe
                      have hResultTyProg :
                          __eo_typeof
                              (__eo_prog_re_range_non_singleton_2 a1 a2 (Proof.pf P)) =
                            Term.Bool := by
                        rw [← hP]
                        change __eo_typeof
                            (__eo_cmd_step_proven s CRule.re_range_non_singleton_2
                              (CArgList.cons a1 (CArgList.cons a2 CArgList.nil))
                              (CIndexList.cons n1 CIndexList.nil)) = Term.Bool
                        exact hResultTy
                      have hConclTy :
                          __eo_typeof (ReRangeNonSingleton2Proof.concl a1 a2) =
                            Term.Bool := by
                        rw [hProgEq] at hResultTyProg
                        exact hResultTyProg
                      have hLhsTy :
                          __eo_typeof (ReRangeNonSingleton2Proof.lhs a1 a2) =
                            Term.UOp UserOp.RegLan := by
                        change __eo_typeof_eq
                            (__eo_typeof (ReRangeNonSingleton2Proof.lhs a1 a2))
                            (__eo_typeof ReRangeNonSingleton2Proof.rhs) =
                          Term.Bool at hConclTy
                        have hTyEq :=
                          support_eo_typeof_eq_bool_operands_eq
                            (__eo_typeof (ReRangeNonSingleton2Proof.lhs a1 a2))
                            (__eo_typeof ReRangeNonSingleton2Proof.rhs) hConclTy
                        simpa [ReRangeNonSingleton2Proof.rhs] using hTyEq
                      have hArgTypes :
                        ...
                        exact
                          ReRangeNonSingleton2Proof.eo_typeof_re_range_eq_reglan_args_seq_char
                      ...
                      have hA2SmtTy :
                        ...
                        exact hTyRaw
                      have hBool :=
                        ReRangeNonSingleton2Proof.typed_concl a1 a2 hA1SmtTy hA2SmtTy
                      rw [hProgEq]
                      ...
                      have hPrem :
                          eo_interprets M
                            (ReRangeNonSingleton2Proof.mkNotLenOne a2) true := by
                        have h := hEv.true_here P (by simp)
                        rwa [hPremShape] at h
                      exact ReRangeNonSingleton2Proof.facts M hM a1 a2
