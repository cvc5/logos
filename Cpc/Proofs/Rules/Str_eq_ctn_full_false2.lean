...
private theorem prog_str_eq_ctn_full_false2_info
    (x y P : Term)
    (hProg : __eo_prog_str_eq_ctn_full_false2 x y (Proof.pf P) ≠ Term.Stuck) :
    ∃ x0 y0,
      P = ctnFullFalsePremise x0 y0 ∧
      x0 = x ∧
      y0 = y ∧
      __eo_prog_str_eq_ctn_full_false2 x y (Proof.pf P) =
        ctnFullFalseConclusion x y := by
  unfold __eo_prog_str_eq_ctn_full_false2 at hProg
  ...
  next heq =>
    ...
    simp [__eo_prog_str_eq_ctn_full_false2, __eo_requires, __eo_eq, __eo_and,
      SmtEval.native_ite, native_teq, native_and, SmtEval.native_not,
      ctnFullFalseConclusion, ctnFullFalseLhs]

private theorem typed___eo_prog_str_eq_ctn_full_false2_impl
    (x y P : Term)
    (hSameTy :
      __smtx_typeof (__eo_to_smt x) = __smtx_typeof (__eo_to_smt y))
    ...
    RuleProofs.eo_has_bool_type
      (__eo_prog_str_eq_ctn_full_false2 x y (Proof.pf P)) := by
  let lhs := ctnFullFalseLhs x y
...
private theorem facts___eo_prog_str_eq_ctn_full_false2_impl
    ...
    (hProgEq :
      __eo_prog_str_eq_ctn_full_false2 x y (Proof.pf P) =
        ctnFullFalseConclusion x y) :
    eo_interprets M (__eo_prog_str_eq_ctn_full_false2 x y (Proof.pf P)) true := by
  let lhs := ctnFullFalseLhs x y
  let rhs := Term.Boolean false
  have hBoolEq :
      RuleProofs.eo_has_bool_type (Term.Apply (Term.Apply Term.eq lhs) rhs) := by
    simpa [hProgEq, ctnFullFalseConclusion, lhs, rhs] using
      typed___eo_prog_str_eq_ctn_full_false2_impl x y P
...
public theorem cmd_step_str_eq_ctn_full_false2_properties
  ...
  __eo_typeof (__eo_cmd_step_proven s CRule.str_eq_ctn_full_false2 args premises) = Term.Bool ->
  StepRuleProperties M (premiseTermList s premises)
    (__eo_cmd_step_proven s CRule.str_eq_ctn_full_false2 args premises) :=
by
  intro hCmdTrans _hPremisesBool hResultTy
  have hProg : __eo_cmd_step_proven s CRule.str_eq_ctn_full_false2 args premises ≠
      Term.Stuck :=
                  ...
                  | nil =>
                      ...
                      have hA2Trans : RuleProofs.eo_has_smt_translation a2 := by
                        simpa [cmdTranslationOk, cArgListTranslationOk] using
                          hCmdTrans.2.1
                      change __eo_typeof
                          (__eo_prog_str_eq_ctn_full_false2 a1 a2 (Proof.pf P)) =
                        Term.Bool at hResultTy
                      have hProgRule :
                          __eo_prog_str_eq_ctn_full_false2 a1 a2 (Proof.pf P) ≠
                            Term.Stuck :=
                        term_ne_stuck_of_typeof_bool hResultTy
                      rcases prog_str_eq_ctn_full_false2_info a1 a2 P hProgRule with
                      ...
                      · intro hTrue
                        ...
                        exact facts___eo_prog_str_eq_ctn_full_false2_impl
                          M hM a1 a2 P hSameSmt.1 hSameSmt.2 hPrem hProgEq
                      · exact RuleProofs.eo_has_smt_translation_of_has_bool_type _
                          (typed___eo_prog_str_eq_ctn_full_false2_impl a1 a2 P
