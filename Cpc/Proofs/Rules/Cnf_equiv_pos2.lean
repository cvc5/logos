...
public theorem cmd_step_cnf_equiv_pos2_properties
    (M : SmtModel) (hM : model_wf M)
    (s : CState) (args : CArgList) (premises : CIndexList) :
  cmdTranslationOk (CCmd.step CRule.cnf_equiv_pos2 args premises) ->
  AllHaveBoolType (premiseTermList s premises) ->
  __eo_typeof (__eo_cmd_step_proven s CRule.cnf_equiv_pos2 args premises) = Term.Bool ->
  StepRuleProperties M (premiseTermList s premises)
    (__eo_cmd_step_proven s CRule.cnf_equiv_pos2 args premises) :=
by
  intro hCmdTrans _hPremisesBool hResultTy
  have hProg : __eo_cmd_step_proven s CRule.cnf_equiv_pos2 args premises ≠ Term.Stuck :=
  ...
  | cons F args =>
      ...
      | nil =>
          cases premises with
          | nil =>
              change __eo_prog_cnf_equiv_pos2 F ≠ Term.Stuck at hProg
              change __eo_typeof (__eo_prog_cnf_equiv_pos2 F) = Term.Bool at hResultTy
              cases F with
              | Apply f F2 =>
                  cases f with
                  | Apply op F1 =>
                      ...
                      | UOp op =>
                          ...
                          | eq =>
                              ...
                              simp [__eo_prog_cnf_equiv_pos2] at hTyData
                                ...
                                · rcases CnfSupport.eo_interprets_bool_cases M hM F2 hF2Bool with
                                  ...
                                  · have hEqFalse :
                                        eo_interprets M
                                          (Term.Apply (Term.Apply (Term.UOp UserOp.eq) F1) F2)
                                          false :=
                                      CnfSupport.eo_interprets_eq_false_of_false_true M F1 F2
                                        hF1False hF2True
                                    simpa [__eo_prog_cnf_equiv_pos2] using
