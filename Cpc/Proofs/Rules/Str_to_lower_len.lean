...
private theorem eo_typeof_str_len_str_to_lower_arg_seq_char_of_ne_stuck
    (T : Term)
...
private theorem typed___eo_prog_str_to_lower_len_impl
    (x : Term)
    (hXTrans : RuleProofs.eo_has_smt_translation x)
    (hXTy : __eo_typeof x = Term.Apply Term.Seq (Term.UOp UserOp.Char)) :
    RuleProofs.eo_has_bool_type (__eo_prog_str_to_lower_len x) := by
  let lhs := Term.Apply Term.str_len (Term.Apply Term.str_to_lower x)
  ...
  have hLhsTy : __smtx_typeof (__eo_to_smt lhs) = SmtType.Int := by
    change __smtx_typeof
        (SmtTerm.str_len (SmtTerm.str_to_lower (__eo_to_smt x))) =
      SmtType.Int
    rw [typeof_str_len_eq, hLowerTy]
  ...
  have hProg :
      __eo_prog_str_to_lower_len x = Term.Apply (Term.Apply Term.eq lhs) rhs := by
    have hProgRaw :
        __eo_prog_str_to_lower_len x =
          Term.Apply
            (Term.Apply Term.eq
              (Term.Apply Term.str_len (Term.Apply Term.str_to_lower x)))
            (Term.Apply Term.str_len x) := by
      cases x <;> simp [__eo_prog_str_to_lower_len] at hXNotStuck ⊢
...
private theorem facts___eo_prog_str_to_lower_len_impl
  ...
  have hBoolEq :
    ...
    simpa [hProg] using
      typed___eo_prog_str_to_lower_len_impl x hXTrans hXTy
  have hXSmtTy : __smtx_typeof (__eo_to_smt x) = SmtType.Seq SmtType.Char := by
    have := smtx_typeof_of_eo_seq x (Term.UOp UserOp.Char) hXTrans hXTy
    simpa using this
  have hXEvalTy :
  ...
  have hEvalEq :
    ...
    rw [smtx_eval_str_len_term_eq, smtx_eval_str_to_lower_term_eq,
      smtx_eval_str_len_term_eq]
    rw [hXEval]
    simp [__smtx_model_eval_str_to_lower, __smtx_model_eval_str_len,
      native_str_to_lower, native_seq_len, native_pack_string,
...
public theorem cmd_step_str_to_lower_len_properties
  ...
  __eo_typeof (__eo_cmd_step_proven s CRule.str_to_lower_len args premises) = Term.Bool ->
  StepRuleProperties M (premiseTermList s premises)
    (__eo_cmd_step_proven s CRule.str_to_lower_len args premises) :=
by
  intro hCmdTrans _hPremisesBool hResultTy
  have hProg : __eo_cmd_step_proven s CRule.str_to_lower_len args premises ≠
      Term.Stuck :=
      ...
      | nil =>
          ...
          | nil =>
              ...
              have hA1NotStuck : a1 ≠ Term.Stuck := by
                ...
                apply hProg
                rfl
              let lhs := Term.Apply Term.str_len (Term.Apply Term.str_to_lower a1)
              let rhs := Term.Apply Term.str_len a1
              have hProgEq :
                  __eo_cmd_step_proven s CRule.str_to_lower_len
                ...
                have hProgEqRaw :
                    __eo_cmd_step_proven s CRule.str_to_lower_len
                      ...
                      Term.Apply
                        (Term.Apply Term.eq
                          (Term.Apply Term.str_len (Term.Apply Term.str_to_lower a1)))
              ...
              have hA1Ty : __eo_typeof a1 = Term.Apply Term.Seq (Term.UOp UserOp.Char) := by
                change __eo_typeof_str_len
                  ...
                  at hLhsNotStuck
                exact eo_typeof_str_len_str_to_lower_arg_seq_char_of_ne_stuck
                  (__eo_typeof a1) hLhsNotStuck
              have hProps :
                  StepRuleProperties M (premiseTermList s CIndexList.nil)
                    (__eo_prog_str_to_lower_len a1) := by
                refine ⟨?_, RuleProofs.eo_has_smt_translation_of_has_bool_type _
                  (typed___eo_prog_str_to_lower_len_impl a1 hA1Trans hA1Ty)⟩
                intro _hTrue
                exact facts___eo_prog_str_to_lower_len_impl M hM a1 hA1Trans hA1Ty
              change StepRuleProperties M [] (__eo_prog_str_to_lower_len a1)
