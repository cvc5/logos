...
private theorem prog_ite_then_lookahead_eq_of_ne_stuck (c1 x1 y1 z1 : Term) :
    ...
    y1 ≠ Term.Stuck ->
    z1 ≠ Term.Stuck ->
    __eo_prog_ite_then_lookahead c1 x1 y1 z1 =
      Term.Apply (Term.Apply Term.eq
        (Term.Apply (Term.Apply (Term.Apply Term.ite c1)
          (Term.Apply (Term.Apply (Term.Apply Term.ite c1) x1) y1)) z1))
        (Term.Apply (Term.Apply (Term.Apply Term.ite c1) x1) z1) := by
  intro hC1 hX1 hY1 hZ1
  unfold __eo_prog_ite_then_lookahead
  split <;> simp_all

private theorem args_of_prog_ite_then_lookahead_bool (c1 x1 y1 z1 : Term) :
    c1 ≠ Term.Stuck ->
    x1 ≠ Term.Stuck ->
    y1 ≠ Term.Stuck ->
    z1 ≠ Term.Stuck ->
    __eo_typeof (__eo_prog_ite_then_lookahead c1 x1 y1 z1) = Term.Bool ->
  ...
  rw [prog_ite_then_lookahead_eq_of_ne_stuck c1 x1 y1 z1 hC1 hX1 hY1 hZ1] at hTy
  ...
  rcases eo_typeof_ite_args_of_ne_stuck (__eo_typeof c1) (__eo_typeof x1)
      (__eo_typeof y1) hInnerNonStuck with
    ⟨_hC1Type2, hXY, hXNonStuck⟩
  have hInnerEqX :
      __eo_typeof_ite (__eo_typeof c1) (__eo_typeof x1) (__eo_typeof y1) =
        __eo_typeof x1 := by
    rw [hC1Type, ← hXY]
...
private theorem typed___eo_prog_ite_then_lookahead_impl (c1 x1 y1 z1 : Term) :
  RuleProofs.eo_has_smt_translation c1 ->
  RuleProofs.eo_has_smt_translation x1 ->
  RuleProofs.eo_has_smt_translation y1 ->
  RuleProofs.eo_has_smt_translation z1 ->
  __eo_typeof (__eo_prog_ite_then_lookahead c1 x1 y1 z1) = Term.Bool ->
  RuleProofs.eo_has_bool_type (__eo_prog_ite_then_lookahead c1 x1 y1 z1) := by
  ...
  rcases args_of_prog_ite_then_lookahead_bool c1 x1 y1 z1 hC1NotStuck hX1NotStuck
  ...
  let inner := Term.Apply (Term.Apply (Term.Apply Term.ite c1) x1) y1
  have hInnerSmt :
  ...
  have hInnerZType : __eo_typeof inner = __eo_typeof z1 := by
    show __eo_typeof_ite (__eo_typeof c1) (__eo_typeof x1) (__eo_typeof y1) =
      __eo_typeof z1
    rw [hC1Type, ← hXY, eo_typeof_ite_bool_self (__eo_typeof x1) hXNonStuck]
    exact hXZ
  have hLeftSmt :
      __smtx_typeof
  ...
  have hRightSmt :
    ...
    smt_type_ite_same_as_then c1 x1 z1 hC1Bool hX1Trans hZ1Trans hXZ
  have hSame :
      __smtx_typeof
        (__eo_to_smt (Term.Apply (Term.Apply (Term.Apply Term.ite c1) inner) z1)) =
      __smtx_typeof
        (__eo_to_smt (Term.Apply (Term.Apply (Term.Apply Term.ite c1) x1) z1)) := by
    rw [hLeftSmt, hInnerSmt, hRightSmt]
  have hLeftNonNone :
      __smtx_typeof
        (__eo_to_smt (Term.Apply (Term.Apply (Term.Apply Term.ite c1) inner) z1)) ≠
        SmtType.None := by
    ...
    exact hX1Trans
  rw [prog_ite_then_lookahead_eq_of_ne_stuck c1 x1 y1 z1 hC1NotStuck hX1NotStuck
    hY1NotStuck hZ1NotStuck]
  exact RuleProofs.eo_has_bool_type_eq_of_same_smt_type _ _ hSame hLeftNonNone

private theorem facts___eo_prog_ite_then_lookahead_impl
  ...
  RuleProofs.eo_has_smt_translation z1 ->
  __eo_typeof (__eo_prog_ite_then_lookahead c1 x1 y1 z1) = Term.Bool ->
  eo_interprets M (__eo_prog_ite_then_lookahead c1 x1 y1 z1) true := by
  intro hC1Trans hX1Trans hY1Trans hZ1Trans hResultTy
  have hC1NotStuck : c1 ≠ Term.Stuck :=
    RuleProofs.term_ne_stuck_of_has_smt_translation c1 hC1Trans
  have hX1NotStuck : x1 ≠ Term.Stuck :=
  ...
  have hZ1NotStuck : z1 ≠ Term.Stuck :=
    RuleProofs.term_ne_stuck_of_has_smt_translation z1 hZ1Trans
  have hProgBool :
      RuleProofs.eo_has_bool_type (__eo_prog_ite_then_lookahead c1 x1 y1 z1) :=
    typed___eo_prog_ite_then_lookahead_impl c1 x1 y1 z1 hC1Trans hX1Trans hY1Trans
      hZ1Trans hResultTy
  rcases args_of_prog_ite_then_lookahead_bool c1 x1 y1 z1 hC1NotStuck hX1NotStuck
      hY1NotStuck hZ1NotStuck hResultTy with
  ...
  · simpa [prog_ite_then_lookahead_eq_of_ne_stuck c1 x1 y1 z1 hC1NotStuck
  ...
  · rcases RuleProofs.eo_eval_is_boolean_of_has_bool_type M hM c1 hC1Bool with
    ...
    rw [show __eo_to_smt
        ...
        SmtTerm.ite (__eo_to_smt c1)
          (SmtTerm.ite (__eo_to_smt c1) (__eo_to_smt x1) (__eo_to_smt y1))
          (__eo_to_smt z1) by
        rfl]
    rw [show __eo_to_smt
...
public theorem cmd_step_ite_then_lookahead_properties
  ...
  __eo_typeof (__eo_cmd_step_proven s CRule.ite_then_lookahead args premises) = Term.Bool ->
  StepRuleProperties M (premiseTermList s premises)
    (__eo_cmd_step_proven s CRule.ite_then_lookahead args premises) :=
by
                  ...
                  | nil =>
                      ...
                      | nil =>
                          ...
                          have hA4Trans : RuleProofs.eo_has_smt_translation a4 :=
                            hATransPair.2.2.2.1
                          change __eo_typeof (__eo_prog_ite_then_lookahead a1 a2 a3 a4) =
                            Term.Bool at hResultTy
                          refine ⟨?_, ?_⟩
                          · intro _hTrue
                            change eo_interprets M
                              (__eo_prog_ite_then_lookahead a1 a2 a3 a4) true
                            exact facts___eo_prog_ite_then_lookahead_impl M hM a1 a2 a3 a4
                              hA1Trans hA2Trans hA3Trans hA4Trans hResultTy
                          · exact RuleProofs.eo_has_smt_translation_of_has_bool_type _
                              (typed___eo_prog_ite_then_lookahead_impl a1 a2 a3 a4
                                hA1Trans hA2Trans hA3Trans hA4Trans hResultTy)
                      | cons _ _ =>
                          change Term.Stuck ≠ Term.Stuck at hProg
                          exact False.elim (hProg rfl)
