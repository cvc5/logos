...
public theorem cmd_step_cnf_equiv_neg1_properties
    (M : SmtModel) (hM : model_wf M)
    (s : CState) (args : CArgList) (premises : CIndexList) :
  cmdTranslationOk (CCmd.step CRule.cnf_equiv_neg1 args premises) ->
  AllHaveBoolType (premiseTermList s premises) ->
  __eo_typeof (__eo_cmd_step_proven s CRule.cnf_equiv_neg1 args premises) = Term.Bool ->
  StepRuleProperties M (premiseTermList s premises)
    (__eo_cmd_step_proven s CRule.cnf_equiv_neg1 args premises) :=
by
  intro hCmdTrans _hPremisesBool hResultTy
  have hProg : __eo_cmd_step_proven s CRule.cnf_equiv_neg1 args premises ≠ Term.Stuck :=
  ...
  | cons F args =>
      ...
      | nil =>
          cases premises with
          | nil =>
              change __eo_prog_cnf_equiv_neg1 F ≠ Term.Stuck at hProg
              change __eo_typeof (__eo_prog_cnf_equiv_neg1 F) = Term.Bool at hResultTy
              cases F with
              | Apply f F2 =>
                  cases f with
                  | Apply op F1 =>
                      ...
                      | UOp op =>
                          ...
                          | eq =>
                              ...
                              simp [__eo_prog_cnf_equiv_neg1] at hTyData
                              change __eo_typeof_or
                                  (__eo_typeof_eq (__eo_typeof F1) (__eo_typeof F2))
                                  (__eo_typeof_or (__eo_typeof F1)
