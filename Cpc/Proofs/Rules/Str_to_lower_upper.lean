...
private theorem eo_typeof_str_to_lower_str_to_upper_arg_seq_char_of_ne_stuck
    (T : Term)
...
private theorem native_char_to_lower_upper (c : native_Char) :
  ...
  · by_cases hUpperHi : c <= 122
    · have hUpperCond : (97 <= c && c <= 122) = true := by
        simp [hUpperLo, hUpperHi]
      have hUpperLoNat : (97 : Nat) <= c := by
        exact hUpperLo
      have hUpperHiNat : c <= (122 : Nat) := by
        exact hUpperHi
      have hLowerLo : 65 <= c - 32 := by
        change (65 : Nat) <= c - 32
        omega
      have hLowerHi : c - 32 <= 90 := by
        simpa using Nat.sub_le_sub_right hUpperHiNat 32
      have hOrigCondFalse : (65 <= c && c <= 90) = false := by
        have hNotHi : ¬ c <= 90 := by
          intro hLe
          have hLt : (90 : Nat) < c :=
            Nat.lt_of_lt_of_le (by decide : (90 : Nat) < 97) hUpperLoNat
          exact (Nat.not_lt_of_ge hLe) hLt
        simp [hNotHi]
      have hSub : c - 32 + 32 = c := by
        exact Nat.sub_add_cancel
          (Nat.le_trans (by decide : (32 : Nat) <= 97) hUpperLoNat)
      simp [hUpperCond, hLowerLo, hLowerHi, hOrigCondFalse, hSub]
    · have hUpperCond : (97 <= c && c <= 122) = false := by
        simp [hUpperLo, hUpperHi]
      simp [hUpperCond]
  · have hUpperCond : (97 <= c && c <= 122) = false := by
      simp [hUpperLo]
...
private theorem native_str_to_lower_upper (s : native_String) :
    native_str_to_lower (native_str_to_upper s) = native_str_to_lower s := by
  unfold native_str_to_lower native_str_to_upper
  induction s with
  | nil => rfl
  | cons c cs ih =>
      simp [List.map, native_char_to_lower_upper c, ih]
...
private theorem typed___eo_prog_str_to_lower_upper_impl
    (x : Term)
    (hXTrans : RuleProofs.eo_has_smt_translation x)
    (hXTy : __eo_typeof x = Term.Apply Term.Seq (Term.UOp UserOp.Char)) :
    RuleProofs.eo_has_bool_type (__eo_prog_str_to_lower_upper x) := by
  let lhs := Term.Apply Term.str_to_lower (Term.Apply Term.str_to_upper x)
  let rhs := Term.Apply Term.str_to_lower x
  ...
  have hLhsTy : __smtx_typeof (__eo_to_smt lhs) = SmtType.Seq SmtType.Char := by
    change __smtx_typeof
        (SmtTerm.str_to_lower (SmtTerm.str_to_upper (__eo_to_smt x))) =
      SmtType.Seq SmtType.Char
    rw [typeof_str_to_lower_eq, hUpperTy]
    simp [native_ite, native_Teq]
  ...
  have hProg :
      __eo_prog_str_to_lower_upper x = Term.Apply (Term.Apply Term.eq lhs) rhs := by
    have hProgRaw :
        __eo_prog_str_to_lower_upper x =
          Term.Apply
            (Term.Apply Term.eq
              (Term.Apply Term.str_to_lower
                (Term.Apply Term.str_to_upper x)))
            (Term.Apply Term.str_to_lower x) := by
      cases x <;> simp [__eo_prog_str_to_lower_upper] at hXNotStuck ⊢
...
private theorem facts___eo_prog_str_to_lower_upper_impl
  ...
  have hBoolEq :
    ...
    simpa [hProg] using
      typed___eo_prog_str_to_lower_upper_impl x hXTrans hXTy
  have hXSmtTy : __smtx_typeof (__eo_to_smt x) = SmtType.Seq SmtType.Char := by
    have := smtx_typeof_of_eo_seq x (Term.UOp UserOp.Char) hXTrans hXTy
    simpa using this
  have hXEvalTy :
  ...
  have hEvalEq :
    ...
    change __smtx_model_eval M
        (SmtTerm.str_to_lower (SmtTerm.str_to_upper (__eo_to_smt x))) =
      __smtx_model_eval M (SmtTerm.str_to_lower (__eo_to_smt x))
    rw [smtx_eval_str_to_lower_term_eq, smtx_eval_str_to_upper_term_eq,
      smtx_eval_str_to_lower_term_eq]
    rw [hXEval]
    simp [__smtx_model_eval_str_to_lower, __smtx_model_eval_str_to_upper,
      native_unpack_string_pack_string, native_str_to_lower_upper]
  rw [hProg]
...
public theorem cmd_step_str_to_lower_upper_properties
  ...
  __eo_typeof (__eo_cmd_step_proven s CRule.str_to_lower_upper args premises) = Term.Bool ->
  StepRuleProperties M (premiseTermList s premises)
    (__eo_cmd_step_proven s CRule.str_to_lower_upper args premises) :=
by
  intro hCmdTrans _hPremisesBool hResultTy
  have hProg : __eo_cmd_step_proven s CRule.str_to_lower_upper args premises ≠
      Term.Stuck :=
  ...
  | cons a1 args =>
      ...
      | nil =>
          ...
          | nil =>
              ...
              let rhs := Term.Apply Term.str_to_lower a1
              have hProgEq :
                  __eo_cmd_step_proven s CRule.str_to_lower_upper
                    ...
                    __eo_cmd_step_proven s CRule.str_to_lower_upper
                      ...
                      Term.Apply
                        (Term.Apply Term.eq
                          (Term.Apply Term.str_to_lower
                            (Term.Apply Term.str_to_upper a1)))
                        (Term.Apply Term.str_to_lower a1) := by
                  cases hA1 : a1 <;> first
              ...
              have hA1Ty : __eo_typeof a1 =
                ...
                change __eo_typeof_str_to_lower
                  ...
                  at hLhsNotStuck
                exact eo_typeof_str_to_lower_str_to_upper_arg_seq_char_of_ne_stuck
                  (__eo_typeof a1) hLhsNotStuck
              have hProps :
                  StepRuleProperties M (premiseTermList s CIndexList.nil)
                    (__eo_prog_str_to_lower_upper a1) := by
                refine ⟨?_, RuleProofs.eo_has_smt_translation_of_has_bool_type _
                  (typed___eo_prog_str_to_lower_upper_impl a1 hA1Trans hA1Ty)⟩
                intro _hTrue
                exact facts___eo_prog_str_to_lower_upper_impl M hM a1 hA1Trans hA1Ty
              change StepRuleProperties M [] (__eo_prog_str_to_lower_upper a1)
