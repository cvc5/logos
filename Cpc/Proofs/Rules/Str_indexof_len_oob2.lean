...
private abbrev needle (s w ss : Term) := mkConcat w (mkConcat s ss)
...
private theorem program_info (t s w ss n P Q : Term)
    (h : __eo_prog_str_indexof_len_oob2 t s w ss n (Proof.pf P) (Proof.pf Q) ≠ Term.Stuck) :
    P = lengthPremise t s ∧ Q = positivePremise w ∧
    __eo_prog_str_indexof_len_oob2 t s w ss n (Proof.pf P) (Proof.pf Q) = conclusion t s w ss n := by
  unfold __eo_prog_str_indexof_len_oob2 at h
  ...
  next hp hq =>
    ...
    subst_vars
    refine ⟨rfl, rfl, ?_⟩
    simp [__eo_prog_str_indexof_len_oob2, __eo_requires, __eo_eq, __eo_and,
...
private theorem needle_type (s w ss : Term) (T : SmtType)
  ...
  change __smtx_typeof (SmtTerm.str_concat (__eo_to_smt w)
    (SmtTerm.str_concat (__eo_to_smt s) (__eo_to_smt ss))) = _
...
private theorem conclusion_facts (M : SmtModel) (hM : model_wf M)
  ...
  have hLong : (native_unpack_seq ts).length <
      (native_unpack_seq wv ++ (native_unpack_seq sv ++ native_unpack_seq ssv)).length := by
  ...
  have hEval : __smtx_model_eval M (__eo_to_smt (lhs t s w ss n)) =
    ...
    change __smtx_model_eval M (SmtTerm.str_indexof (__eo_to_smt t)
      (SmtTerm.str_concat (__eo_to_smt w) (SmtTerm.str_concat (__eo_to_smt s) (__eo_to_smt ss)))
...
public theorem cmd_step_str_indexof_len_oob2_properties
    (M : SmtModel) (hM : model_wf M)
    (state : CState) (args : CArgList) (premises : CIndexList) :
  cmdTranslationOk (CCmd.step CRule.str_indexof_len_oob2 args premises) ->
  AllHaveBoolType (premiseTermList state premises) ->
  __eo_typeof (__eo_cmd_step_proven state CRule.str_indexof_len_oob2 args premises) = Term.Bool ->
  StepRuleProperties M (premiseTermList state premises)
    (__eo_cmd_step_proven state CRule.str_indexof_len_oob2 args premises) := by
  intro hCmd _ hTy
              ...
              | cons i premises =>
                ...
                | cons j premises =>
                  ...
                  | nil =>
                    ...
                    change __eo_typeof (__eo_prog_str_indexof_len_oob2 t s w ss n (Proof.pf P) (Proof.pf Q)) = Term.Bool at hTy
                    rcases program_info t s w ss n P Q (term_ne_stuck_of_typeof_bool hTy) with ⟨hP, hQ, hp⟩
                    change StepRuleProperties M _ (__eo_prog_str_indexof_len_oob2 t s w ss n (Proof.pf P) (Proof.pf Q))
                    rw [hp] at hTy ⊢
                    ...
                    rcases StrConcatUnifySupport.eo_typeof_str_concat_args_of_result_seq w (mkConcat s ss) T hneedle with ⟨hw, hinner⟩
                    rcases StrConcatUnifySupport.eo_typeof_str_concat_args_of_result_seq s ss T hinner with ⟨hs, hss⟩
