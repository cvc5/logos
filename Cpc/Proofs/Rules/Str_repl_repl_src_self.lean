...
private abbrev replSrcSelfInner (x y : Term) : Term :=
  Term.Apply (Term.Apply (Term.Apply Term.str_replace y) x) y

private abbrev replSrcSelfLhs (x y z : Term) : Term :=
  Term.Apply
    (Term.Apply (Term.Apply Term.str_replace x) (replSrcSelfInner x y)) z

private abbrev replSrcSelfRhs (x y z : Term) : Term :=
  Term.Apply (Term.Apply (Term.Apply Term.str_replace x) y) z

private abbrev replSrcSelfConclusion (x y z : Term) : Term :=
  Term.Apply (Term.Apply Term.eq (replSrcSelfLhs x y z))
    (replSrcSelfRhs x y z)

private theorem prog_str_repl_repl_src_self_eq
    (x y z : Term)
    (hX : x ≠ Term.Stuck) (hY : y ≠ Term.Stuck)
    (hZ : z ≠ Term.Stuck) :
    __eo_prog_str_repl_repl_src_self x y z =
      replSrcSelfConclusion x y z := by
  unfold __eo_prog_str_repl_repl_src_self
  split
  · contradiction
  · contradiction
  · contradiction
...
private theorem typed___eo_prog_str_repl_repl_src_self_impl
    (x y z : Term)
    ...
    RuleProofs.eo_has_bool_type
      (__eo_prog_str_repl_repl_src_self x y z) := by
  let lhs := replSrcSelfLhs x y z
  let rhs := replSrcSelfRhs x y z
  ...
  have hZNN := RuleProofs.term_ne_stuck_of_has_smt_translation z hZTrans
  rw [prog_str_repl_repl_src_self_eq x y z hXNN hYNN hZNN]
  exact hBool

private theorem facts___eo_prog_str_repl_repl_src_self_impl
    ...
    (hYTy : __eo_typeof y = Term.Apply Term.Seq T)
    (hZTy : __eo_typeof z = Term.Apply Term.Seq T) :
    eo_interprets M (__eo_prog_str_repl_repl_src_self x y z) true := by
  let lhs := replSrcSelfLhs x y z
  let rhs := replSrcSelfRhs x y z
  have hXNN := RuleProofs.term_ne_stuck_of_has_smt_translation x hXTrans
  have hYNN := RuleProofs.term_ne_stuck_of_has_smt_translation y hYTrans
  have hZNN := RuleProofs.term_ne_stuck_of_has_smt_translation z hZTrans
  have hProgEq := prog_str_repl_repl_src_self_eq x y z hXNN hYNN hZNN
  have hBool : RuleProofs.eo_has_bool_type
      (replSrcSelfConclusion x y z) := by
    simpa [hProgEq] using
      typed___eo_prog_str_repl_repl_src_self_impl
        x y z hXTrans hYTrans hZTrans hXTy hYTy hZTy
  have hXSmtTy := smtx_typeof_of_eo_seq x T hXTrans hXTy
  have hYSmtTy := smtx_typeof_of_eo_seq y T hYTrans hYTy
  have hZSmtTy := smtx_typeof_of_eo_seq z T hZTrans hZTy
  have hXEvalTy :
...
public theorem cmd_step_str_repl_repl_src_self_properties
    (M : SmtModel) (hM : model_wf M)
    (s : CState) (args : CArgList) (premises : CIndexList) :
  cmdTranslationOk (CCmd.step CRule.str_repl_repl_src_self args premises) ->
  AllHaveBoolType (premiseTermList s premises) ->
  __eo_typeof (__eo_cmd_step_proven s CRule.str_repl_repl_src_self args premises) = Term.Bool ->
  StepRuleProperties M (premiseTermList s premises)
    (__eo_cmd_step_proven s CRule.str_repl_repl_src_self args premises) :=
by
                  ...
                  | nil =>
                      ...
                      have hA3NN :=
                        RuleProofs.term_ne_stuck_of_has_smt_translation
                          a3 hA3Trans
                      change __eo_typeof
                          (__eo_prog_str_repl_repl_src_self a1 a2 a3) =
                        Term.Bool at hResultTy
                      have hProgEq :=
                        prog_str_repl_repl_src_self_eq
                          a1 a2 a3 hA1NN hA2NN hA3NN
                      rw [hProgEq] at hResultTy
                      have hLhsNN :
                          __eo_typeof (replSrcSelfLhs a1 a2 a3) ≠
                            Term.Stuck := by
                        change __eo_typeof_eq
                            (__eo_typeof (replSrcSelfLhs a1 a2 a3))
                            (__eo_typeof (replSrcSelfRhs a1 a2 a3)) =
                          Term.Bool at hResultTy
                        exact
                      ...
                      change __eo_typeof_str_replace
                          (__eo_typeof a1)
                          (__eo_typeof (replSrcSelfInner a1 a2))
                          (__eo_typeof a3) ≠
                      ...
                      refine ⟨?_, ?_⟩
                      · intro _hTrue
                        exact facts___eo_prog_str_repl_repl_src_self_impl
