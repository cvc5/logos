...
private abbrev replTgtSelfInner (x y : Term) : Term :=
  Term.Apply (Term.Apply (Term.Apply Term.str_replace y) x) y

private abbrev replTgtSelfLhs (x y : Term) : Term :=
  Term.Apply
    (Term.Apply (Term.Apply Term.str_replace x) y) (replTgtSelfInner x y)

private abbrev replTgtSelfConclusion (x y : Term) : Term :=
  Term.Apply (Term.Apply Term.eq (replTgtSelfLhs x y)) x

private theorem prog_str_repl_repl_tgt_self_eq
    (x y : Term) (hX : x ≠ Term.Stuck) (hY : y ≠ Term.Stuck) :
    __eo_prog_str_repl_repl_tgt_self x y = replTgtSelfConclusion x y := by
  unfold __eo_prog_str_repl_repl_tgt_self
  split
  · contradiction
  · contradiction
  · rfl

private theorem typed___eo_prog_str_repl_repl_tgt_self_impl
    (x y : Term)
    (hXTrans : RuleProofs.eo_has_smt_translation x)
    (hYTrans : RuleProofs.eo_has_smt_translation y)
    (hXTy : __eo_typeof x = Term.Apply Term.Seq T)
    (hYTy : __eo_typeof y = Term.Apply Term.Seq T) :
    RuleProofs.eo_has_bool_type (__eo_prog_str_repl_repl_tgt_self x y) := by
  let inner := replTgtSelfInner x y
  let lhs := replTgtSelfLhs x y
  ...
  have hLhsTy :
    ...
    simp [hXSmtTy, hYSmtTy, __smtx_typeof_seq_op_3, native_ite,
      native_Teq]
  have hBool : RuleProofs.eo_has_bool_type (replTgtSelfConclusion x y) :=
  ...
  rw [prog_str_repl_repl_tgt_self_eq x y hXNN hYNN]
  exact hBool

private theorem facts___eo_prog_str_repl_repl_tgt_self_impl
    ...
    (hXTy : __eo_typeof x = Term.Apply Term.Seq T)
    (hYTy : __eo_typeof y = Term.Apply Term.Seq T) :
    eo_interprets M (__eo_prog_str_repl_repl_tgt_self x y) true := by
  let lhs := replTgtSelfLhs x y
  have hXNN := RuleProofs.term_ne_stuck_of_has_smt_translation x hXTrans
  have hYNN := RuleProofs.term_ne_stuck_of_has_smt_translation y hYTrans
  have hProgEq := prog_str_repl_repl_tgt_self_eq x y hXNN hYNN
  have hBool : RuleProofs.eo_has_bool_type (replTgtSelfConclusion x y) := by
    simpa [hProgEq] using
      typed___eo_prog_str_repl_repl_tgt_self_impl
  ...
  have hEvalEq :
    ...
    simp [__smtx_model_eval_str_replace, Smtm.native_unpack_pack_seq,
      native_seq_replace_tgt_self, native_pack_unpack_seq]
  rw [hProgEq]
...
public theorem cmd_step_str_repl_repl_tgt_self_properties
    (M : SmtModel) (hM : model_wf M)
    (s : CState) (args : CArgList) (premises : CIndexList) :
  cmdTranslationOk (CCmd.step CRule.str_repl_repl_tgt_self args premises) ->
  AllHaveBoolType (premiseTermList s premises) ->
  __eo_typeof (__eo_cmd_step_proven s CRule.str_repl_repl_tgt_self args premises) = Term.Bool ->
  StepRuleProperties M (premiseTermList s premises)
    (__eo_cmd_step_proven s CRule.str_repl_repl_tgt_self args premises) :=
by
      ...
      | cons a2 args =>
          ...
          | nil =>
              ...
              | nil =>
                  ...
                  change __eo_typeof
                      (__eo_prog_str_repl_repl_tgt_self a1 a2) =
                    Term.Bool at hResultTy
                  have hProgEq :=
                    prog_str_repl_repl_tgt_self_eq a1 a2 hA1NN hA2NN
                  rw [hProgEq] at hResultTy
                  have hLhsNN :
                      __eo_typeof (replTgtSelfLhs a1 a2) ≠
                        Term.Stuck := by
                  ...
                  change __eo_typeof_str_replace
                      ...
                      (__eo_typeof (replTgtSelfInner a1 a2)) ≠
                    Term.Stuck at hLhsNN
                  rcases eo_typeof_str_replace_args_of_ne_stuck
                      (__eo_typeof a1) (__eo_typeof a2)
                      (__eo_typeof (replTgtSelfInner a1 a2)) hLhsNN with
                    ⟨T, hA1Ty, hA2Ty, _hInnerTy⟩
                  refine ⟨?_, ?_⟩
                  · intro _hTrue
                    exact facts___eo_prog_str_repl_repl_tgt_self_impl
