...
private theorem take_drop_nested_of_actual_cover
    (xs : List SmtValue) (i m j n : Nat)
    (hCover : ((xs.drop i).take m).length ≤ j + n) :
    (((xs.drop i).take m).drop j).take n =
      (xs.drop (i + j)).take (m - j) := by
  have hTailLen : (((xs.drop i).take m).drop j).length ≤ n := by
    rw [List.length_drop]
    omega
  rw [List.drop_take, List.drop_drop] at hTailLen ⊢
  exact List.take_of_length_le hTailLen

private theorem native_seq_extract_nested_of_actual_cover
    (xs : List SmtValue) (i m j n : native_Int)
    (hi : 0 ≤ i) (hj : 0 ≤ j)
    (hCover :
      Int.ofNat (native_seq_extract xs i m).length ≤ j + n) :
  ...
  · have hk : 0 < m - j := Int.lt_of_not_ge hkLe
    ...
    have hInnerEq := native_seq_extract_eq_drop_take xs i m hi hm
    by_cases hnLe : n ≤ 0
    · rw [native_seq_extract_empty_of_len_nonpos_local
          (native_seq_extract xs i m) j n hnLe]
      ...
      rw [hSubNat]
      rw [hInnerEq] at hCover
      have hCoverNat :
          ((xs.drop (Int.toNat i)).take (Int.toNat m)).length ≤
            Int.toNat j := by
        apply Int.ofNat_le.mp
        rw [Int.toNat_of_nonneg hj]
        have hJnLe : j + n ≤ j := by
          simpa using Int.add_le_add_left hnLe j
        exact Int.le_trans hCover hJnLe
      have hList := take_drop_nested_of_actual_cover xs
        (Int.toNat i) (Int.toNat m) (Int.toNat j) 0 (by omega)
      simpa using hList
    · have hn : 0 < n := Int.lt_of_not_ge hnLe
      ...
      have hSubNat : Int.toNat (m - j) = Int.toNat m - Int.toNat j := by
        ...
        exact Int.toNat_sub (Int.toNat m) (Int.toNat j)
      rw [hSubNat]
      apply take_drop_nested_of_actual_cover
      apply Int.ofNat_le.mp
...
private abbrev combine4NonnegPremise (n : Term) : Term :=
  Term.Apply
    (Term.Apply Term.eq
      (Term.Apply (Term.Apply Term.geq n) (Term.Numeral 0)))
    (Term.Boolean true)

private abbrev combine4Inner (s n m : Term) : Term :=
  Term.Apply (Term.Apply (Term.Apply Term.str_substr s) n) m

private abbrev combine4BoundPremise
  ...
  Term.Apply
    (Term.Apply Term.eq
      (Term.Apply
        ...
        (Term.Apply Term.str_len (combine4Inner s n1 m1))))
    (Term.Boolean true)

private abbrev combine4Lhs
    (s n1 m1 n2 m2 : Term) : Term :=
  combine4Inner (combine4Inner s n1 m1) n2 m2

private abbrev combine4Start (n1 n2 : Term) : Term :=
  Term.Apply (Term.Apply Term.plus n1)
    (Term.Apply (Term.Apply Term.plus n2) (Term.Numeral 0))

private abbrev combine4Rhs
    (s n1 m1 n2 : Term) : Term :=
  combine4Inner s (combine4Start n1 n2)
    (Term.Apply (Term.Apply Term.neg m1) n2)

private abbrev combine4Conclusion
    (s n1 m1 n2 m2 : Term) : Term :=
  Term.Apply (Term.Apply Term.eq (combine4Lhs s n1 m1 n2 m2))
    (combine4Rhs s n1 m1 n2)

private theorem prog_str_substr_combine4_info
    (s n1 m1 n2 m2 P1 P2 P3 : Term)
    (hProg :
      __eo_prog_str_substr_combine4 s n1 m1 n2 m2
          (Proof.pf P1) (Proof.pf P2) (Proof.pf P3) ≠ Term.Stuck) :
    P1 = combine4NonnegPremise n1 ∧
      P2 = combine4NonnegPremise n2 ∧
      P3 = combine4BoundPremise s n1 m1 n2 m2 ∧
      __eo_prog_str_substr_combine4 s n1 m1 n2 m2
          (Proof.pf P1) (Proof.pf P2) (Proof.pf P3) =
        combine4Conclusion s n1 m1 n2 m2 := by
  unfold __eo_prog_str_substr_combine4 at hProg
  ...
  next heq1 heq2 heq3 =>
    ...
    simp [__eo_prog_str_substr_combine4, __eo_requires, __eo_eq,
      __eo_and, SmtEval.native_ite, native_teq, native_and,
      SmtEval.native_not, combine4Conclusion, combine4Lhs,
      combine4Rhs, combine4Inner, combine4Start]

private theorem typed___eo_prog_str_substr_combine4_impl
    (s n1 m1 n2 m2 P1 P2 P3 : Term)
    ...
    (hProgEq :
      __eo_prog_str_substr_combine4 s n1 m1 n2 m2
          (Proof.pf P1) (Proof.pf P2) (Proof.pf P3) =
        combine4Conclusion s n1 m1 n2 m2) :
    RuleProofs.eo_has_bool_type
      (__eo_prog_str_substr_combine4 s n1 m1 n2 m2
        (Proof.pf P1) (Proof.pf P2) (Proof.pf P3)) := by
  let inner := combine4Inner s n1 m1
  let lhs := combine4Lhs s n1 m1 n2 m2
  let rhs := combine4Rhs s n1 m1 n2
  ...
  have hStartTy : __smtx_typeof (__eo_to_smt (combine4Start n1 n2)) =
      SmtType.Int := by
    change __smtx_typeof
  ...
  have hRhsTy : __smtx_typeof (__eo_to_smt rhs) =
    ...
    change __smtx_typeof
        (SmtTerm.str_substr (__eo_to_smt s)
          (__eo_to_smt (combine4Start n1 n2))
...
private theorem facts___eo_prog_str_substr_combine4_impl
    ...
    (hN2Ty : __eo_typeof n2 = Term.Int)
    (hM2Ty : __eo_typeof m2 = Term.Int)
    (hPrem1 : eo_interprets M (combine4NonnegPremise n1) true)
    (hPrem2 : eo_interprets M (combine4NonnegPremise n2) true)
    (hPrem3 : eo_interprets M
      (combine4BoundPremise s n1 m1 n2 m2) true)
    (hProgEq :
      __eo_prog_str_substr_combine4 s n1 m1 n2 m2
          (Proof.pf P1) (Proof.pf P2) (Proof.pf P3) =
        combine4Conclusion s n1 m1 n2 m2) :
  ...
  have hBool : RuleProofs.eo_has_bool_type
      (combine4Conclusion s n1 m1 n2 m2) := by
    simpa [hProgEq] using typed___eo_prog_str_substr_combine4_impl
  ...
  have hNonnegOfPrem (t : Term) (ti : native_Int)
      ...
      (hPrem : eo_interprets M (combine4NonnegPremise t) true) :
      0 ≤ ti := by
  ...
  have hBound :
      Int.ofNat (native_seq_extract
        (native_unpack_seq ss) n1i m1i).length ≤ n2i + m2i := by
    ...
    | intro_true _ hEval =>
        ...
        rw [smtx_eval_eq_term_eq, smtx_eval_geq_term_eq,
          smtx_eval_plus_term_eq, hN2Eval, smtx_eval_plus_term_eq,
          hM2Eval, smtx_eval_numeral_term_eq, smtx_eval_str_len_term_eq,
          smtx_eval_str_substr_term_eq, hSEval, hN1Eval, hM1Eval,
          smtx_eval_boolean_term_eq] at hEval
        have hLeBool : native_zleq
            (Int.ofNat
              (native_seq_extract (native_unpack_seq ss) n1i m1i).length)
            (native_zplus n2i (native_zplus m2i 0)) = true := by
  ...
  have hEvalEq : __smtx_model_eval M (__eo_to_smt lhs) =
    ...
    simp [__smtx_model_eval_str_substr, __smtx_model_eval_plus,
      ...
      native_seq_extract_nested_of_actual_cover _ _ _ _ _ hN1Nonneg
        hN2Nonneg hBound]
  rw [hProgEq]
...
public theorem cmd_step_str_substr_combine4_properties
    (M : SmtModel) (hM : model_wf M)
    (s : CState) (args : CArgList) (premises : CIndexList) :
  cmdTranslationOk (CCmd.step CRule.str_substr_combine4 args premises) ->
  AllHaveBoolType (premiseTermList s premises) ->
  __eo_typeof (__eo_cmd_step_proven s CRule.str_substr_combine4 args premises) = Term.Bool ->
  StepRuleProperties M (premiseTermList s premises)
    (__eo_cmd_step_proven s CRule.str_substr_combine4 args premises) :=
by
                                      ...
                                      | nil =>
                                          ...
                                          have hRuleProg :
                                              __eo_prog_str_substr_combine4
                                                ...
                                                Term.Stuck :=
                                            term_ne_stuck_of_typeof_bool hResultTy
                                          rcases prog_str_substr_combine4_info
                                          ...
                                          · exact
                                              RuleProofs.eo_has_smt_translation_of_has_bool_type
                                                _
                                                (typed___eo_prog_str_substr_combine4_impl
                                                  a1 a2 a3 a4 a5 P1 P2 P3
