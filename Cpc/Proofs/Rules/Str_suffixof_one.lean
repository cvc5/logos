...
private abbrev suffixOnePremise (t : Term) : Term :=
  Term.Apply
    (Term.Apply Term.eq (Term.Apply Term.str_len t))
    (Term.Numeral 1)

private abbrev suffixOneLhs (s t : Term) : Term :=
  Term.Apply (Term.Apply Term.str_suffixof s) t

private abbrev suffixOneRhs (s t : Term) : Term :=
  Term.Apply (Term.Apply Term.str_contains t) s

private abbrev suffixOneConclusion (s t : Term) : Term :=
  Term.Apply (Term.Apply Term.eq (suffixOneLhs s t)) (suffixOneRhs s t)
...
private theorem eval_str_suffixof_one_eq_contains
          ...
          | cons x xsTail =>
              ...
              | cons z zs =>
                  ...
                  · have hMin :
                    ...
                    have hStartNeg :
                        (1 : Int) + -(↑zs.length + 1 + 1) < 0 := by
                      omega
                    simp [hIf, hMin, hStartNeg, native_pack_seq]
      | cons z zs =>
          simp [hys] at hLen

private theorem prog_str_suffixof_one_info
    (s t P : Term)
    (hProg : __eo_prog_str_suffixof_one s t (Proof.pf P) ≠ Term.Stuck) :
    ∃ t0,
      P = suffixOnePremise t0 ∧
      t0 = t ∧
      __eo_prog_str_suffixof_one s t (Proof.pf P) =
        suffixOneConclusion s t := by
  unfold __eo_prog_str_suffixof_one at hProg
  ...
  next heq =>
    ...
    subst_vars
    refine ⟨_, rfl, rfl, ?_⟩
    simp [__eo_prog_str_suffixof_one, __eo_requires, __eo_eq,
      SmtEval.native_ite, native_teq, SmtEval.native_not,
      suffixOnePremise, suffixOneConclusion, suffixOneLhs, suffixOneRhs]

private theorem typed___eo_prog_str_suffixof_one_impl
    (s t P : Term)
    ...
    (hProgEq :
      __eo_prog_str_suffixof_one s t (Proof.pf P) =
        suffixOneConclusion s t) :
    RuleProofs.eo_has_bool_type
      (__eo_prog_str_suffixof_one s t (Proof.pf P)) := by
  rcases hTy with ⟨T, hSTy, hTTy⟩
  let lhs := suffixOneLhs s t
  let rhs := suffixOneRhs s t
  ...
  rw [hProgEq]
  simpa [suffixOneConclusion, lhs, rhs] using hBoolEq

private theorem facts___eo_prog_str_suffixof_one_impl
    (M : SmtModel) (hM : model_wf M) (s t P : Term)
    (hSTrans : RuleProofs.eo_has_smt_translation s)
    (hTTrans : RuleProofs.eo_has_smt_translation t)
    (hTy : ∃ T, __eo_typeof s = Term.Apply Term.Seq T ∧
      __eo_typeof t = Term.Apply Term.Seq T)
    (hPrem : eo_interprets M (suffixOnePremise t) true)
    (hProgEq :
      __eo_prog_str_suffixof_one s t (Proof.pf P) =
        suffixOneConclusion s t) :
    eo_interprets M (__eo_prog_str_suffixof_one s t (Proof.pf P)) true := by
  rcases hTy with ⟨T, hSTy, hTTy⟩
  let lhs := suffixOneLhs s t
  let rhs := suffixOneRhs s t
  have hBoolEq :
      RuleProofs.eo_has_bool_type (Term.Apply (Term.Apply Term.eq lhs) rhs) := by
    simpa [hProgEq, suffixOneConclusion, lhs, rhs] using
      typed___eo_prog_str_suffixof_one_impl s t P hSTrans hTTrans
        ⟨T, hSTy, hTTy⟩ hProgEq
  have hSSmtTy := smtx_typeof_of_eo_seq s T hSTrans hSTy
  have hTSmtTy := smtx_typeof_of_eo_seq t T hTTrans hTTy
  have hSEvalTy :
  ...
  have hEvalEq :
    ...
    change __smtx_model_eval M
      ...
      __smtx_model_eval M
        (SmtTerm.str_contains (__eo_to_smt t) (__eo_to_smt s))
    rw [smtx_eval_str_suffixof_term_eq, smtx_eval_str_contains_term_eq,
      hSEval, hTEval]
    exact eval_str_suffixof_one_eq_contains sx sy (__eo_to_smt_type T)
...
public theorem cmd_step_str_suffixof_one_properties
    (M : SmtModel) (hM : model_wf M)
    (s : CState) (args : CArgList) (premises : CIndexList) :
  cmdTranslationOk (CCmd.step CRule.str_suffixof_one args premises) ->
  AllHaveBoolType (premiseTermList s premises) ->
  __eo_typeof (__eo_cmd_step_proven s CRule.str_suffixof_one args premises) = Term.Bool ->
  StepRuleProperties M (premiseTermList s premises)
    (__eo_cmd_step_proven s CRule.str_suffixof_one args premises) :=
by
  intro hCmdTrans _hPremisesBool hResultTy
  have hProg : __eo_cmd_step_proven s CRule.str_suffixof_one args premises ≠
      Term.Stuck :=
                  ...
                  | nil =>
                      ...
                      have hA2Trans : RuleProofs.eo_has_smt_translation a2 := by
                        simpa [cmdTranslationOk, cArgListTranslationOk] using
                          hCmdTrans.2.1
                      change __eo_typeof
                          (__eo_prog_str_suffixof_one a1 a2 (Proof.pf P)) =
                        Term.Bool at hResultTy
                      have hProgRule :
                          __eo_prog_str_suffixof_one a1 a2 (Proof.pf P) ≠
                            Term.Stuck :=
                        term_ne_stuck_of_typeof_bool hResultTy
                      rcases prog_str_suffixof_one_info a1 a2 P hProgRule with
                        ⟨t0, hPremShape, ht0, hProgEq⟩
                      subst t0
                      let lhs := suffixOneLhs a1 a2
                      let rhs := suffixOneRhs a1 a2
                      ...
                      · intro hTrue
                        ...
                        have hPrem :
                            eo_interprets M (suffixOnePremise a2) true := by
                          simpa [hPremShape] using hPremRaw
                        exact facts___eo_prog_str_suffixof_one_impl M hM a1 a2 P
                          hA1Trans hA2Trans hArgTypes hPrem hProgEq
                      · exact RuleProofs.eo_has_smt_translation_of_has_bool_type _
                          (typed___eo_prog_str_suffixof_one_impl a1 a2 P
