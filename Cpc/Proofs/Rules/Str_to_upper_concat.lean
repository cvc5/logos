...
private theorem prog_str_to_upper_concat_raw
    (s1 s2 s3 : Term)
    (hS1 : s1 ≠ Term.Stuck) (hS2 : s2 ≠ Term.Stuck) (hS3 : s3 ≠ Term.Stuck) :
    __eo_prog_str_to_upper_concat s1 s2 s3 =
      __eo_mk_apply
        (Term.Apply Term.eq
            (Term.Apply Term.str_to_upper (mkConcat s1 (mkConcat s2 s3))))
        (__eo_mk_apply
          (Term.Apply Term.str_concat (Term.Apply Term.str_to_upper s1))
          (__eo_mk_apply
            (__eo_mk_apply Term.str_concat
              (__eo_mk_apply Term.str_to_upper
...
private theorem prog_str_to_upper_concat_eq_of_ne_stuck
    (s1 s2 s3 : Term)
    (hS1 : s1 ≠ Term.Stuck) (hS2 : s2 ≠ Term.Stuck) (hS3 : s3 ≠ Term.Stuck)
    ...
    __eo_prog_str_to_upper_concat s1 s2 s3 =
      mkEq
        (Term.Apply Term.str_to_upper (mkConcat s1 (mkConcat s2 s3)))
        (mkConcat (Term.Apply Term.str_to_upper s1)
          (mkConcat (Term.Apply Term.str_to_upper (__str_nary_elim (mkConcat s2 s3)))
            (Term.String []))) := by
  rw [prog_str_to_upper_concat_raw s1 s2 s3 hS1 hS2 hS3]
  -- innermost: __eo_mk_apply str_to_upper elim = Apply str_to_upper elim
  have hStuElim :
      __eo_mk_apply Term.str_to_upper (__str_nary_elim (mkConcat s2 s3)) =
        Term.Apply Term.str_to_upper (__str_nary_elim (mkConcat s2 s3)) :=
    eo_mk_apply_eq_apply_of_ne_stuck Term.str_to_upper
      (__str_nary_elim (mkConcat s2 s3)) (by
        cases hcase : __str_nary_elim (mkConcat s2 s3) with
        | Stuck => exact absurd hcase hElim
        | _ => simp [__eo_mk_apply, Term.str_to_upper])
  rw [hStuElim]
  -- now every remaining __eo_mk_apply has structurally non-stuck arguments
  simp only [__eo_mk_apply, mkEq, mkConcat, Term.str_to_upper, Term.str_concat,
    Term.eq]

/-- If the nary-elim is stuck, the whole program is stuck. -/
private theorem prog_str_to_upper_concat_stuck_of_elim_stuck
    (s1 s2 s3 : Term)
    (hS1 : s1 ≠ Term.Stuck) (hS2 : s2 ≠ Term.Stuck) (hS3 : s3 ≠ Term.Stuck)
    (hElim : __str_nary_elim (mkConcat s2 s3) = Term.Stuck) :
    __eo_prog_str_to_upper_concat s1 s2 s3 = Term.Stuck := by
  rw [prog_str_to_upper_concat_raw s1 s2 s3 hS1 hS2 hS3]
  rw [hElim]
  simp [__eo_mk_apply]
...
private theorem typeof_args_of_prog_bool
    (s1 s2 s3 : Term)
    (hS1 : s1 ≠ Term.Stuck) (hS2 : s2 ≠ Term.Stuck) (hS3 : s3 ≠ Term.Stuck)
    (hTy : __eo_typeof (__eo_prog_str_to_upper_concat s1 s2 s3) = Term.Bool) :
  ...
  have hElimNe : __str_nary_elim (mkConcat s2 s3) ≠ Term.Stuck := by
    intro hElim
    rw [prog_str_to_upper_concat_stuck_of_elim_stuck s1 s2 s3 hS1 hS2 hS3 hElim]
      at hTy
    have : __eo_typeof Term.Stuck ≠ Term.Bool := by native_decide
    exact this hTy
  rw [prog_str_to_upper_concat_eq_of_ne_stuck s1 s2 s3 hS1 hS2 hS3 hElimNe] at hTy
  -- Decompose the eq typeof.
  change __eo_typeof_eq
      (__eo_typeof (Term.Apply Term.str_to_upper (mkConcat s1 (mkConcat s2 s3))))
      (__eo_typeof
        (mkConcat (Term.Apply Term.str_to_upper s1)
          (mkConcat (Term.Apply Term.str_to_upper (__str_nary_elim (mkConcat s2 s3)))
...
private theorem native_str_to_upper_append (a b : native_String) :
    native_str_to_upper (a ++ b) =
      native_str_to_upper a ++ native_str_to_upper b := by
  simp [native_str_to_upper, List.map_append]
...
private theorem typed___eo_prog_str_to_upper_concat_impl
    (s1 s2 s3 : Term)
    ...
    (hElim : __str_nary_elim (mkConcat s2 s3) ≠ Term.Stuck) :
    RuleProofs.eo_has_bool_type (__eo_prog_str_to_upper_concat s1 s2 s3) := by
  ...
  have hConcat123Ty :
    ...
    smt_typeof_str_concat_of_seq s1 (mkConcat s2 s3) SmtType.Char hS1SmtTy
      hConcat23Ty
  -- LHS = str_to_upper (mkConcat s1 (mkConcat s2 s3)) : Seq Char.
  have hLhsTy :
      __smtx_typeof
          (__eo_to_smt (Term.Apply Term.str_to_upper
            (mkConcat s1 (mkConcat s2 s3)))) = SmtType.Seq SmtType.Char := by
    change __smtx_typeof
        (SmtTerm.str_to_upper (__eo_to_smt (mkConcat s1 (mkConcat s2 s3)))) =
      SmtType.Seq SmtType.Char
    rw [typeof_str_to_upper_eq, hConcat123Ty]
    simp [native_ite, native_Teq]
  -- str_to_upper s1 : Seq Char.
  have hStuS1Ty :
      __smtx_typeof (__eo_to_smt (Term.Apply Term.str_to_upper s1)) =
        SmtType.Seq SmtType.Char := by
    change __smtx_typeof (SmtTerm.str_to_upper (__eo_to_smt s1)) =
      SmtType.Seq SmtType.Char
    rw [typeof_str_to_upper_eq, hS1SmtTy]
    simp [native_ite, native_Teq]
  -- elim : Seq Char.
  have hElimTy :
    ...
    smt_typeof_str_nary_elim_of_seq_ne_stuck (mkConcat s2 s3) SmtType.Char
      hConcat23Ty hElim
  -- str_to_upper elim : Seq Char.
  have hStuElimTy :
      __smtx_typeof
          (__eo_to_smt (Term.Apply Term.str_to_upper
            (__str_nary_elim (mkConcat s2 s3)))) = SmtType.Seq SmtType.Char := by
    change __smtx_typeof
        (SmtTerm.str_to_upper (__eo_to_smt (__str_nary_elim (mkConcat s2 s3)))) =
      SmtType.Seq SmtType.Char
    rw [typeof_str_to_upper_eq, hElimTy]
    simp [native_ite, native_Teq]
  -- String [] : Seq Char.
  have hEmptyTy :
      __smtx_typeof (__eo_to_smt (Term.String [])) = SmtType.Seq SmtType.Char :=
    smt_typeof_empty_string
  -- mkConcat (str_to_upper elim) (String []) : Seq Char.
  ...
  -- RHS = mkConcat (str_to_upper s1) (...) : Seq Char.
  have hRhsTy :
      __smtx_typeof
          (__eo_to_smt (mkConcat (Term.Apply Term.str_to_upper s1)
            (mkConcat
              (Term.Apply Term.str_to_upper (__str_nary_elim (mkConcat s2 s3)))
              (Term.String [])))) = SmtType.Seq SmtType.Char :=
    smt_typeof_str_concat_of_seq _ _ SmtType.Char hStuS1Ty hConcatElimEmptyTy
  rw [prog_str_to_upper_concat_eq_of_ne_stuck s1 s2 s3 hS1NotStuck hS2NotStuck
    hS3NotStuck hElim]
...
private theorem facts___eo_prog_str_to_upper_concat_impl
    ...
    (hS3Ty : __eo_typeof s3 = Term.Apply Term.Seq (Term.UOp UserOp.Char))
    (hElim : __str_nary_elim (mkConcat s2 s3) ≠ Term.Stuck) :
    eo_interprets M (__eo_prog_str_to_upper_concat s1 s2 s3) true := by
  have hS1NotStuck : s1 ≠ Term.Stuck :=
    RuleProofs.term_ne_stuck_of_has_smt_translation s1 hS1Trans
  have hS2NotStuck : s2 ≠ Term.Stuck :=
    RuleProofs.term_ne_stuck_of_has_smt_translation s2 hS2Trans
  ...
  have hStuS1Ty :
    ...
    change __smtx_typeof (SmtTerm.str_to_upper (__eo_to_smt s1)) =
      SmtType.Seq SmtType.Char
    rw [typeof_str_to_upper_eq, hS1SmtTy]; simp [native_ite, native_Teq]
  have hStuElimTy :
      __smtx_typeof
          (__eo_to_smt (Term.Apply Term.str_to_upper
            (__str_nary_elim (mkConcat s2 s3)))) = SmtType.Seq SmtType.Char := by
    change __smtx_typeof
        (SmtTerm.str_to_upper (__eo_to_smt (__str_nary_elim (mkConcat s2 s3)))) =
      SmtType.Seq SmtType.Char
    rw [typeof_str_to_upper_eq, hElimTy]; simp [native_ite, native_Teq]
  have hStuConcat23Ty :
      __smtx_typeof
          (__eo_to_smt (Term.Apply Term.str_to_upper (mkConcat s2 s3))) =
        SmtType.Seq SmtType.Char := by
    change __smtx_typeof
        (SmtTerm.str_to_upper (__eo_to_smt (mkConcat s2 s3))) =
      SmtType.Seq SmtType.Char
    rw [typeof_str_to_upper_eq, hConcat23Ty]; simp [native_ite, native_Teq]
  have hEmptyTy :
      __smtx_typeof (__eo_to_smt (Term.String [])) = SmtType.Seq SmtType.Char :=
    smt_typeof_empty_string
  have hConcatElimEmptyTy :
      ...
      __smtx_typeof
          (__eo_to_smt (mkConcat (Term.Apply Term.str_to_upper s1)
            (mkConcat
              ...
              (Term.String [])))) = SmtType.Seq SmtType.Char :=
    smt_typeof_str_concat_of_seq _ _ SmtType.Char hStuS1Ty hConcatElimEmptyTy
  -- str_to_upper distributes over str_concat (value level, on s1 ++ (s2++s3)).
        ...
        (Term.Apply (Term.Apply Term.eq
          ...
          (mkConcat (Term.Apply Term.str_to_upper s1)
            (mkConcat
              (Term.Apply Term.str_to_upper (__str_nary_elim (mkConcat s2 s3)))
              (Term.String [])))) := by
    have := typed___eo_prog_str_to_upper_concat_impl s1 s2 s3 hS1Trans hS2Trans
      hS3Trans hS1Ty hS2Ty hS3Ty hElim
    rw [prog_str_to_upper_concat_eq_of_ne_stuck s1 s2 s3 hS1NotStuck hS2NotStuck
      hS3NotStuck hElim] at this
    exact this
  -- Step A: eval(str_to_upper elim) rel eval(str_to_upper (mkConcat s2 s3)).
  have hElimRel :
      RuleProofs.smt_value_rel
      ...
      RuleProofs.smt_value_rel
        (__smtx_model_eval M
          (__eo_to_smt (Term.Apply Term.str_to_upper
            (__str_nary_elim (mkConcat s2 s3)))))
        (__smtx_model_eval M
          (__eo_to_smt (Term.Apply Term.str_to_upper (mkConcat s2 s3)))) := by
    have hL :
        __smtx_model_eval M
            (__eo_to_smt (Term.Apply Term.str_to_upper
              (__str_nary_elim (mkConcat s2 s3)))) =
      ...
      change __smtx_model_eval M
          (SmtTerm.str_to_upper (__eo_to_smt (__str_nary_elim (mkConcat s2 s3)))) = _
      rw [smtx_eval_str_to_upper_term_eq]
    have hR :
        __smtx_model_eval M
            (__eo_to_smt (Term.Apply Term.str_to_upper (mkConcat s2 s3))) =
          __smtx_model_eval_str_to_upper
      ...
      change __smtx_model_eval M
          (SmtTerm.str_to_upper (__eo_to_smt (mkConcat s2 s3))) = _
      rw [smtx_eval_str_to_upper_term_eq]
    rw [hL, hR]
    exact smt_value_rel_model_eval_str_to_upper_of_rel _ _ hElimRel
  -- Step B: mkConcat (str_to_upper elim) (String []) rel str_to_upper (mkConcat s2 s3)
  have hEmptyRel :
      RuleProofs.smt_value_rel
  ...
  -- drop trailing empty on (str_to_upper elim) ++ String []
  have hDropEmpty :
      RuleProofs.smt_value_rel
        (__smtx_model_eval M
          (__eo_to_smt (mkConcat
  ...
  -- combine: mkConcat (str_to_upper elim) (String []) rel str_to_upper (mkConcat s2 s3)
  have hCElim :
      RuleProofs.smt_value_rel
        (__smtx_model_eval M
          (__eo_to_smt (mkConcat
  ...
  -- Step C: right_congr to inject under (str_to_upper s1) ++ _
  have hRhsRel :
      RuleProofs.smt_value_rel
        (__smtx_model_eval M
          (__eo_to_smt (mkConcat (Term.Apply Term.str_to_upper s1)
        ...
        (__smtx_model_eval M
          (__eo_to_smt (mkConcat (Term.Apply Term.str_to_upper s1)
            (Term.Apply Term.str_to_upper (mkConcat s2 s3))))) :=
    smt_value_rel_str_concat_right_congr M hM
      (Term.Apply Term.str_to_upper s1)
      (mkConcat
        (Term.Apply Term.str_to_upper (__str_nary_elim (mkConcat s2 s3)))
        (Term.String []))
      (Term.Apply Term.str_to_upper (mkConcat s2 s3))
      SmtType.Char hStuS1Ty hConcatElimEmptyTy hStuConcat23Ty hCElim
  -- Step D: distribution. eval(str_to_upper s1 ++ str_to_upper (s2++s3))
  --   = eval(str_to_upper (s1 ++ (s2++s3))).
  have hDistrEq :
      __smtx_model_eval M
          (__eo_to_smt (mkConcat (Term.Apply Term.str_to_upper s1)
            (Term.Apply Term.str_to_upper (mkConcat s2 s3)))) =
    ...
    -- LHS: str_to_upper s1 ++ str_to_upper (s2++s3)
    have hLhs :
        __smtx_model_eval M
            (__eo_to_smt (mkConcat (Term.Apply Term.str_to_upper s1)
              (Term.Apply Term.str_to_upper (mkConcat s2 s3)))) =
          SmtValue.Seq (native_pack_seq SmtType.Char
            ((native_str_to_upper (native_unpack_string v1)).map SmtValue.Char ++
             (native_str_to_upper (native_unpack_string v23)).map SmtValue.Char)) := by
      rw [smtx_model_eval_str_concat_term_eq]
      change __smtx_model_eval_str_concat
          (__smtx_model_eval M (SmtTerm.str_to_upper (__eo_to_smt s1)))
          (__smtx_model_eval M
            (SmtTerm.str_to_upper (__eo_to_smt (mkConcat s2 s3)))) = _
      rw [smtx_eval_str_to_upper_term_eq, smtx_eval_str_to_upper_term_eq,
        hV1Eval, hV23Eval]
      simp only [__smtx_model_eval_str_to_upper, __smtx_model_eval_str_concat,
        native_seq_concat, native_pack_string, elem_typeof_pack_seq,
        Smtm.native_unpack_pack_seq]
    -- RHS: str_to_upper (s1 ++ (s2++s3))
    have hRhs :
        __smtx_model_eval M
            (__eo_to_smt (Term.Apply Term.str_to_upper
          ...
          SmtValue.Seq (native_pack_seq SmtType.Char
            ((native_str_to_upper
              (native_unpack_string v1 ++ native_unpack_string v23)).map
                SmtValue.Char)) := by
      change __smtx_model_eval M
          (SmtTerm.str_to_upper (__eo_to_smt (mkConcat s1 (mkConcat s2 s3)))) = _
      rw [smtx_eval_str_to_upper_term_eq, smtx_model_eval_str_concat_term_eq,
        hV1Eval, hV23Eval]
      simp only [__smtx_model_eval_str_to_upper, __smtx_model_eval_str_concat,
        native_seq_concat, native_pack_string, native_unpack_string, hV1Elem,
        Smtm.native_unpack_pack_seq, List.map_append]
    rw [hLhs, hRhs, native_str_to_upper_append, List.map_append]
  -- chain everything: rhsT to lhsT
  -- rhsRel : RHS-full rel (str_to_upper s1 ++ str_to_upper (s2++s3))
  -- hDistrEq : (str_to_upper s1 ++ str_to_upper (s2++s3)) = lhsT (as eval values)
  have hRhsToLhs :
      RuleProofs.smt_value_rel
        (__smtx_model_eval M
          (__eo_to_smt (mkConcat (Term.Apply Term.str_to_upper s1)
...
public theorem cmd_step_str_to_upper_concat_properties
  ...
  __eo_typeof (__eo_cmd_step_proven s CRule.str_to_upper_concat args premises) = Term.Bool ->
  StepRuleProperties M (premiseTermList s premises)
    (__eo_cmd_step_proven s CRule.str_to_upper_concat args premises) :=
by
  intro hCmdTrans _hPremisesBool hResultTy
  have hProg : __eo_cmd_step_proven s CRule.str_to_upper_concat args premises ≠
      Term.Stuck :=
          ...
          | cons a3 args =>
              ...
              | nil =>
                  ...
                  | nil =>
                      ...
                      change
                        __eo_typeof (__eo_prog_str_to_upper_concat a1 a2 a3) =
                      ...
                      have hProps :
                          StepRuleProperties M (premiseTermList s CIndexList.nil)
                            (__eo_prog_str_to_upper_concat a1 a2 a3) := by
                        refine ⟨?_,
                          RuleProofs.eo_has_smt_translation_of_has_bool_type _
                            (typed___eo_prog_str_to_upper_concat_impl a1 a2 a3
                              hA1Trans hA2Trans hA3Trans hA1Ty hA2Ty hA3Ty hElim)⟩
                        intro _hTrue
                        exact facts___eo_prog_str_to_upper_concat_impl M hM a1 a2 a3
                          hA1Trans hA2Trans hA3Trans hA1Ty hA2Ty hA3Ty hElim
                      change StepRuleProperties M []
                        (__eo_prog_str_to_upper_concat a1 a2 a3)
