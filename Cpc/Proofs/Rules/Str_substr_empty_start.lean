...
private abbrev substrStartPremise (n x : Term) : Term :=
  Term.Apply
    (Term.Apply Term.eq
      (Term.Apply (Term.Apply Term.geq n) (Term.Apply Term.str_len x)))
    (Term.Boolean true)

private abbrev substrStartLhs (x n m : Term) : Term :=
  Term.Apply (Term.Apply (Term.Apply Term.str_substr x) n) m

private abbrev substrStartConclusion (x n m A : Term) : Term :=
  Term.Apply (Term.Apply Term.eq (substrStartLhs x n m)) (__seq_empty A)
...
private theorem prog_str_substr_empty_start_info
    (x n m A P : Term)
    (hProg : __eo_prog_str_substr_empty_start x n m A (Proof.pf P) ≠
      Term.Stuck) :
    ∃ T n0 x0,
      A = Term.Apply Term.Seq T ∧
      P = substrStartPremise n0 x0 ∧
      n0 = n ∧
      x0 = x ∧
      __eo_prog_str_substr_empty_start x n m A (Proof.pf P) =
        substrStartConclusion x n m (Term.Apply Term.Seq T) := by
  unfold __eo_prog_str_substr_empty_start at hProg
  ...
  next heq =>
    ...
    simp [__eo_prog_str_substr_empty_start, __eo_requires, __eo_and, __eo_eq,
      SmtEval.native_ite, native_teq, SmtEval.native_not, SmtEval.native_and,
      eo_mk_apply_eq_seq_empty, substrStartConclusion, substrStartLhs]
...
private theorem facts___eo_prog_str_substr_empty_start_impl
    (M : SmtModel) (hM : model_wf M) (x n m P T : Term)
    (hBoolEq :
      RuleProofs.eo_has_bool_type
        (substrStartConclusion x n m (Term.Apply Term.Seq T)))
    ...
    (hPrem : eo_interprets M (substrStartPremise n x) true)
    (hProgEq :
      __eo_prog_str_substr_empty_start x n m (Term.Apply Term.Seq T)
          (Proof.pf P) =
        substrStartConclusion x n m (Term.Apply Term.Seq T)) :
    eo_interprets M
      (__eo_prog_str_substr_empty_start x n m (Term.Apply Term.Seq T)
        (Proof.pf P)) true := by
  let lhs := substrStartLhs x n m
  ...
  have hNStartGeLen : native_seq_len (native_unpack_seq sx) <= ni := by
    ...
    | intro_true _ hEval =>
        ...
        rw [smtx_eval_eq_term_eq, smtx_eval_geq_term_eq, hNEval,
          smtx_eval_str_len_term_eq, hXEval,
          smtx_eval_boolean_term_eq] at hEval
        have hLeBool :
            native_zleq (native_seq_len (native_unpack_seq sx)) ni = true := by
  ...
  have hEvalEq :
    ...
    simp [__smtx_model_eval_str_substr, hXElem,
      native_seq_extract_empty_of_start_ge_len _ _ _ hNStartGeLen,
      native_pack_seq]
  rw [hProgEq]
  exact RuleProofs.eo_interprets_eq_of_rel M lhs rhs
    (by simpa [substrStartConclusion, lhs, rhs] using hBoolEq) <| by
    rw [hEvalEq]
...
public theorem cmd_step_str_substr_empty_start_properties
  ...
  __eo_typeof (__eo_cmd_step_proven s CRule.str_substr_empty_start args premises) = Term.Bool ->
  StepRuleProperties M (premiseTermList s premises)
    (__eo_cmd_step_proven s CRule.str_substr_empty_start args premises) :=
by
  intro hCmdTrans _hPremisesBool hResultTy
  have hProg : __eo_cmd_step_proven s CRule.str_substr_empty_start args premises ≠
      Term.Stuck :=
                  ...
                  | nil =>
                      ...
                      | cons p premises =>
                          ...
                          | nil =>
                              ...
                              change __eo_typeof
                                  (__eo_prog_str_substr_empty_start a1 a2 a3 A
                                    (Proof.pf P)) =
                              ...
                              have hProgRule :
                                  __eo_prog_str_substr_empty_start a1 a2 a3 A
                                    ...
                                    Term.Stuck :=
                                term_ne_stuck_of_typeof_bool hResultTy
                              rcases prog_str_substr_empty_start_info a1 a2 a3 A P
                                  hProgRule with
                                ⟨T, n0, x0, hATyArg, hPremShape, hn0, hx0,
                                  hProgEq⟩
                              subst A
                              subst n0
                              subst x0
                              let lhs := substrStartLhs a1 a2 a3
                              ...
                              have hBoolEq :
                                  RuleProofs.eo_has_bool_type
                                    (substrStartConclusion a1 a2 a3
                                      (Term.Apply Term.Seq T)) := by
                                have hEqBool :
                                    RuleProofs.eo_has_bool_type
                                ...
                                simpa [substrStartConclusion, lhs, rhs] using
                                  hEqBool
                              refine ⟨?_, ?_⟩
                              · intro hTrue
                                ...
                                have hPrem :
                                    eo_interprets M
                                      (substrStartPremise a2 a1) true := by
                                  simpa [hPremShape] using hPremRaw
                                change eo_interprets M
                                  (__eo_prog_str_substr_empty_start a1 a2 a3
                                    (Term.Apply Term.Seq T) (Proof.pf P)) true
                                exact facts___eo_prog_str_substr_empty_start_impl
