...
private theorem prog_bool_xor_elim_eq_of_ne_stuck (x1 y1 : Term) :
    x1 ≠ Term.Stuck ->
    y1 ≠ Term.Stuck ->
    __eo_prog_bool_xor_elim x1 y1 =
  ...
  intro hX1 hY1
  cases x1 <;> cases y1 <;> simp [__eo_prog_bool_xor_elim] at hX1 hY1 ⊢

private theorem typeof_args_of_prog_bool_xor_elim_bool (x1 y1 : Term) :
    __eo_typeof (__eo_prog_bool_xor_elim x1 y1) = Term.Bool ->
  ...
  · by_cases hY1 : y1 = Term.Stuck
    · subst y1
      simp [__eo_prog_bool_xor_elim] at hTy
      have hStuckTy : __eo_typeof Term.Stuck ≠ Term.Bool := by
        native_decide
      exact False.elim (hStuckTy hTy)
    · rw [prog_bool_xor_elim_eq_of_ne_stuck x1 y1 hX1 hY1] at hTy
      change __eo_typeof_eq
        (__eo_typeof_or (__eo_typeof x1) (__eo_typeof y1))
...
private theorem typed___eo_prog_bool_xor_elim_impl (x1 y1 : Term) :
  RuleProofs.eo_has_smt_translation x1 ->
  RuleProofs.eo_has_smt_translation y1 ->
  __eo_typeof (__eo_prog_bool_xor_elim x1 y1) = Term.Bool ->
  RuleProofs.eo_has_bool_type (__eo_prog_bool_xor_elim x1 y1) := by
  ...
  rcases typeof_args_of_prog_bool_xor_elim_bool x1 y1 hResultTy with
    ⟨hX1Type, hY1Type⟩
  ...
  rw [prog_bool_xor_elim_eq_of_ne_stuck x1 y1 hX1NotStuck hY1NotStuck]
...
private theorem facts___eo_prog_bool_xor_elim_impl
    (M : SmtModel) (hM : model_wf M) (x1 y1 : Term) :
  RuleProofs.eo_has_smt_translation x1 ->
  RuleProofs.eo_has_smt_translation y1 ->
  __eo_typeof (__eo_prog_bool_xor_elim x1 y1) = Term.Bool ->
  eo_interprets M (__eo_prog_bool_xor_elim x1 y1) true := by
  intro hX1Trans hY1Trans hResultTy
  have hX1NotStuck : x1 ≠ Term.Stuck :=
    RuleProofs.term_ne_stuck_of_has_smt_translation x1 hX1Trans
  have hY1NotStuck : y1 ≠ Term.Stuck :=
    RuleProofs.term_ne_stuck_of_has_smt_translation y1 hY1Trans
  have hProgBool : RuleProofs.eo_has_bool_type (__eo_prog_bool_xor_elim x1 y1) :=
    typed___eo_prog_bool_xor_elim_impl x1 y1 hX1Trans hY1Trans hResultTy
  rcases typeof_args_of_prog_bool_xor_elim_bool x1 y1 hResultTy with
    ⟨hX1Type, hY1Type⟩
  have hX1Bool : RuleProofs.eo_has_bool_type x1 :=
    TranslationProofs.eo_to_smt_well_typed_and_typeof_implies_smt_type
  ...
  · simpa [prog_bool_xor_elim_eq_of_ne_stuck x1 y1 hX1NotStuck hY1NotStuck]
      using hProgBool
  · rcases RuleProofs.eo_eval_is_boolean_of_has_bool_type M hM x1 hX1Bool with
    ...
    rw [show __eo_to_smt
      ...
      rfl]
    rw [__smtx_model_eval.eq_11, smtx_eval_eq_term_eq,
      __smtx_model_eval.eq_7, hEvalX1, hEvalY1]
...
public theorem cmd_step_bool_xor_elim_properties
    (M : SmtModel) (hM : model_wf M)
    (s : CState) (args : CArgList) (premises : CIndexList) :
  cmdTranslationOk (CCmd.step CRule.bool_xor_elim args premises) ->
  AllHaveBoolType (premiseTermList s premises) ->
  __eo_typeof (__eo_cmd_step_proven s CRule.bool_xor_elim args premises) = Term.Bool ->
  StepRuleProperties M (premiseTermList s premises)
    (__eo_cmd_step_proven s CRule.bool_xor_elim args premises) :=
by
  intro hCmdTrans _hPremisesBool hResultTy
  have hProg : __eo_cmd_step_proven s CRule.bool_xor_elim args premises ≠ Term.Stuck :=
      ...
      | cons a2 args =>
          ...
          | nil =>
              ...
              | nil =>
                  ...
                  change __eo_typeof (__eo_prog_bool_xor_elim a1 a2) = Term.Bool
                    at hResultTy
                  refine ⟨?_, ?_⟩
                  · intro _hTrue
                    change eo_interprets M (__eo_prog_bool_xor_elim a1 a2) true
                    exact facts___eo_prog_bool_xor_elim_impl M hM a1 a2
                      hA1Trans hA2Trans hResultTy
                  · exact RuleProofs.eo_has_smt_translation_of_has_bool_type _
                      (typed___eo_prog_bool_xor_elim_impl a1 a2 hA1Trans
                        hA2Trans hResultTy)
              | cons _ _ =>
                  change Term.Stuck ≠ Term.Stuck at hProg
                  exact False.elim (hProg rfl)
