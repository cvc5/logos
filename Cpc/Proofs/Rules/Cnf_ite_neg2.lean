...
public theorem cmd_step_cnf_ite_neg2_properties
    (M : SmtModel) (hM : model_wf M)
    (s : CState) (args : CArgList) (premises : CIndexList) :
  cmdTranslationOk (CCmd.step CRule.cnf_ite_neg2 args premises) ->
  AllHaveBoolType (premiseTermList s premises) ->
  __eo_typeof (__eo_cmd_step_proven s CRule.cnf_ite_neg2 args premises) = Term.Bool ->
  StepRuleProperties M (premiseTermList s premises)
    (__eo_cmd_step_proven s CRule.cnf_ite_neg2 args premises) :=
by
  intro hCmdTrans _hPremisesBool hResultTy
  have hProg : __eo_cmd_step_proven s CRule.cnf_ite_neg2 args premises ≠ Term.Stuck :=
  ...
  | cons F args =>
      ...
      | nil =>
          cases premises with
          | nil =>
              change __eo_prog_cnf_ite_neg2 F ≠ Term.Stuck at hProg
              change __eo_typeof (__eo_prog_cnf_ite_neg2 F) = Term.Bool at hResultTy
              cases F with
              | Apply f F2 =>
                  cases f with
                  | Apply g F1 =>
                      ...
                      | Apply op C =>
                          ...
                          | UOp op =>
                              ...
                              | ite =>
                                  ...
                                  simp [__eo_prog_cnf_ite_neg2] at hTyData
                                  ...
                                  have hResultTrue :
                                      eo_interprets M
                                        (__eo_prog_cnf_ite_neg2
                                          (Term.Apply
                                            (Term.Apply (Term.Apply (Term.UOp UserOp.ite) C) F1)
                                            F2)) true := by
                                    rcases CnfSupport.eo_interprets_bool_cases M hM C hCBool with
                                      hCTrue | hCFalse
                                    · simpa [__eo_prog_cnf_ite_neg2] using
                                        CnfSupport.clause3_middle_true M hM
                                          (Term.Apply
                                            (Term.Apply (Term.Apply (Term.UOp UserOp.ite) C) F1)
                                            F2)
                                          C (Term.Apply (Term.UOp UserOp.not) F2)
                                          hIteBool hCTrue hNotF2Bool
