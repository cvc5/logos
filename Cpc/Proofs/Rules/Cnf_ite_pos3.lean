...
public theorem cmd_step_cnf_ite_pos3_properties
    (M : SmtModel) (hM : model_wf M)
    (s : CState) (args : CArgList) (premises : CIndexList) :
  cmdTranslationOk (CCmd.step CRule.cnf_ite_pos3 args premises) ->
  AllHaveBoolType (premiseTermList s premises) ->
  __eo_typeof (__eo_cmd_step_proven s CRule.cnf_ite_pos3 args premises) = Term.Bool ->
  StepRuleProperties M (premiseTermList s premises)
    (__eo_cmd_step_proven s CRule.cnf_ite_pos3 args premises) :=
by
  intro hCmdTrans _hPremisesBool hResultTy
  have hProg : __eo_cmd_step_proven s CRule.cnf_ite_pos3 args premises ≠ Term.Stuck :=
  ...
  | cons F args =>
      ...
      | nil =>
          cases premises with
          | nil =>
              change __eo_prog_cnf_ite_pos3 F ≠ Term.Stuck at hProg
              change __eo_typeof (__eo_prog_cnf_ite_pos3 F) = Term.Bool at hResultTy
              cases F with
              | Apply f F2 =>
                  cases f with
                  | Apply g F1 =>
                      ...
                      | Apply op C =>
                          ...
                          | UOp op =>
                              ...
                              | ite =>
                                  ...
                                  simp [__eo_prog_cnf_ite_pos3] at hTyData
                                          ...
                                          CnfSupport.clause3_right_true M hM
                                            (Term.Apply (Term.UOp UserOp.not)
                                              (Term.Apply
                                                (Term.Apply (Term.Apply (Term.UOp UserOp.ite) C) F1)
                                                F2))
                                            F1 F2 hNotIteBool hF1Bool hF2True
                                        ...
                                        · have hIteFalse :
                                              eo_interprets M
                                                (Term.Apply
                                                  ...
                                                  F2)
                                                false :=
                                            CnfSupport.eo_interprets_ite_false_of_cond_true M C F1 F2
                                              hCTrue hF1False hF2Bool
                                          simpa [__eo_prog_cnf_ite_pos3] using
