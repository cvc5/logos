...
private def bvRedandElimBody (x w : Term) : Term :=
...
private theorem bv_redand_eliminate_shape_of_ne_stuck (x w P : Term) :
    __eo_prog_bv_redand_eliminate x w (Proof.pf P) ≠ Term.Stuck ->
  ...
  have hx : x ≠ Term.Stuck := by
    intro hx
    subst x
    exact hProg (__eo_prog_bv_redand_eliminate.eq_1 w (Proof.pf P))
  have hw : w ≠ Term.Stuck := by
    intro hw
    subst w
    exact hProg (__eo_prog_bv_redand_eliminate.eq_2 x (Proof.pf P) hx)
  refine ⟨hx, hw, ?_⟩
  ...
  · exact False.elim (hProg
      (__eo_prog_bv_redand_eliminate.eq_4 x w (Proof.pf P) hx hw (by
        intro pw px hP
        cases hP
        exact hShape ⟨pw, px, rfl⟩)))

private theorem bv_redand_eliminate_guard_eqs_of_ne_stuck
    {x w pw px body : Term} :
...
private theorem prog_bv_redand_eliminate_canonical_eq_of_ne_stuck (x w : Term) :
    x ≠ Term.Stuck ->
    w ≠ Term.Stuck ->
    __eo_prog_bv_redand_eliminate x w
        (Proof.pf (Term.Apply (Term.Apply (Term.UOp UserOp.eq) w)
          (Term.Apply (Term.UOp UserOp._at_bvsize) x))) =
      bvRedandElimBody x w := by
  intro hx hw
  rw [__eo_prog_bv_redand_eliminate.eq_3 x w w x hx hw]
  rw [RuleProofs.eo_eq_self_of_ne_stuck w hw,
    RuleProofs.eo_eq_self_of_ne_stuck x hx]
  simp [bvRedandElimBody, __eo_and, __eo_requires, native_ite, native_teq,
...
private theorem typeof_args_of_redand_body_bool (x w : Term) :
    w ≠ Term.Stuck ->
    __eo_typeof (bvRedandElimBody x w) = Term.Bool ->
  ...
  rcases RuleProofs.eo_typeof_bvredand_arg_bitvec_of_ne_stuck _ hOperandsNN.1 with
    ⟨u, hXTy⟩
  rcases RuleProofs.eo_typeof_bvcomp_args_of_ne_stuck _ _ hOperandsNN.2 with
    ⟨v, hXTy', hNotAtTy⟩
  rw [hXTy] at hXTy'
  ...
  have hNotAtNN :
    ...
    intro h
    cases h
  rcases RuleProofs.eo_typeof_bvnot_arg_bitvec_of_ne_stuck _ hNotAtNN with
    ⟨q, hAtTy⟩
  have hNotAtTy' :
      Term.Apply (Term.UOp UserOp.BitVec) q =
        Term.Apply (Term.UOp UserOp.BitVec) u := by
    simpa [__eo_typeof_bvnot, hAtTy] using hNotAtTy
  injection hNotAtTy' with _ hQU
  subst q
  have hWU : w = u :=
...
private theorem smt_typeof_redand_rhs
    (x : Term) (n : native_Int) :
    native_zleq 0 n = true ->
          ...
          (Term.Apply (Term.Apply (Term.UOp UserOp.bvcomp) x)
            (Term.Apply (Term.UOp UserOp.bvnot)
              (Term.Apply (Term.UOp1 UserOp1.int_to_bv (Term.Numeral n))
                (Term.Numeral 0))))) =
      __smtx_model_eval M
        (__eo_to_smt (Term.Apply (Term.UOp UserOp.bvredand) x)) := by
  intro hNonneg hXSmtTy
...
private theorem smt_typeof_redand_lhs
    (x : Term) (n : native_Int) :
    __smtx_typeof (__eo_to_smt x) = SmtType.BitVec (native_int_to_nat n) ->
    __smtx_typeof (__eo_to_smt (Term.Apply (Term.UOp UserOp.bvredand) x)) =
      SmtType.BitVec 1 := by
  intro hXSmtTy
  change __smtx_typeof
      (SmtTerm.bvcomp (__eo_to_smt x)
...
private theorem typed_redand_body (x w : Term) :
    ...
    w ≠ Term.Stuck ->
    __eo_typeof (bvRedandElimBody x w) = Term.Bool ->
    RuleProofs.eo_has_bool_type (bvRedandElimBody x w) := by
  intro hXTrans hW hTy
  rcases typeof_args_of_redand_body_bool x w hW hTy with ⟨u, hXTy, hWU⟩
  subst w
  rcases smt_bitvec_type_of_eo_bitvec_type_with_width x u hXTrans hXTy with
    ⟨n, hU, hNonneg, hXSmtTy⟩
  subst u
  unfold RuleProofs.eo_has_bool_type
  unfold bvRedandElimBody
  change __smtx_typeof
      (SmtTerm.eq (__eo_to_smt (Term.Apply (Term.UOp UserOp.bvredand) x))
        (__eo_to_smt
          (Term.Apply (Term.Apply (Term.UOp UserOp.bvcomp) x)
            (Term.Apply (Term.UOp UserOp.bvnot)
              (Term.Apply (Term.UOp1 UserOp1.int_to_bv (Term.Numeral n)) (Term.Numeral 0)))))) =
    SmtType.Bool
  have hLhsTy := smt_typeof_redand_lhs x n hXSmtTy
  have hRhsTy := smt_typeof_redand_rhs x n hNonneg hXSmtTy
  rw [typeof_eq_eq]
  simp [__smtx_typeof_eq, hLhsTy, hRhsTy, __smtx_typeof_guard,
    native_Teq, native_ite]

private theorem facts_redand_body
    (M : SmtModel) (x w : Term) :
    RuleProofs.eo_has_smt_translation x ->
    w ≠ Term.Stuck ->
    __eo_typeof (bvRedandElimBody x w) = Term.Bool ->
    eo_interprets M (bvRedandElimBody x w) true := by
  intro hXTrans hW hTy
  rcases typeof_args_of_redand_body_bool x w hW hTy with ⟨u, hXTy, hWU⟩
  subst w
  rcases smt_bitvec_type_of_eo_bitvec_type_with_width x u hXTrans hXTy with
  ...
  have hBodyBool : RuleProofs.eo_has_bool_type
      (bvRedandElimBody x (Term.Numeral n)) :=
    typed_redand_body x (Term.Numeral n) hXTrans (by intro h; cases h)
      (by simpa using hTy)
  unfold bvRedandElimBody
  apply RuleProofs.eo_interprets_eq_of_rel M
  · simpa [bvRedandElimBody] using hBodyBool
  · change RuleProofs.smt_value_rel
      (__smtx_model_eval M (__eo_to_smt (Term.Apply (Term.UOp UserOp.bvredand) x)))
      (__smtx_model_eval M
        (__eo_to_smt
          (Term.Apply (Term.Apply (Term.UOp UserOp.bvcomp) x)
            (Term.Apply (Term.UOp UserOp.bvnot)
              (Term.Apply (Term.UOp1 UserOp1.int_to_bv (Term.Numeral n)) (Term.Numeral 0))))))
    rw [smt_eval_redand_rhs_eq_lhs M x n hNonneg hXSmtTy]
    exact RuleProofs.smt_value_rel_refl _

private theorem trusted_bv_redand_eliminate_canonical_properties
    (M : SmtModel) (x w : Term) :
  ...
  have hProgEq := prog_bv_redand_eliminate_canonical_eq_of_ne_stuck x w hxNe hwNe
  have hBodyTy : __eo_typeof (bvRedandElimBody x w) = Term.Bool := by
    simpa [hProgEq] using hResultTy
  rw [hProgEq]
  refine ⟨?_, ?_⟩
  · intro _hPrem
    exact facts_redand_body M x w hXTrans hwNe hBodyTy
...
public theorem cmd_step_bv_redand_eliminate_properties
  ...
  __eo_typeof (__eo_cmd_step_proven s CRule.bv_redand_eliminate args premises) = Term.Bool ->
  StepRuleProperties M (premiseTermList s premises)
    (__eo_cmd_step_proven s CRule.bv_redand_eliminate args premises) :=
by
  intro hCmdTrans _hPremisesBool hResultTy
  have hProg : __eo_cmd_step_proven s CRule.bv_redand_eliminate args premises ≠
      Term.Stuck :=
              ...
              | cons n1 premises =>
                  ...
                  | nil =>
                      ...
                      change StepRuleProperties M [P1]
                        (__eo_prog_bv_redand_eliminate a1 a2 (Proof.pf P1))
                      have hProgLocal :
                          __eo_prog_bv_redand_eliminate a1 a2 (Proof.pf P1) ≠
                            Term.Stuck := by
                        exact hProg
                      rcases bv_redand_eliminate_shape_of_ne_stuck a1 a2 P1 hProgLocal with
                        ⟨ha1, ha2, pw, px, hP1⟩
                      subst P1
                      have hReqNe := by
                        have h := hProgLocal
                        rw [hP1] at h
                        rw [__eo_prog_bv_redand_eliminate.eq_3 a1 a2 pw px ha1 ha2] at h
                        exact h
                      rcases bv_redand_eliminate_guard_eqs_of_ne_stuck hReqNe with
                        ⟨hpw, hpx⟩
                      subst pw
                      subst px
                      have hArgsTrans :
                      ...
                      have hResultTyCanonical :
                          __eo_typeof
                              (__eo_prog_bv_redand_eliminate a1 a2
                                (Proof.pf
                        ...
                        simpa [hP1] using h
                      simpa [hP1] using
                        trusted_bv_redand_eliminate_canonical_properties M a1 a2
