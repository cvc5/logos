...
private theorem prog_bv_sge_eliminate_eq_of_ne_stuck (x1 y1 : Term) :
    x1 ≠ Term.Stuck ->
    y1 ≠ Term.Stuck ->
    __eo_prog_bv_sge_eliminate x1 y1 =
      Term.Apply (Term.Apply Term.eq
        (Term.Apply (Term.Apply (Term.UOp UserOp.bvsge) x1) y1))
        (Term.Apply (Term.Apply (Term.UOp UserOp.bvsle) y1) x1) := by
  intro hX1 hY1
  cases x1 <;> cases y1 <;> simp [__eo_prog_bv_sge_eliminate] at hX1 hY1 ⊢
...
private theorem typeof_args_of_prog_bv_sge_eliminate_bool (x1 y1 : Term) :
    __eo_typeof (__eo_prog_bv_sge_eliminate x1 y1) = Term.Bool ->
  ...
  · by_cases hY1 : y1 = Term.Stuck
    · subst y1
      cases x1 <;> simp [__eo_prog_bv_sge_eliminate] at hX1 hTy
      all_goals
        have hStuckTy : __eo_typeof Term.Stuck ≠ Term.Bool := by
          native_decide
        exact False.elim (hStuckTy hTy)
    · rw [prog_bv_sge_eliminate_eq_of_ne_stuck x1 y1 hX1 hY1] at hTy
...
private theorem eo_has_bool_type_bvsge_same
    (x1 y1 : Term) :
    RuleProofs.eo_has_smt_translation x1 ->
    RuleProofs.eo_has_smt_translation y1 ->
    __eo_typeof (__eo_prog_bv_sge_eliminate x1 y1) = Term.Bool ->
    RuleProofs.eo_has_bool_type
      (Term.Apply (Term.Apply (Term.UOp UserOp.bvsge) x1) y1) := by
  intro hX1Trans hY1Trans hResultTy
  rcases typeof_args_of_prog_bv_sge_eliminate_bool x1 y1 hResultTy with
...
private theorem typed___eo_prog_bv_sge_eliminate_impl (x1 y1 : Term) :
    ...
    RuleProofs.eo_has_smt_translation y1 ->
    __eo_typeof (__eo_prog_bv_sge_eliminate x1 y1) = Term.Bool ->
    RuleProofs.eo_has_bool_type (__eo_prog_bv_sge_eliminate x1 y1) := by
  intro hX1Trans hY1Trans hResultTy
  ...
  have hSgeBool :
      RuleProofs.eo_has_bool_type
        (Term.Apply (Term.Apply (Term.UOp UserOp.bvsge) x1) y1) :=
    eo_has_bool_type_bvsge_same x1 y1 hX1Trans hY1Trans hResultTy
  have hUleSwapBool :
      RuleProofs.eo_has_bool_type
        (Term.Apply (Term.Apply (Term.UOp UserOp.bvsle) y1) x1) :=
    eo_has_bool_type_bvsle_swap x1 y1 hX1Trans hY1Trans hResultTy
  rw [prog_bv_sge_eliminate_eq_of_ne_stuck x1 y1 hX1NotStuck hY1NotStuck]
  exact RuleProofs.eo_has_bool_type_eq_of_same_smt_type
    (Term.Apply (Term.Apply (Term.UOp UserOp.bvsge) x1) y1)
    (Term.Apply (Term.Apply (Term.UOp UserOp.bvsle) y1) x1)
    (hSgeBool.trans hUleSwapBool.symm)
    (by
      rw [hSgeBool]
      decide)

private theorem eval_bvsge_matches_bvsle_swap
  ...
  rw [__smtx_model_eval.eq_63, __smtx_model_eval.eq_61]
  rfl
...
private theorem facts___eo_prog_bv_sge_eliminate_impl
    ...
    RuleProofs.eo_has_smt_translation y1 ->
    __eo_typeof (__eo_prog_bv_sge_eliminate x1 y1) = Term.Bool ->
    eo_interprets M (__eo_prog_bv_sge_eliminate x1 y1) true := by
  intro hX1Trans hY1Trans hResultTy
  have hX1NotStuck : x1 ≠ Term.Stuck :=
    RuleProofs.term_ne_stuck_of_has_smt_translation x1 hX1Trans
  have hY1NotStuck : y1 ≠ Term.Stuck :=
    RuleProofs.term_ne_stuck_of_has_smt_translation y1 hY1Trans
  have hProgBool : RuleProofs.eo_has_bool_type (__eo_prog_bv_sge_eliminate x1 y1) :=
    typed___eo_prog_bv_sge_eliminate_impl x1 y1 hX1Trans hY1Trans hResultTy
  rw [prog_bv_sge_eliminate_eq_of_ne_stuck x1 y1 hX1NotStuck hY1NotStuck]
  apply RuleProofs.eo_interprets_eq_of_rel M
  · simpa [prog_bv_sge_eliminate_eq_of_ne_stuck x1 y1 hX1NotStuck hY1NotStuck] using
      hProgBool
  · change RuleProofs.smt_value_rel
      ...
      (__smtx_model_eval M
        (__eo_to_smt
          (Term.Apply (Term.Apply (Term.UOp UserOp.bvsle) y1) x1)))
    rw [eval_bvsge_matches_bvsle_swap M hM x1 y1 hX1Trans hY1Trans hResultTy]
    exact RuleProofs.smt_value_rel_refl _

public theorem cmd_step_bv_sge_eliminate_properties
    (M : SmtModel) (hM : model_wf M)
    (s : CState) (args : CArgList) (premises : CIndexList) :
  cmdTranslationOk (CCmd.step CRule.bv_sge_eliminate args premises) ->
  AllHaveBoolType (premiseTermList s premises) ->
  __eo_typeof (__eo_cmd_step_proven s CRule.bv_sge_eliminate args premises) = Term.Bool ->
  StepRuleProperties M (premiseTermList s premises)
    (__eo_cmd_step_proven s CRule.bv_sge_eliminate args premises) :=
by
  intro hCmdTrans _hPremisesBool hResultTy
  have hProg : __eo_cmd_step_proven s CRule.bv_sge_eliminate args premises ≠ Term.Stuck :=
      ...
      | cons a2 args =>
          ...
          | nil =>
              ...
              | nil =>
                  ...
                  change __eo_typeof (__eo_prog_bv_sge_eliminate a1 a2) = Term.Bool at hResultTy
                  refine ⟨?_, ?_⟩
                  · intro _hTrue
                    change eo_interprets M (__eo_prog_bv_sge_eliminate a1 a2) true
                    exact facts___eo_prog_bv_sge_eliminate_impl M hM a1 a2
                      hA1Trans hA2Trans hResultTy
                  · exact RuleProofs.eo_has_smt_translation_of_has_bool_type _
                      (typed___eo_prog_bv_sge_eliminate_impl a1 a2 hA1Trans hA2Trans
                        hResultTy)
              | cons _ _ =>
                  change Term.Stuck ≠ Term.Stuck at hProg
                  exact False.elim (hProg rfl)
