...
private abbrev updateNegPremise (n : Term) : Term :=
  Term.Apply
    (Term.Apply Term.eq
      (Term.Apply (Term.Apply Term.gt (Term.Numeral 0)) n))
    (Term.Boolean true)

private abbrev updateNegLhs (t n r : Term) : Term :=
  Term.Apply (Term.Apply (Term.Apply Term.str_update t) n) r

private abbrev updateNegConclusion (t n r : Term) : Term :=
  Term.Apply
    (Term.Apply Term.eq (updateNegLhs t n r))
    t
...
private theorem prog_str_update_neg_info
    (t n r P : Term)
    (hProg : __eo_prog_str_update_neg t n r (Proof.pf P) ≠
      Term.Stuck) :
    ∃ n0,
      P = updateNegPremise n0 ∧
      n0 = n ∧
      __eo_prog_str_update_neg t n r (Proof.pf P) =
        updateNegConclusion t n r := by
  unfold __eo_prog_str_update_neg at hProg
  ...
  next heq =>
    ...
    subst_vars
    refine ⟨_, rfl, rfl, ?_⟩
    simp [__eo_prog_str_update_neg, __eo_requires, __eo_eq,
      SmtEval.native_ite, native_teq, SmtEval.native_not,
      updateNegConclusion, updateNegLhs]

private theorem typed___eo_prog_str_update_neg_impl
    (t n r P : Term)
    ...
    (hProgEq : __eo_prog_str_update_neg t n r (Proof.pf P) = updateNegConclusion t n r) :
    RuleProofs.eo_has_bool_type (__eo_prog_str_update_neg t n r (Proof.pf P)) := by
  ...
  have hu : __smtx_typeof (__eo_to_smt (updateNegLhs t n r)) =
  ...
  rw [hProgEq]
  exact RuleProofs.eo_has_bool_type_eq_of_same_smt_type (updateNegLhs t n r) t
...
private theorem facts___eo_prog_str_update_neg_impl
    ...
    (hPrem : eo_interprets M (updateNegPremise n) true)
    (hProgEq :
      __eo_prog_str_update_neg t n r (Proof.pf P) =
        updateNegConclusion t n r) :
    eo_interprets M
      (__eo_prog_str_update_neg t n r (Proof.pf P)) true := by
  rcases hTy with ⟨T, hTTy, hNTy, hRTy⟩
  let lhs := updateNegLhs t n r
  let rhs := t
  have hBoolEq :
      RuleProofs.eo_has_bool_type (Term.Apply (Term.Apply Term.eq lhs) rhs) := by
    simpa [hProgEq, updateNegConclusion, lhs, rhs] using
      typed___eo_prog_str_update_neg_impl t n r P
  ...
  have hEvalEq :
    ...
    simp [__smtx_model_eval_str_update, native_seq_update, hNNeg,
...
public theorem cmd_step_str_update_neg_properties
    (M : SmtModel) (hM : model_wf M)
    (r : CState) (args : CArgList) (premises : CIndexList) :
  cmdTranslationOk (CCmd.step CRule.str_update_neg args premises) ->
  AllHaveBoolType (premiseTermList r premises) ->
  __eo_typeof (__eo_cmd_step_proven r CRule.str_update_neg args premises) = Term.Bool ->
  StepRuleProperties M (premiseTermList r premises)
    (__eo_cmd_step_proven r CRule.str_update_neg args premises) :=
by
  intro hCmdTrans _hPremisesBool hResultTy
  have hProg : __eo_cmd_step_proven r CRule.str_update_neg args premises ≠
      Term.Stuck :=
                      ...
                      | nil =>
                          ...
                          have hA3Trans : RuleProofs.eo_has_smt_translation a3 := by
                            simpa [cmdTranslationOk, cArgListTranslationOk] using
                              hCmdTrans.2.2.1
                          change __eo_typeof
                              (__eo_prog_str_update_neg a1 a2 a3 (Proof.pf P)) =
                            Term.Bool at hResultTy
                          have hProgRule :
                              __eo_prog_str_update_neg a1 a2 a3 (Proof.pf P) ≠
                                Term.Stuck :=
                            term_ne_stuck_of_typeof_bool hResultTy
                          rcases prog_str_update_neg_info a1 a2 a3 P hProgRule with
                            ⟨n0, hPremShape, hn0, hProgEq⟩
                          subst n0
                          let lhs := updateNegLhs a1 a2 a3
                          let rhs := a1
                          ...
                          · intro hTrue
                            ...
                            have hPrem :
                                eo_interprets M (updateNegPremise a2) true := by
                              simpa [hPremShape] using hPremRaw
                            exact facts___eo_prog_str_update_neg_impl
                              M hM a1 a2 a3 P hA1Trans hA2Trans hA3Trans
                              hArgTypes hPrem hProgEq
                          · exact RuleProofs.eo_has_smt_translation_of_has_bool_type _
                              (typed___eo_prog_str_update_neg_impl
