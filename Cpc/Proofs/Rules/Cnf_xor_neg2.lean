...
public theorem cmd_step_cnf_xor_neg2_properties
    (M : SmtModel) (hM : model_wf M)
    (s : CState) (args : CArgList) (premises : CIndexList) :
  cmdTranslationOk (CCmd.step CRule.cnf_xor_neg2 args premises) ->
  AllHaveBoolType (premiseTermList s premises) ->
  __eo_typeof (__eo_cmd_step_proven s CRule.cnf_xor_neg2 args premises) = Term.Bool ->
  StepRuleProperties M (premiseTermList s premises)
    (__eo_cmd_step_proven s CRule.cnf_xor_neg2 args premises) :=
by
  intro hCmdTrans _hPremisesBool hResultTy
  have hProg : __eo_cmd_step_proven s CRule.cnf_xor_neg2 args premises ≠ Term.Stuck :=
  ...
  | cons F args =>
      ...
      | nil =>
          cases premises with
          | nil =>
              change __eo_prog_cnf_xor_neg2 F ≠ Term.Stuck at hProg
              change __eo_typeof (__eo_prog_cnf_xor_neg2 F) = Term.Bool at hResultTy
              cases F with
              | Apply f F2 =>
                  cases f with
                  | Apply op F1 =>
                                ...
                                · rcases CnfSupport.eo_interprets_bool_cases M hM F2 hF2Bool with
                                  ...
                                  · have hXorTrue :
                                        eo_interprets M
                                          (Term.Apply (Term.Apply (Term.UOp UserOp.xor) F1) F2)
                                          true :=
                                      CnfSupport.eo_interprets_xor_true_of_false_true M F1 F2
                                        hF1False hF2True
                                    simpa [__eo_prog_cnf_xor_neg2] using
                                      CnfSupport.clause3_left_true M hM
                                        (Term.Apply (Term.Apply (Term.UOp UserOp.xor) F1) F2)
                                        F1 (Term.Apply (Term.UOp UserOp.not) F2)
                                        hXorTrue hF1Bool hNotF2Bool
                                  · simpa [__eo_prog_cnf_xor_neg2] using
