...
private abbrev nempSrcEmptyPremise (emp : Term) : Term :=
  Term.Apply
    (Term.Apply Term.eq (Term.Apply Term.str_len emp))
    (Term.Numeral 0)

private abbrev nempSrcNonemptyPremise (z emp : Term) : Term :=
  Term.Apply (Term.Apply Term.eq (Term.Apply (Term.Apply Term.eq z) emp))
    (Term.Boolean false)

private abbrev nempSrcValue (x y emp : Term) : Term :=
  Term.Apply (Term.Apply (Term.Apply Term.str_replace emp) x) y

private abbrev nempSrcLhs (x y z emp : Term) : Term :=
  Term.Apply (Term.Apply Term.eq (nempSrcValue x y emp)) z

private abbrev nempSrcRhs (x y z emp : Term) : Term :=
  Term.Apply
    (Term.Apply Term.and (Term.Apply (Term.Apply Term.eq x) emp))
    (Term.Apply
      (Term.Apply Term.and (Term.Apply (Term.Apply Term.eq y) z))
      (Term.Boolean true))

private abbrev nempSrcConclusion (x y z emp : Term) : Term :=
  Term.Apply (Term.Apply Term.eq (nempSrcLhs x y z emp))
    (nempSrcRhs x y z emp)

private theorem prog_str_eq_repl_nemp_src_emp_info
    (x y z emp P1 P2 : Term)
    (hProg :
      __eo_prog_str_eq_repl_nemp_src_emp x y z emp
          (Proof.pf P1) (Proof.pf P2) ≠
        Term.Stuck) :
    ∃ emp0 z0 emp1,
      P1 = nempSrcEmptyPremise emp0 ∧
      P2 = nempSrcNonemptyPremise z0 emp1 ∧
      emp0 = emp ∧ z0 = z ∧ emp1 = emp ∧
      __eo_prog_str_eq_repl_nemp_src_emp x y z emp
          (Proof.pf P1) (Proof.pf P2) =
        nempSrcConclusion x y z emp := by
  unfold __eo_prog_str_eq_repl_nemp_src_emp at hProg
  ...
  next heq1 heq2 =>
    ...
    simp [__eo_prog_str_eq_repl_nemp_src_emp, __eo_requires, __eo_eq,
      __eo_and, SmtEval.native_ite, native_teq, native_and,
      SmtEval.native_not, nempSrcConclusion, nempSrcLhs, nempSrcRhs,
      nempSrcValue]

private theorem typed___eo_prog_str_eq_repl_nemp_src_emp_impl
    (x y z emp P1 P2 : Term)
    ...
    (hProgEq :
      __eo_prog_str_eq_repl_nemp_src_emp x y z emp
          (Proof.pf P1) (Proof.pf P2) =
        nempSrcConclusion x y z emp) :
    RuleProofs.eo_has_bool_type
      (__eo_prog_str_eq_repl_nemp_src_emp x y z emp
        (Proof.pf P1) (Proof.pf P2)) := by
  let value := nempSrcValue x y emp
  let lhs := nempSrcLhs x y z emp
  let eqXEmp := Term.Apply (Term.Apply Term.eq x) emp
  let eqYZ := Term.Apply (Term.Apply Term.eq y) z
  let innerAnd := Term.Apply (Term.Apply Term.and eqYZ) (Term.Boolean true)
  let rhs := nempSrcRhs x y z emp
  ...
  have hValueTy :
    ...
    change __smtx_typeof
        (SmtTerm.str_replace (__eo_to_smt emp) (__eo_to_smt x)
          (__eo_to_smt y)) = SmtType.Seq (__eo_to_smt_type T)
    rw [typeof_str_replace_eq]
    simp [hEmpSmtTy, hXSmtTy, hYSmtTy, __smtx_typeof_seq_op_3,
      native_ite, native_Teq]
  have hLhsTy : __smtx_typeof (__eo_to_smt lhs) = SmtType.Bool :=
    RuleProofs.eo_has_bool_type_eq_of_same_smt_type value z
      (by rw [hValueTy, hZSmtTy]) (by rw [hValueTy]; simp)
  ...
  have hEqYZTy : __smtx_typeof (__eo_to_smt eqYZ) = SmtType.Bool :=
    RuleProofs.eo_has_bool_type_eq_of_same_smt_type y z
      (by rw [hYSmtTy, hZSmtTy]) (by rw [hYSmtTy]; simp)
  have hInnerAndTy :
...
private theorem facts___eo_prog_str_eq_repl_nemp_src_emp_impl
    ...
    (hZTy : __eo_typeof z = Term.Apply Term.Seq T)
    (hEmpTy : __eo_typeof emp = Term.Apply Term.Seq T)
    (hPrem1 : eo_interprets M (nempSrcEmptyPremise emp) true)
    (hPrem2 : eo_interprets M (nempSrcNonemptyPremise z emp) true)
    (hProgEq :
      __eo_prog_str_eq_repl_nemp_src_emp x y z emp
          (Proof.pf P1) (Proof.pf P2) =
        nempSrcConclusion x y z emp) :
  ...
  have hBool : RuleProofs.eo_has_bool_type
      (nempSrcConclusion x y z emp) := by
    simpa [hProgEq] using
      typed___eo_prog_str_eq_repl_nemp_src_emp_impl x y z emp P1 P2
  ...
  have hValueIff :
      native_pack_seq (__smtx_elem_typeof_seq_value semp)
          (native_seq_replace (native_unpack_seq semp) (native_unpack_seq sx)
            (native_unpack_seq sy)) = sz ↔
        sx = semp ∧ sy = sz := by
    constructor
    · intro hValue
      ...
      rcases (native_seq_replace_empty_src_eq_iff _ _ _ hZNonempty).mp
          hReplace with ⟨hXNil, hYZ⟩
      refine ⟨?_, ?_⟩
      · exact seq_eq_of_unpack_eq_of_elem_eq sx semp
          (by rw [hSxElem, hSempElem]) (by rw [hXNil, hEmpNil])
      · exact seq_eq_of_unpack_eq_of_elem_eq sy sz
          (by rw [hSyElem, hSzElem]) hYZ
    · rintro ⟨hXEmp, hYZ⟩
      have hXNil : native_unpack_seq sx = [] := by
        have := congrArg native_unpack_seq hXEmp
        simpa [hEmpNil] using this
      have hYZEval : native_unpack_seq sy = native_unpack_seq sz :=
        congrArg native_unpack_seq hYZ
      have hReplace :
          native_seq_replace [] (native_unpack_seq sx)
              (native_unpack_seq sy) = native_unpack_seq sz :=
        (native_seq_replace_empty_src_eq_iff _ _ _ hZNonempty).2
          ⟨hXNil, hYZEval⟩
      rw [hEmpNil, hReplace, hSempElem]
      simpa [hSzElem] using native_pack_unpack_seq sz
  have hEvalEq :
    ...
    change __smtx_model_eval M
      ...
      __smtx_model_eval M
        (SmtTerm.and (SmtTerm.eq (__eo_to_smt x) (__eo_to_smt emp))
          (SmtTerm.and (SmtTerm.eq (__eo_to_smt y) (__eo_to_smt z))
    ...
    rw [smtx_eval_eq_term_eq, smtx_eval_str_replace_term_eq,
      ...
      smtx_eval_eq_term_eq, hEmpEval, hXEval, hYEval, hZEval,
      smtx_eval_boolean_term_eq]
    simp [__smtx_model_eval_str_replace, __smtx_model_eval_eq,
      __smtx_model_eval_and, native_veq, native_and, hValueIff]
  rw [hProgEq]
...
public theorem cmd_step_str_eq_repl_nemp_src_emp_properties
    (M : SmtModel) (hM : model_wf M)
    (s : CState) (args : CArgList) (premises : CIndexList) :
  cmdTranslationOk (CCmd.step CRule.str_eq_repl_nemp_src_emp args premises) ->
  AllHaveBoolType (premiseTermList s premises) ->
  __eo_typeof (__eo_cmd_step_proven s CRule.str_eq_repl_nemp_src_emp args premises) = Term.Bool ->
  StepRuleProperties M (premiseTermList s premises)
    (__eo_cmd_step_proven s CRule.str_eq_repl_nemp_src_emp args premises) :=
by
                              ...
                              | nil =>
                                  ...
                                  have hA4Trans :
                                    ...
                                    simpa [cmdTranslationOk,
                                      cArgListTranslationOk] using
                                        hCmdTrans.2.2.2.1
                                  change __eo_typeof
                                      (__eo_prog_str_eq_repl_nemp_src_emp
                                  ...
                                  have hRuleProg :
                                      __eo_prog_str_eq_repl_nemp_src_emp
                                        ...
                                        Term.Stuck :=
                                    term_ne_stuck_of_typeof_bool hResultTy
                                  rcases prog_str_eq_repl_nemp_src_emp_info
                                  ...
                                  have hOuterLeftTy :
                                      __eo_typeof
                                          (nempSrcLhs a1 a2 a3 a4) ≠
                                        Term.Stuck := by
                                    change __eo_typeof_eq
                                        (__eo_typeof
                                          (nempSrcLhs a1 a2 a3 a4))
                                        (__eo_typeof
                                          (nempSrcRhs a1 a2 a3 a4)) =
                                      Term.Bool at hResultTy
                                    exact
                                  ...
                                  rcases
                                      eo_typeof_str_replace_args_of_ne_stuck
                                        (__eo_typeof a4) (__eo_typeof a1)
                                        (__eo_typeof a2) hValueTy with
                                    ⟨T, hA4Ty, hA1Ty, hA2Ty⟩
                                  have hValueTyEq :
                                      __eo_typeof
                                          (nempSrcValue a1 a2 a4) =
                                    ...
                                    rw [hA4Ty, hA1Ty, hA2Ty] at hValueNN
                                    rw [hA4Ty, hA1Ty, hA2Ty]
                                    exact
                                  ...
                                  · intro hTrue
                                    ...
                                    have hPrem1 :
                                        eo_interprets M
                                          (nempSrcEmptyPremise a4) true := by
                                      simpa [hP1] using hPrem1Raw
                                    have hPrem2 :
                                        eo_interprets M
                                          (nempSrcNonemptyPremise a3 a4)
                                          true := by
                                      simpa [hP2] using hPrem2Raw
                                    exact
                                      facts___eo_prog_str_eq_repl_nemp_src_emp_impl
                                  ...
                                  · exact
                                      RuleProofs.eo_has_smt_translation_of_has_bool_type
                                        _
                                        (typed___eo_prog_str_eq_repl_nemp_src_emp_impl
