...
public theorem cmd_step_cnf_xor_pos1_properties
    (M : SmtModel) (hM : model_wf M)
    (s : CState) (args : CArgList) (premises : CIndexList) :
  cmdTranslationOk (CCmd.step CRule.cnf_xor_pos1 args premises) ->
  AllHaveBoolType (premiseTermList s premises) ->
  __eo_typeof (__eo_cmd_step_proven s CRule.cnf_xor_pos1 args premises) = Term.Bool ->
  StepRuleProperties M (premiseTermList s premises)
    (__eo_cmd_step_proven s CRule.cnf_xor_pos1 args premises) :=
by
  intro hCmdTrans _hPremisesBool hResultTy
  have hProg : __eo_cmd_step_proven s CRule.cnf_xor_pos1 args premises ≠ Term.Stuck :=
  ...
  | cons F args =>
      ...
      | nil =>
          cases premises with
          | nil =>
              change __eo_prog_cnf_xor_pos1 F ≠ Term.Stuck at hProg
              change __eo_typeof (__eo_prog_cnf_xor_pos1 F) = Term.Bool at hResultTy
              cases F with
              | Apply f F2 =>
                  cases f with
                  | Apply op F1 =>
                                ...
                                · rcases CnfSupport.eo_interprets_bool_cases M hM F2 hF2Bool with
                                  ...
                                  · simpa [__eo_prog_cnf_xor_pos1] using
                                      CnfSupport.clause3_right_true M hM
                                        (Term.Apply (Term.UOp UserOp.not)
                                          (Term.Apply (Term.Apply (Term.UOp UserOp.xor) F1) F2))
                                        F1 F2 hNotXorBool hF1Bool hF2True
                                  · have hXorFalse :
                                        eo_interprets M
                                          (Term.Apply (Term.Apply (Term.UOp UserOp.xor) F1) F2)
                                          false :=
                                      CnfSupport.eo_interprets_xor_false_of_false_false M F1 F2
                                        hF1False hF2False
                                    simpa [__eo_prog_cnf_xor_pos1] using
