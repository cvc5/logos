...
private abbrev replDualIte1Premise (x z : Term) : Term :=
  Term.Apply
    (Term.Apply Term.eq (Term.Apply (Term.Apply Term.str_contains x) z))
    (Term.Boolean false)

private abbrev replDualIte1Inner (x y z : Term) : Term :=
  Term.Apply (Term.Apply (Term.Apply Term.str_replace x) y) z

private abbrev replDualIte1Lhs (x y z w : Term) : Term :=
  Term.Apply
    (Term.Apply (Term.Apply Term.str_replace x)
      (replDualIte1Inner x y z)) w

private abbrev replDualIte1Rhs (x y w : Term) : Term :=
  Term.Apply
    (Term.Apply
      (Term.Apply Term.ite
        (Term.Apply (Term.Apply Term.str_contains x) y)) x) w

private abbrev replDualIte1Conclusion (x y z w : Term) : Term :=
  Term.Apply (Term.Apply Term.eq (replDualIte1Lhs x y z w))
    (replDualIte1Rhs x y w)

private theorem prog_str_repl_repl_dual_ite1_info
    (x y z w P : Term)
    (hProg :
      __eo_prog_str_repl_repl_dual_ite1 x y z w (Proof.pf P) ≠
        Term.Stuck) :
    ∃ x0 z0,
      P = replDualIte1Premise x0 z0 ∧ x0 = x ∧ z0 = z ∧
      __eo_prog_str_repl_repl_dual_ite1 x y z w (Proof.pf P) =
        replDualIte1Conclusion x y z w := by
  unfold __eo_prog_str_repl_repl_dual_ite1 at hProg
  ...
  next heq =>
    ...
    simp [__eo_prog_str_repl_repl_dual_ite1, __eo_requires,
      __eo_eq, __eo_and, SmtEval.native_ite, native_teq, native_and,
      SmtEval.native_not, replDualIte1Conclusion, replDualIte1Lhs,
      replDualIte1Rhs, replDualIte1Inner]

private theorem typed___eo_prog_str_repl_repl_dual_ite1_impl
    (x y z w P : Term)
    ...
    (hProgEq :
      __eo_prog_str_repl_repl_dual_ite1 x y z w (Proof.pf P) =
        replDualIte1Conclusion x y z w) :
    RuleProofs.eo_has_bool_type
      (__eo_prog_str_repl_repl_dual_ite1 x y z w (Proof.pf P)) := by
  let lhs := replDualIte1Lhs x y z w
  let rhs := replDualIte1Rhs x y w
...
private theorem facts___eo_prog_str_repl_repl_dual_ite1_impl
    ...
    (hZTy : __eo_typeof z = Term.Apply Term.Seq T)
    (hWTy : __eo_typeof w = Term.Apply Term.Seq T)
    (hPrem : eo_interprets M (replDualIte1Premise x z) true)
    (hProgEq :
      __eo_prog_str_repl_repl_dual_ite1 x y z w (Proof.pf P) =
        replDualIte1Conclusion x y z w) :
    eo_interprets M
      (__eo_prog_str_repl_repl_dual_ite1 x y z w (Proof.pf P)) true := by
  let lhs := replDualIte1Lhs x y z w
  let rhs := replDualIte1Rhs x y w
  have hBool : RuleProofs.eo_has_bool_type
      (replDualIte1Conclusion x y z w) := by
    simpa [hProgEq] using
      typed___eo_prog_str_repl_repl_dual_ite1_impl
  ...
  have hEvalEq :
    ...
    · simp [__smtx_model_eval_str_replace, __smtx_model_eval_str_contains,
        __smtx_model_eval_ite, Smtm.native_unpack_pack_seq, hContains,
        native_seq_replace_dual_ite1_of_contains_true _ _ _ _
          hAbsent hContains,
...
public theorem cmd_step_str_repl_repl_dual_ite1_properties
    (M : SmtModel) (hM : model_wf M)
    (s : CState) (args : CArgList) (premises : CIndexList) :
  cmdTranslationOk (CCmd.step CRule.str_repl_repl_dual_ite1 args premises) ->
  AllHaveBoolType (premiseTermList s premises) ->
  __eo_typeof (__eo_cmd_step_proven s CRule.str_repl_repl_dual_ite1 args premises) = Term.Bool ->
  StepRuleProperties M (premiseTermList s premises)
    (__eo_cmd_step_proven s CRule.str_repl_repl_dual_ite1 args premises) :=
by
                          ...
                          | nil =>
                              ...
                              have hA4Trans :
                                ...
                                simpa [cmdTranslationOk,
                                  cArgListTranslationOk] using
                                    hCmdTrans.2.2.2.1
                              change __eo_typeof
                                  (__eo_prog_str_repl_repl_dual_ite1
                              ...
                              have hRuleProg :
                                  __eo_prog_str_repl_repl_dual_ite1
                                    ...
                                    Term.Stuck :=
                                term_ne_stuck_of_typeof_bool hResultTy
                              rcases prog_str_repl_repl_dual_ite1_info
                              ...
                              have hLhsNN :
                                  __eo_typeof
                                      (replDualIte1Lhs a1 a2 a3 a4) ≠
                                    Term.Stuck := by
                                change __eo_typeof_eq
                                    (__eo_typeof
                                      (replDualIte1Lhs a1 a2 a3 a4))
                                    (__eo_typeof
                                      (replDualIte1Rhs a1 a2 a4)) =
                                  Term.Bool at hResultTy
                                exact
                              ...
                              · intro hTrue
                                ...
                                have hPrem :
                                    eo_interprets M
                                      (replDualIte1Premise a1 a3) true := by
                                  simpa [hP] using hPremRaw
                                exact
                                  facts___eo_prog_str_repl_repl_dual_ite1_impl
