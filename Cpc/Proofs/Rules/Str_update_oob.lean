...
private abbrev updateOobConclusion (t n r : Term) : Term :=
  Term.Apply
    (Term.Apply Term.eq (updateOobLhs t n r))
    t
...
private theorem prog_str_update_oob_info
    (t n r P : Term)
    (hProg : __eo_prog_str_update_oob t n r (Proof.pf P) ≠
      Term.Stuck) :
    ∃ t0 n0,
      P = updateOobPremise t0 n0 ∧
      t0 = t ∧
      n0 = n ∧
      __eo_prog_str_update_oob t n r (Proof.pf P) =
        updateOobConclusion t n r := by
  unfold __eo_prog_str_update_oob at hProg
  ...
  next heq =>
    ...
    simp [__eo_prog_str_update_oob, __eo_requires, __eo_eq,
      __eo_and, SmtEval.native_ite, native_teq, native_and,
      SmtEval.native_not, updateOobConclusion, updateOobLhs]

private theorem typed___eo_prog_str_update_oob_impl
    (t n r P : Term)
    ...
    (hProgEq : __eo_prog_str_update_oob t n r (Proof.pf P) = updateOobConclusion t n r) :
    RuleProofs.eo_has_bool_type (__eo_prog_str_update_oob t n r (Proof.pf P)) := by
  ...
  have hu : __smtx_typeof (__eo_to_smt (updateOobLhs t n r)) =
  ...
  rw [hProgEq]
  exact RuleProofs.eo_has_bool_type_eq_of_same_smt_type (updateOobLhs t n r) t
...
private theorem facts___eo_prog_str_update_oob_impl
    ...
    (hPrem : eo_interprets M (updateOobPremise t n) true)
    (hProgEq :
      __eo_prog_str_update_oob t n r (Proof.pf P) =
        updateOobConclusion t n r) :
    eo_interprets M
      (__eo_prog_str_update_oob t n r (Proof.pf P)) true := by
  rcases hTy with ⟨T, hTTy, hNTy, hRTy⟩
  let lhs := updateOobLhs t n r
  let rhs := t
  have hBoolEq :
      RuleProofs.eo_has_bool_type (Term.Apply (Term.Apply Term.eq lhs) rhs) := by
    simpa [hProgEq, updateOobConclusion, lhs, rhs] using
      typed___eo_prog_str_update_oob_impl t n r P
...
public theorem cmd_step_str_update_oob_properties
    (M : SmtModel) (hM : model_wf M)
    (r : CState) (args : CArgList) (premises : CIndexList) :
  cmdTranslationOk (CCmd.step CRule.str_update_oob args premises) ->
  AllHaveBoolType (premiseTermList r premises) ->
  __eo_typeof (__eo_cmd_step_proven r CRule.str_update_oob args premises) = Term.Bool ->
  StepRuleProperties M (premiseTermList r premises)
    (__eo_cmd_step_proven r CRule.str_update_oob args premises) :=
by
  intro hCmdTrans _hPremisesBool hResultTy
  have hProg : __eo_cmd_step_proven r CRule.str_update_oob args premises ≠
      Term.Stuck :=
                      ...
                      | nil =>
                          ...
                          have hA3Trans : RuleProofs.eo_has_smt_translation a3 := by
                            simpa [cmdTranslationOk, cArgListTranslationOk] using
                              hCmdTrans.2.2.1
                          change __eo_typeof
                              (__eo_prog_str_update_oob a1 a2 a3 (Proof.pf P)) =
                            Term.Bool at hResultTy
                          have hProgRule :
                              __eo_prog_str_update_oob a1 a2 a3 (Proof.pf P) ≠
                                Term.Stuck :=
                            term_ne_stuck_of_typeof_bool hResultTy
                          rcases prog_str_update_oob_info a1 a2 a3 P hProgRule with
                            ⟨t0, n0, hPremShape, ht0, hn0, hProgEq⟩
                          subst t0
                          subst n0
                          let lhs := updateOobLhs a1 a2 a3
                          let rhs := a1
                          ...
                          · intro hTrue
                            ...
                            have hPrem :
                                eo_interprets M (updateOobPremise a1 a2) true := by
                              simpa [hPremShape] using hPremRaw
                            exact facts___eo_prog_str_update_oob_impl
                              M hM a1 a2 a3 P hA1Trans hA2Trans hA3Trans
                              hArgTypes hPrem hProgEq
                          · exact RuleProofs.eo_has_smt_translation_of_has_bool_type _
                              (typed___eo_prog_str_update_oob_impl
