...
private theorem prog_str_to_lower_from_int_eq_of_ne_stuck (n1 : Term) :
    n1 ≠ Term.Stuck ->
    __eo_prog_str_to_lower_from_int n1 =
      Term.Apply (Term.Apply Term.eq
        (Term.Apply (Term.UOp UserOp.str_to_lower)
          (Term.Apply (Term.UOp UserOp.str_from_int) n1)))
        (Term.Apply (Term.UOp UserOp.str_from_int) n1) := by
  intro hN1
  cases n1 <;> simp [__eo_prog_str_to_lower_from_int] at hN1 ⊢

private theorem eo_typeof_str_to_lower_str_from_int_arg_int_of_ne_stuck
    (T : Term)
...
private theorem typeof_arg_of_prog_str_to_lower_from_int_bool (n1 : Term) :
    __eo_typeof (__eo_prog_str_to_lower_from_int n1) = Term.Bool ->
  ...
  · rw [prog_str_to_lower_from_int_eq_of_ne_stuck n1 hN1] at hTy
    ...
    exact eo_typeof_str_to_lower_str_from_int_arg_int_of_ne_stuck
...
private theorem native_char_to_lower_digitChar_of_lt_10
    {d : Nat} (hd : d < 10) :
    impl_native_char_to_lower (Char.toNat (Nat.digitChar d)) =
      Char.toNat (Nat.digitChar d) := by
  unfold impl_native_char_to_lower Nat.digitChar
  by_cases h0 : d = 0
  · simp [h0]
  · by_cases h1 : d = 1
    · simp [h1]
...
private theorem map_native_char_to_lower_toDigitsCore :
    ∀ fuel n ds,
      List.map impl_native_char_to_lower (ds.map Char.toNat) = ds.map Char.toNat ->
        List.map impl_native_char_to_lower
  ...
  | fuel + 1, n, ds, hds => by
      ...
      have hDigit :
          impl_native_char_to_lower (Char.toNat (Nat.digitChar (n % 10))) =
...
private theorem native_str_to_lower_nat_toString (n : Nat) :
    native_str_to_lower (native_string_lit (toString n)) =
      native_string_lit (toString n) := by
  unfold native_str_to_lower native_string_lit
  rw [show (toString n).toList = Nat.toDigits 10 n by
    rw [show (toString n) = n.repr by rfl]
    unfold Nat.repr
    rw [String.toList_ofList]]
  rw [Nat.toDigits.eq_1]
  exact map_native_char_to_lower_toDigitsCore (n + 1) n [] (by simp)

private theorem native_str_to_lower_from_int (i : native_Int) :
    native_str_to_lower (native_str_from_int i) = native_str_from_int i := by
  cases i with
  | ofNat n =>
...
private theorem typed___eo_prog_str_to_lower_from_int_impl
    ...
    RuleProofs.eo_has_bool_type (__eo_prog_str_to_lower_from_int n1) := by
  let rhs := Term.Apply (Term.UOp UserOp.str_from_int) n1
  let lhs := Term.Apply (Term.UOp UserOp.str_to_lower) rhs
  ...
  have hLhsTy : __smtx_typeof (__eo_to_smt lhs) = SmtType.Seq SmtType.Char := by
    ...
    change __smtx_typeof
      ...
      SmtType.Seq SmtType.Char
    rw [typeof_str_to_lower_eq, hRhsTy']
    simp [native_ite, native_Teq]
  have hBoolEq :
  ...
  rw [prog_str_to_lower_from_int_eq_of_ne_stuck n1 hNNotStuck]
  exact hBoolEq

private theorem facts___eo_prog_str_to_lower_from_int_impl
    (M : SmtModel) (hM : model_wf M) (n1 : Term)
    (hNTrans : RuleProofs.eo_has_smt_translation n1)
    (hNTy : __eo_typeof n1 = Term.UOp UserOp.Int) :
    eo_interprets M (__eo_prog_str_to_lower_from_int n1) true := by
  let rhs := Term.Apply (Term.UOp UserOp.str_from_int) n1
  let lhs := Term.Apply (Term.UOp UserOp.str_to_lower) rhs
  have hNNotStuck : n1 ≠ Term.Stuck :=
    RuleProofs.term_ne_stuck_of_has_smt_translation n1 hNTrans
  have hProg :
    ...
    simpa [lhs, rhs] using
      prog_str_to_lower_from_int_eq_of_ne_stuck n1 hNNotStuck
  have hBoolEq :
      RuleProofs.eo_has_bool_type (Term.Apply (Term.Apply Term.eq lhs) rhs) := by
    simpa [hProg] using
      typed___eo_prog_str_to_lower_from_int_impl n1 hNTrans hNTy
  have hNSmtTy : __smtx_typeof (__eo_to_smt n1) = SmtType.Int :=
    smtx_typeof_of_eo_int n1 hNTrans hNTy
  have hNEvalTy :
  ...
  have hEvalEq :
    ...
    change __smtx_model_eval M
        (SmtTerm.str_to_lower (SmtTerm.str_from_int (__eo_to_smt n1))) =
      __smtx_model_eval M (SmtTerm.str_from_int (__eo_to_smt n1))
    rw [smtx_eval_str_to_lower_term_eq, hFromEval]
    simp [__smtx_model_eval_str_to_lower, native_unpack_string_pack_string,
      native_str_to_lower_from_int]
  rw [hProg]
  exact RuleProofs.eo_interprets_eq_of_rel M lhs rhs hBoolEq <| by
    rw [hEvalEq]
    exact RuleProofs.smt_value_rel_refl (__smtx_model_eval M (__eo_to_smt rhs))

public theorem cmd_step_str_to_lower_from_int_properties
...
by
  ...
  have hProg : __eo_cmd_step_proven s CRule.str_to_lower_from_int args premises ≠
      Term.Stuck :=
  ...
  | cons a1 args =>
      ...
      | nil =>
          ...
          | nil =>
              ...
              change __eo_typeof (__eo_prog_str_to_lower_from_int a1) = Term.Bool
                at hResultTy
              have hA1Ty : __eo_typeof a1 = Term.UOp UserOp.Int :=
                typeof_arg_of_prog_str_to_lower_from_int_bool a1 hResultTy
              have hProps :
                  StepRuleProperties M (premiseTermList s CIndexList.nil)
                    (__eo_prog_str_to_lower_from_int a1) := by
                refine ⟨?_, RuleProofs.eo_has_smt_translation_of_has_bool_type _
                  (typed___eo_prog_str_to_lower_from_int_impl a1 hA1Trans hA1Ty)⟩
                intro _hTrue
                exact facts___eo_prog_str_to_lower_from_int_impl M hM a1
                  hA1Trans hA1Ty
              change StepRuleProperties M [] (__eo_prog_str_to_lower_from_int a1)
