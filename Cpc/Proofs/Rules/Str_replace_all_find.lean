...
private abbrev after (t pat : Term) := plus (idx t pat) (plus (len pat) (Term.Numeral 0))
private abbrev indexPremise (t pat : Term) := eq (Term.Apply (Term.Apply Term.geq (idx t pat)) (Term.Numeral 0)) (Term.Boolean true)
...
private abbrev rhs (pat repl pre post : Term) := mkConcat pre (mkConcat repl (mkConcat (replace post pat repl) (nilFor pre)))
private abbrev conclusion (t pat repl pre post : Term) := eq (replace t pat repl) (rhs pat repl pre post)

private theorem program_info (t pat repl pre post P Q R S : Term)
    (h : __eo_prog_str_replace_all_find t pat repl pre post (Proof.pf P) (Proof.pf Q) (Proof.pf R) (Proof.pf S) ≠ Term.Stuck) :
    P = indexPremise t pat ∧ Q = positivePremise pat ∧ R = prePremise t pat pre ∧ S = postPremise t pat post ∧
    __eo_prog_str_replace_all_find t pat repl pre post (Proof.pf P) (Proof.pf Q) (Proof.pf R) (Proof.pf S) = conclusion t pat repl pre post := by
  unfold __eo_prog_str_replace_all_find at h
  ...
  next hp hq hr hs =>
    ...
    cases hr
    cases hs
    have hg13 := support_eo_requires_cond_eq_of_non_stuck h
    have hg12 := StrEqReplSupport.eo_and_eq_true_left hg13
    have hg11 := StrEqReplSupport.eo_and_eq_true_left hg12
    have hg10 := StrEqReplSupport.eo_and_eq_true_left hg11
    have hg9 := StrEqReplSupport.eo_and_eq_true_left hg10
    have hg8 := StrEqReplSupport.eo_and_eq_true_left hg9
    have hg7 := StrEqReplSupport.eo_and_eq_true_left hg8
    ...
    have he8 := support_eq_of_eo_eq_true _ _ (StrEqReplSupport.eo_and_eq_true_right hg8)
    have he9 := support_eq_of_eo_eq_true _ _ (StrEqReplSupport.eo_and_eq_true_right hg9)
    have he10 := support_eq_of_eo_eq_true _ _ (StrEqReplSupport.eo_and_eq_true_right hg10)
    have he11 := support_eq_of_eo_eq_true _ _ (StrEqReplSupport.eo_and_eq_true_right hg11)
    have he12 := support_eq_of_eo_eq_true _ _ (StrEqReplSupport.eo_and_eq_true_right hg12)
    have he13 := support_eq_of_eo_eq_true _ _ (StrEqReplSupport.eo_and_eq_true_right hg13)
    subst_vars
    ...
    have h3 := eo_mk_apply_arg_ne_stuck_of_ne_stuck _ _ h2
    refine ⟨rfl, rfl, rfl, rfl, ?_⟩
    simp only [__eo_prog_str_replace_all_find, __eo_requires, __eo_eq, __eo_and,
...
private theorem conclusion_typed (t pat repl pre post U : Term)
    ...
    (hpre : __smtx_typeof (__eo_to_smt pre) = SmtType.Seq (__eo_to_smt_type U))
    (hpost : __smtx_typeof (__eo_to_smt post) = SmtType.Seq (__eo_to_smt_type U))
    (hpreEO : __eo_typeof pre = Term.Apply Term.Seq U) :
    RuleProofs.eo_has_bool_type (conclusion t pat repl pre post) := by
  have hl := replace_type t pat repl _ ht hp hr
  have htail := concat_type (replace post pat repl) (nilFor pre) _
    (replace_type post pat repl _ hpost hp hr) (nil_type pre U hpreEO hpre)
  have hrhs := concat_type pre (mkConcat repl (mkConcat (replace post pat repl) (nilFor pre))) _ hpre
    (concat_type repl (mkConcat (replace post pat repl) (nilFor pre)) _ hr htail)
...
private theorem conclusion_facts (M : SmtModel) (hM : model_wf M) (t pat repl pre post T : Term)
    ...
    (hPreSmtTy : __smtx_typeof (__eo_to_smt pre) = SmtType.Seq (__eo_to_smt_type T))
    ...
    eo_interprets M (conclusion t pat repl pre post) true := by
  ...
  have hNativeReplace :
    ...
    rw [StrReplaceAllSupport.replace_all_step _ _ _ hNonempty hIndexNonneg, ← hPreList, ← hPostList]
  ...
  have hNil : nilFor pre = __seq_empty (__eo_typeof pre) := by rw [nilFor, hPreTy]; rfl
  have hEvalEq : __smtx_model_eval M (__eo_to_smt (replace t pat repl)) =
      __smtx_model_eval M (__eo_to_smt (rhs pat repl pre post)) := by
    unfold rhs
    rw [hNil]
    change __smtx_model_eval M (SmtTerm.str_replace_all (__eo_to_smt t) (__eo_to_smt pat) (__eo_to_smt repl)) =
      __smtx_model_eval M (SmtTerm.str_concat (__eo_to_smt pre) (SmtTerm.str_concat (__eo_to_smt repl)
    ...
    simp only [eval_replace, eval_concat, hTEval, hPatEval, hReplEval, hPreEval, hPostEval, hEmptyEval]
    simp [__smtx_model_eval_str_replace_all, __smtx_model_eval_str_concat,
      ...
      hTElem, hPatElem, hReplElem, hPreElem, hPostElem, elem_typeof_pack_seq, hNativeReplace, List.append_assoc]
  exact RuleProofs.eo_interprets_eq_of_rel M _ _
    (conclusion_typed t pat repl pre post T hTSmtTy hPatSmtTy hReplSmtTy hPreSmtTy hPostSmtTy hPreTy)
    (by rw [hEvalEq]; exact RuleProofs.smt_value_rel_refl _)

public theorem cmd_step_str_replace_all_find_properties
    (M : SmtModel) (hM : model_wf M)
    (s : CState) (args : CArgList) (premises : CIndexList) :
  cmdTranslationOk (CCmd.step CRule.str_replace_all_find args premises) ->
  AllHaveBoolType (premiseTermList s premises) ->
  __eo_typeof (__eo_cmd_step_proven s CRule.str_replace_all_find args premises) = Term.Bool ->
  StepRuleProperties M (premiseTermList s premises)
    (__eo_cmd_step_proven s CRule.str_replace_all_find args premises) := by
                  ...
                  | cons k premises =>
                    ...
                    | cons l premises =>
                      ...
                      | nil =>
                        ...
                        let R := __eo_state_proven_nth s k
                        let S := __eo_state_proven_nth s l
                        change __eo_typeof (__eo_prog_str_replace_all_find t pat repl pre post (Proof.pf P) (Proof.pf Q) (Proof.pf R) (Proof.pf S)) = Term.Bool at hTy
                        rcases program_info t pat repl pre post P Q R S (term_ne_stuck_of_typeof_bool hTy) with ⟨hP, hQ, hR, hS, he⟩
                        change StepRuleProperties M _ (__eo_prog_str_replace_all_find t pat repl pre post (Proof.pf P) (Proof.pf Q) (Proof.pf R) (Proof.pf S))
                        rw [he] at hTy ⊢
                        ...
                        have hTNe : T ≠ Term.Stuck := by
                          have hw := (smt_seq_component_wf_of_non_none_type (__eo_to_smt t) _ htS).2
                          intro hT
                          subst T
                          change false = true at hw
                          contradiction
                        have hIdxTy : __eo_typeof_str_indexof (Term.Apply Term.Seq T) (Term.Apply Term.Seq T) Term.Int = Term.Int := by
                          change __eo_requires (__eo_eq T T) (Term.Boolean true) Term.Int = Term.Int
                          rw [eo_eq_self_true_of_ne_stuck T hTNe]
                          rfl
                        have hRBool : RuleProofs.eo_has_bool_type (prePremise t pat pre) := by
                          have h := hPremTy R (by simp [R, premiseTermList]); simpa [hR] using h
                        ...
                        have hpre : __eo_typeof pre = Term.Apply Term.Seq T := by
                          have h := StrReplaceFindSupport.eo_typeof_eq_operands_of_has_bool_type _ _ hRBool
                          rw [h]
                          change __eo_typeof_str_substr (__eo_typeof t) Term.Int
                            (__eo_typeof_str_indexof (__eo_typeof t) (__eo_typeof pat) Term.Int) = _
                          rw [ht, hpat, hIdxTy]
                          rfl
                        have hpost : __eo_typeof post = Term.Apply Term.Seq T := by
                          ...
                          rw [h]
                          change __eo_typeof_str_substr (__eo_typeof t)
                            (__eo_typeof_plus (__eo_typeof_str_indexof (__eo_typeof t) (__eo_typeof pat) Term.Int)
                              (__eo_typeof_plus (__eo_typeof_str_len (__eo_typeof pat)) Term.Int))
                            (__eo_typeof_str_len (__eo_typeof t)) = _
                          rw [ht, hpat, hIdxTy]
                          rfl
                        have hpS := smtx_typeof_of_eo_seq pat T hCmd.2.1 hpat
                        have hrS := smtx_typeof_of_eo_seq repl T hCmd.2.2.1 hrepl
                        have hpreS := smtx_typeof_of_eo_seq pre T hCmd.2.2.2.1 hpre
                        have hpostS := smtx_typeof_of_eo_seq post T hCmd.2.2.2.2.1 hpost
                        refine ⟨?_, RuleProofs.eo_has_smt_translation_of_has_bool_type _ (conclusion_typed t pat repl pre post T htS hpS hrS hpreS hpostS hpre)⟩
                        intro hTrue
                        apply conclusion_facts M hM t pat repl pre post T htS hpS hrS hpreS hpostS hpre
                        · have h := hTrue P (by simp [P, premiseTermList]); simpa [hP] using h
                        · have h := hTrue Q (by simp [Q, premiseTermList]); simpa [hQ] using h
                        · have h := hTrue R (by simp [R, premiseTermList]); simpa [hR] using h
