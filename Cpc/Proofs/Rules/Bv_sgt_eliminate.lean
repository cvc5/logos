...
private theorem prog_bv_sgt_eliminate_eq_of_ne_stuck (x1 y1 : Term) :
    x1 ≠ Term.Stuck ->
    y1 ≠ Term.Stuck ->
    __eo_prog_bv_sgt_eliminate x1 y1 =
      Term.Apply (Term.Apply Term.eq
        (Term.Apply (Term.Apply (Term.UOp UserOp.bvsgt) x1) y1))
        (Term.Apply (Term.Apply (Term.UOp UserOp.bvslt) y1) x1) := by
  intro hX1 hY1
  cases x1 <;> cases y1 <;> simp [__eo_prog_bv_sgt_eliminate] at hX1 hY1 ⊢
...
private theorem typeof_args_of_prog_bv_sgt_eliminate_bool (x1 y1 : Term) :
  ...
  · by_cases hY1 : y1 = Term.Stuck
    · subst y1
      cases x1 <;> simp [__eo_prog_bv_sgt_eliminate] at hX1 hTy
      all_goals
        have hStuckTy : __eo_typeof Term.Stuck ≠ Term.Bool := by
          native_decide
        exact False.elim (hStuckTy hTy)
    · rw [prog_bv_sgt_eliminate_eq_of_ne_stuck x1 y1 hX1 hY1] at hTy
...
private theorem eo_has_bool_type_bvsgt_same
    (x1 y1 : Term) :
    RuleProofs.eo_has_smt_translation x1 ->
    RuleProofs.eo_has_smt_translation y1 ->
    __eo_typeof (__eo_prog_bv_sgt_eliminate x1 y1) = Term.Bool ->
    RuleProofs.eo_has_bool_type
      (Term.Apply (Term.Apply (Term.UOp UserOp.bvsgt) x1) y1) := by
  intro hX1Trans hY1Trans hResultTy
  rcases typeof_args_of_prog_bv_sgt_eliminate_bool x1 y1 hResultTy with
  ...
  change __smtx_typeof (SmtTerm.bvsgt (__eo_to_smt x1) (__eo_to_smt y1)) = SmtType.Bool
...
private theorem typed___eo_prog_bv_sgt_eliminate_impl (x1 y1 : Term) :
    RuleProofs.eo_has_smt_translation x1 ->
    RuleProofs.eo_has_smt_translation y1 ->
    __eo_typeof (__eo_prog_bv_sgt_eliminate x1 y1) = Term.Bool ->
    RuleProofs.eo_has_bool_type (__eo_prog_bv_sgt_eliminate x1 y1) := by
  intro hX1Trans hY1Trans hResultTy
  ...
  have hSgtBool :
      RuleProofs.eo_has_bool_type
        (Term.Apply (Term.Apply (Term.UOp UserOp.bvsgt) x1) y1) :=
    eo_has_bool_type_bvsgt_same x1 y1 hX1Trans hY1Trans hResultTy
  have hUltSwapBool :
      RuleProofs.eo_has_bool_type
        (Term.Apply (Term.Apply (Term.UOp UserOp.bvslt) y1) x1) :=
    eo_has_bool_type_bvslt_swap x1 y1 hX1Trans hY1Trans hResultTy
  rw [prog_bv_sgt_eliminate_eq_of_ne_stuck x1 y1 hX1NotStuck hY1NotStuck]
  exact RuleProofs.eo_has_bool_type_eq_of_same_smt_type
    (Term.Apply (Term.Apply (Term.UOp UserOp.bvsgt) x1) y1)
    (Term.Apply (Term.Apply (Term.UOp UserOp.bvslt) y1) x1)
    (hSgtBool.trans hUltSwapBool.symm)
    (by
      rw [hSgtBool]
      decide)
...
private theorem eval_bvsgt_matches_bvslt_swap
  ...
  change __smtx_model_eval M (SmtTerm.bvsgt (__eo_to_smt x1) (__eo_to_smt y1)) =
    __smtx_model_eval M (SmtTerm.bvslt (__eo_to_smt y1) (__eo_to_smt x1))
...
private theorem facts___eo_prog_bv_sgt_eliminate_impl
    ...
    RuleProofs.eo_has_smt_translation y1 ->
    __eo_typeof (__eo_prog_bv_sgt_eliminate x1 y1) = Term.Bool ->
    eo_interprets M (__eo_prog_bv_sgt_eliminate x1 y1) true := by
  intro hX1Trans hY1Trans hResultTy
  have hX1NotStuck : x1 ≠ Term.Stuck :=
    RuleProofs.term_ne_stuck_of_has_smt_translation x1 hX1Trans
  have hY1NotStuck : y1 ≠ Term.Stuck :=
    RuleProofs.term_ne_stuck_of_has_smt_translation y1 hY1Trans
  have hProgBool : RuleProofs.eo_has_bool_type (__eo_prog_bv_sgt_eliminate x1 y1) :=
    typed___eo_prog_bv_sgt_eliminate_impl x1 y1 hX1Trans hY1Trans hResultTy
  rw [prog_bv_sgt_eliminate_eq_of_ne_stuck x1 y1 hX1NotStuck hY1NotStuck]
  apply RuleProofs.eo_interprets_eq_of_rel M
  · simpa [prog_bv_sgt_eliminate_eq_of_ne_stuck x1 y1 hX1NotStuck hY1NotStuck] using
      hProgBool
  · change RuleProofs.smt_value_rel
      ...
      (__smtx_model_eval M
        (__eo_to_smt
          (Term.Apply (Term.Apply (Term.UOp UserOp.bvslt) y1) x1)))
    rw [eval_bvsgt_matches_bvslt_swap M hM x1 y1 hX1Trans hY1Trans hResultTy]
...
public theorem cmd_step_bv_sgt_eliminate_properties
  ...
  __eo_typeof (__eo_cmd_step_proven s CRule.bv_sgt_eliminate args premises) = Term.Bool ->
  StepRuleProperties M (premiseTermList s premises)
    (__eo_cmd_step_proven s CRule.bv_sgt_eliminate args premises) :=
by
  intro hCmdTrans _hPremisesBool hResultTy
  have hProg : __eo_cmd_step_proven s CRule.bv_sgt_eliminate args premises ≠ Term.Stuck :=
      ...
      | cons a2 args =>
          ...
          | nil =>
              ...
              | nil =>
                  ...
                  change __eo_typeof (__eo_prog_bv_sgt_eliminate a1 a2) = Term.Bool at hResultTy
                  refine ⟨?_, ?_⟩
                  · intro _hTrue
                    change eo_interprets M (__eo_prog_bv_sgt_eliminate a1 a2) true
                    exact facts___eo_prog_bv_sgt_eliminate_impl M hM a1 a2
                      hA1Trans hA2Trans hResultTy
                  · exact RuleProofs.eo_has_smt_translation_of_has_bool_type _
                      (typed___eo_prog_bv_sgt_eliminate_impl a1 a2 hA1Trans hA2Trans
                        hResultTy)
              | cons _ _ =>
                  change Term.Stuck ≠ Term.Stuck at hProg
                  exact False.elim (hProg rfl)
