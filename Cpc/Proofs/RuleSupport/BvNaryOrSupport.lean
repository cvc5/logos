...
theorem binaryArgsSmtType
    ...
    __smtx_typeof (__eo_to_smt x) = SmtType.BitVec w ∧
      __smtx_typeof (__eo_to_smt y) = SmtType.BitVec w :=
  bvor_args_of_bitvec_type x y w
...
theorem evalAssoc
  ...
  rcases _root_.smt_eval_binary_of_smt_type_bitvec M hM (__eo_to_smt z) w hZ with
    ⟨nz, hZE, _⟩
  exact eval_bor_assoc M x y z w nx ny nz hXE hYE hZE
...
theorem evalComm
    ...
    __smtx_model_eval M
      ...
      __smtx_model_eval M
        (__eo_to_smt (Term.Apply (Term.Apply (Term.UOp UserOp.bvor) y) x)) := by
  intro hX hY
  rcases _root_.smt_eval_binary_of_smt_type_bitvec M hM (__eo_to_smt x) w hX with
    ⟨nx, hXE, _⟩
  rcases _root_.smt_eval_binary_of_smt_type_bitvec M hM (__eo_to_smt y) w hY with
...
theorem evalRightNil
    ...
    __smtx_model_eval M
        (__eo_to_smt
          (Term.Apply (Term.Apply (Term.UOp UserOp.bvor) x) nil)) =
      __smtx_model_eval M (__eo_to_smt x) := by
  ...
  subst nnil
  exact eval_bor_right_zero M x nil w nx hXEval hNilEval hXMod
