...
private theorem zero_is_list (w : Nat) :
    __eo_is_list (Term.UOp UserOp.bvor) (zero w) = Term.Boolean true := by
...
private theorem guardedLhs_lists_of_ne_stuck
    ...
    __eo_is_list (Term.UOp UserOp.bvor) xs = Term.Boolean true ∧
      __eo_is_list (Term.UOp UserOp.bvor) ys = Term.Boolean true ∧
      __eo_is_list (Term.UOp UserOp.bvor) zs = Term.Boolean true := by
  intro hLhs
  ...
  have hInnerList :
      __eo_is_list (Term.UOp UserOp.bvor) (guardedInner ys zs second) =
  ...
  have hZs : __eo_is_list (Term.UOp UserOp.bvor) zs = Term.Boolean true :=
...
private theorem lhs_smt_type
    ...
    __eo_is_list (Term.UOp UserOp.bvor) xs = Term.Boolean true ->
    __eo_is_list (Term.UOp UserOp.bvor) ys = Term.Boolean true ->
    __eo_is_list (Term.UOp UserOp.bvor) zs = Term.Boolean true ->
  ...
  have hSecondZsList :
      __eo_is_list (Term.UOp UserOp.bvor) (bor second zs) = Term.Boolean true :=
    eo_is_list_cons_self_true_of_tail_list op second zs
      (by decide) hZsList
  have hSecondZsTy := bvor_smt_type second zs width hSecondTy hZsTy
  have hInnerList :
      __eo_is_list (Term.UOp UserOp.bvor) (inner ys zs second) = Term.Boolean true :=
  ...
  have hInsertedList :
      __eo_is_list (Term.UOp UserOp.bvor) (inserted ys zs first second) = Term.Boolean true :=
